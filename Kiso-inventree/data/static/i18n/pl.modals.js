...
function createNewModal(options={}) {
    ...
    if (!options.hideCloseButton) {
        buttons += `<button type='button' class='btn btn-secondary' id='modal-form-close' data-bs-dismiss='modal'>Anuluj</button>`;
    }
    ...
    modalSetTitle(modal_name, options.title || 'Tytuł formularza');
    modalSetSubmitText(modal_name, options.submitText || 'Wyślij');
    modalSetCloseText(modal_name, options.closeText || 'Anuluj');
...
function loadingMessageContent() {
    ...
    return `<span class='glyphicon glyphicon-refresh glyphicon-refresh-animate'></span> Oczekiwanie na serwer...`;
}
        ...
        <div class='panel'>
            <div class='panel panel-heading'>
                <div class='panel-title'>
                    <a data-bs-toggle='collapse' href="#collapse-error-info">Pokaż informacje o błędzie</a>
                </div>
            </div>
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.title = title;
    options.submitText = options.accept_text || 'Zaakceptuj';
    options.closeText = options.cancel_text || 'Anuluj';
...
function openModal(options) {
    ...
    if (options.title) {
        ...
        modalSetTitle(modal, 'Wczytywanie danych...');
    }
    ...
    modalSubmit(modal, function() {
        $(modal).find('.js-modal-form').ajaxSubmit({
                ...
                if ('form_valid' in response) {
                    ...
                    if (response.form_valid) {
                        ...
                        if (response.html_form) {
                            ...
                            $(modal).modal('hide');
                            showAlertDialog('Niepoprawna odpowiedź serwera', 'Brak danych formularza z odpowiedzi serwera');
                        }
                    }
                } else {
                    $(modal).modal('hide');
            ...
            error: function(xhr) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Błąd podczas wysyłania danych formularza', renderErrorMessage(xhr));
            },
            complete: function() {
                // TODO
            }
    ...
    var ajax_data = {
        ...
        success: function(response) {
            ...
            if (response.html_form) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Niepoprawna odpowiedź serwera', 'Brak danych w formularzu odpowiedzi JSON');
            }
        },
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    'Brak odpowiedzi',
                    'Brak odpowiedzi z serwera InvenTree',
                );
            } else if (xhr.status == 400) {
                showAlertDialog(
                    '400: Nieprawidłowe zapytanie',
                    'Serwer zwrócił kod błędu 400',
                );
            } else if (xhr.status == 401) {
                showAlertDialog(
                    'Błąd 401: Nieuwierzytelniony',
                    'Dane uwierzytelniające nie zostały dostarczone',
                );
            } else if (xhr.status == 403) {
                showAlertDialog(
                    'Błąd 403: Odmowa dostępu',
                    'Nie masz uprawnień wymaganych do dostępu do tej funkcji',
                );
            } else if (xhr.status == 404) {
                showAlertDialog(
                    'Błąd 404: Nie znaleziono zasobu',
                    'Żądany zasób nie mógł być zlokalizowany na serwerze',
                );
            } else if (xhr.status == 408) {
                showAlertDialog(
                    'Błąd 408: Przekroczony limit czasu',
                    'Limit czasu połączenia podczas żądania danych z serwera',
                );
            } else {
                showAlertDialog('Błąd podczas żądania danych formularza', renderErrorMessage(xhr));
            }
