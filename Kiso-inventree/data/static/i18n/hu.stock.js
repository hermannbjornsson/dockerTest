...
function loadStockTestResultsTable(table, options) {
    ...
    function makeButtons(row, grouped) {
        ...
        html += makeIconButton('fa-plus icon-green', 'button-test-add', row.test_name, 'Teszt eredmény hozzáadása');
    ...
    $(table).on('click', '.button-test-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
        <strong>Teszt eredmény törlése:</strong> ${row.test_name || row.test || row.key}
        </div>`;
    ...
    cols.push({
        ...
        formatter: function(details, row) {
            ...
            if (!details || !Object.keys(details).length) {
                return `<small><em>Nem változott</em></small>`;
            }
            ...
            if (details.returnorder) {
                html += `<tr><th>Visszavételi utasítás</th>`;
                html += '<td>';
                ...
                if (details.returnorder_detail) {
                    ...
                    html += `<em>Visszavételi utasítás már nem létezik</em>`;
                }
