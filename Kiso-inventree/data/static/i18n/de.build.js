        ...
        `/api/build/${build_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Sind Sie sicher, dass sie diesen Bauauftrag abbrechen möchten?
                </div>`;

                if (opts.context.has_allocated_stock) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Lagerbestand wurde zu diesem Bauauftrag hinzugefügt
                    </div>`;
                }
                ...
                if (opts.context.incomplete_outputs) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Für diesen Bau-Auftrag sind noch unvollständige Endprodukte vorhanden
                    </div>`;
                }
    ...
    constructForm(`/api/build/${build_id}/finish/`, {
        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                Bauauftrag ist bereit abgeschlossen zu werden'
                </div>`;
            } else {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>Bauauftrag hat unvollständige Aufbauten</strong><br>
                    Dieser Bauauftrag kann nicht abgeschlossen werden, da es unfertige Endprodukte gibt
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    <strong>Bauauftrag ist unvollständig</strong>
                    </div>
                    `;
                }

                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>Benötigter Bestand wurde nicht vollständig zugewiesen</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>Benötigte Teil-Anzahl wurde noch nicht fertiggestellt</div>`;
                }
            }
        ...
        reload: true,
        confirm: true,
        title: 'Bauauftrag fertigstellen',
        method: 'POST',
    });
}
        ...
        {},
        {
            success: function(build) {
                ...
                inventreeGet(`/api/part/${build.part}/serial-numbers/`, {}, {
                    success: function(data) {
                        if (data.next) {
                            fields.serial_numbers.placeholder = `Nächste verfügbare Seriennummer: ${data.next}`;
                        } else if (data.latest) {
                            fields.serial_numbers.placeholder = `Letzte Seriennummer: ${data.latest}`;
                        }
                    },
                    async: false,
                });
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Die Stückliste enthält verfolgbare Teile.<br>
                        Endprodukte müssen individuell angelegt werden.
                    </div>
                    `;
                }
                ...
                if (trackable) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Nachverfolgbare Teile können Seriennummern haben<br>
                        Seriennummeren für mehrere einzelne Endprodukte angeben
                    </div>
                    `;
                }
...
function makeBuildOutputButtons(output_id, build_info, options={}) {
    ...
    if (options.has_tracked_lines) {
        ...
        html += makeIconButton(
            ...
            output_id,
            'Lagerartikel zu diesem Endprodukt zuweisen',
        );
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Seriennummer: ${output.serial}`;
    } else {
        output_html += `Anzahl: ${output.quantity}`;
    ...
    buttons += makeRemoveButton('button-row-remove', pk, 'Zeile entfernen');
...
function completeBuildOutputs(build_id, outputs, options={}) {
    ...
    if (outputs.length == 0) {
        showAlertDialog(
            'Endprodukte auswählen',
            'Mindestens ein Endprodukt muss ausgewählt werden',
        );
        return;
    }
    ...
    constructForm(`/api/build/${build_id}/complete/`, {
        ...
        confirm: true,
        title: 'Endprodukte fertigstellen',
...
function loadBuildOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine Allokationen für Bauauftrag gefunden';
        },
        columns: [
            {
                field: 'pk',
...
function loadBuildOutputTable(build_info, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine aktiven Endprodukte gefunden';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        text = `Seriennummer: ${row.serial}`;
                    } else {
                        text = `Anzahl: ${row.quantity}`;
                    ...
                    if (row.batch) {
                        text += ` <small>(Losnummer: ${row.batch})</small>`;
                    }
...
function allocateStockToBuild(build_id, line_items, options={}) {
    ...
    if (line_items.length == 0) {
        ...
        showAlertDialog(
            'Teile auswählen',
            'Sie müssen mindestens ein Teil auswählen',
        );
    ...
    function renderBuildLineRow(build_line, quantity) {
        ...
        var quantity_input = constructField(
            `items_quantity_${pk}`,
            {
                ...
                min_value: 0,
                value: quantity || 0,
                title: 'Anzahl für Bestandszuordnung eingeben',
                required: true,
            },
            {
                hideLabels: true,
    ...
    if (table_entries.length == 0) {
        ...
        showAlertDialog(
            'Alle Teile zugeordnet',
            'Alle ausgewählten Teile wurden vollständig zugeordnet',
        );
    ...
    html += constructField(
        ...
        {
            type: 'related field',
            label: 'Quell-Lagerort',
            help_text: 'Wählen Sie den Quellort aus (leer lassen um von allen Standorten zu nehmen)',
            required: false,
        },
        {},
    );
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Teil</th>
                <th>Zugeordnet</th>
                <th style='min-width: 250px;'>Lagerartikel</th>
                <th>Anzahl</th>
                <th></th>
            </tr>
        </thead>
    ...
    constructForm(`/api/build/${build_id}/allocate/`, {
        ...
        fields: {},
        preFormContent: html,
        title: 'Lagerartikel für Bauauftrag zuweisen',
        afterRender: function(fields, options) {
            ...
            var take_from_field = {
                ...
                noResults: function(query) {
                    return 'Keine passenden Lagerstandorte';
                },
            };
                ...
                initializeRelatedField(
                    {
                        ...
                        noResults: function(query) {
                            return 'Keine passenden Lagerbestände';
                        }
                    },
                    null,
                    options,
...
function autoAllocateStockToBuild(build_id, bom_items=[], options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
    <strong>Automatische Lagerzuordnung</strong><br>
    Lagerartikel werden automatisch diesem Bauauftrag zugewiesen, entsprechend den angegebenen Richtlinien:
    <ul>
        <li>Wenn ein Lagerort angegeben ist, wird der Lagerbestand nur von diesem Ort zugewiesen</li>
        <li>Wenn der Lagerbestand als austauschbar gilt, wird er vom ersten Standort zugewiesen, an dem er gefunden wird</li>
        <li>Wenn ein Ersatzbestand erlaubt ist, wird es dort verwendet, wo kein Vorrat des Primärteils gefunden werden kann</li>
    </ul>
    </div>
    `;
...
function loadBuildTable(table, options) {
    ...
    $(table).inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Keine Bauaufträge passen zur Anfrage';
        },
        url: '/api/build/',
        queryParams: filters,
        groupBy: false,
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Bauauftrag ist überfällig');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.location_detail) {
                        ...
                        return '<i>Kein Lagerort gesetzt</i>';
                    }
                }
            },
            {
                ...
                formatter: function(value, row) {
                    let buttons = '';
                    buttons += makeEditButton('button-allocation-edit', row.pk, 'Bestands-Zuordnung bearbeiten');
                    buttons += makeDeleteButton('button-allocation-delete', row.pk, 'Bestands-Zuordnung löschen');
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.allow_variants) {
                        html += makeIconBadge('fa-sitemap', 'Varianten erlaubt');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.consumable) {
                        icons += `<span class='fas fa-info-circle icon-blue float-right' title='Verbrauchsartikel'></span>`;
                    } else {
                        if (available < (row.quantity - row.allocated)) {
                            icons += makeIconBadge('fa-times-circle icon-red', 'Unzureichender Bestand verfügbar');
                        } else {
                            icons += makeIconBadge('fa-check-circle icon-green', 'Ausreichender Bestand verfügbar');
                        }

                        if (available <= 0) {
                            icons += `<span class='badge rounded-pill bg-danger'>Kein Lagerbestand verfügbar</span>`;
                        } else {
                            let extra = '';
                            if ((row.available_substitute_stock > 0) && (row.available_variant_stock > 0)) {
                                extra = 'Beinhaltet Variante und Ersatzbestand';
                            } else if (row.available_variant_stock > 0) {
                                extra = 'Beinhaltet Variantenbestand';
                            } else if (row.available_substitute_stock > 0) {
                                extra = 'Enthält Ersatzbestand';
                            }
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `Bestellt: ${formatDecimal(row.on_order)}`);
                    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.allocated < row.quantity) {
                        ...
                        if (row.part_detail.assembly) {
                            buttons += makeIconButton('fa-tools icon-blue', 'button-build', pk, 'Bestand bauen');
                        }

                        // Add a button to "purchase" stock for this line
                        if (row.part_detail.purchaseable) {
                            buttons += makeIconButton('fa-shopping-cart icon-blue', 'button-buy', pk, 'Bestand bestellen');
                        }

                        // Add a button to "allocate" stock for this line
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-allocate', pk, 'Bestand zuweisen');
                    }
