...
function yesNoLabel(value, options={}) {
    ...
    if (toBool(value)) {
        text = options.pass || 'JA';
        color = 'bg-success';
    } else {
        text = options.fail || 'NEIN';
        color = 'bg-warning';
    }
...
function editButton(url, text='Bearbeiten') {
...
function renderClipboard(s, prepend=false) {
    ...
    let clipString = `<span class="d-none d-xl-inline"><button class="btn clip-btn" type="button" data-bs-toggle='tooltip' title='In die Zwischenablage kopieren'><em class="fas fa-copy"></em></button></span>`;
