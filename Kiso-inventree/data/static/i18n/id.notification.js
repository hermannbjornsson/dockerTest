...
function getReadEditButton(pk, state, small=false) {
    ...
    if (state) {
        bReadText = 'Mark as unread';
        bReadIcon = 'fas fa-bookmark icon-red';
        bReadTarget = 'unread';
    } else {
        bReadText = 'Mark as read';
        ...
        },
        {
            success: function(response) {
                if (response.length == 0) {
                    html = `<p class='text-muted'><em>No unread notifications</em><span class='fas fa-check-circle icon-green float-right'></span></p>`;
                } else {
                    // build up items
