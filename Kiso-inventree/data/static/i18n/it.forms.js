    ...
    getApiEndpointOptions(url, function(OPTIONS) {
        ...
        case 'POST':
            if (canCreate(OPTIONS)) {
                ...
                showMessage('Azione Vietata', {
                    style: 'danger',
                    details: 'Crea operazione non consentita',
        ...
        case 'PATCH':
            if (canChange(OPTIONS)) {
                ...
                showMessage('Azione Vietata', {
                    style: 'danger',
                    details: 'Operazione di aggiornamento non consentita',
                    icon: 'fas fa-user-times',
                });
        ...
        case 'DELETE':
            if (canDelete(OPTIONS)) {
                ...
                showMessage('Azione Vietata', {
                    style: 'danger',
                    details: 'Operazione di eliminazione non consentita',
                    icon: 'fas fa-user-times',
                });
        ...
        case 'GET':
            if (canView(OPTIONS)) {
                ...
                showMessage('Azione Vietata', {
                    style: 'danger',
                    details: 'Mostra operazione non consentita',
                    icon: 'fas fa-user-times',
                });
...
function insertPersistButton(options) {
    ...
    var message = options.persistMessage || 'Mantieni aperto questo modulo';
        ...
        if (field) {
            ...
            case 'decimal':
                if (!validateFormField(name, options)) {
                    ...
                    data_errors[name] = ['Inserisci un numero valido'];
                }
                break;
            default:
                break;
...
function handleFormErrors(errors, fields={}, options={}) {
    ...
    non_field_errors.append(
        `<div class='alert alert-block alert-danger'>
            <b>Esistono errori nel modulo</b>
...
function constructField(name, parameters, options={}) {
    ...
    if (extra) {
        ...
        if (!parameters.required && !options.hideClearButton) {
            html += `
            <span class='input-group-text form-clear' id='clear_${field_name}' title='Cancella input'>
...
function selectImportFields(url, data={}, options={}) {
    ...
    var headers = `<tr><th>Colonna File</th><th>Nome del campo</th></tr>`;
