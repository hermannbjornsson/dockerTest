...
function constructOrderTableButtons(options={}) {
    ...
    if (!options.disableCalendarView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_calendar}' title='Mostrar vista de calendario'><span class='fas fa-calendar-alt'></span></button>`,
            event: function() {
    ...
    if (!options.disableListView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_list}' title='Mostrar vista de lista'><span class='fas fa-th-list'></span></button>`,
            event: function() {
                buttonCallback('list');
            }
        });
    ...
    if (!options.disableTreeView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_tree}' title='Mostrar vista de árbol'><span class='fas fa-sitemap'></span></button>`,
            event: function() {
                buttonCallback('tree');
            }
        });
...
function constructExpandCollapseButtons(table, idx=0) {
    ...
    return [
        {
            html: `<button type='button' name='${idx++}' class='btn btn-outline-secondary' title='Ampliar todas las filas'><span class='fas fa-expand'></span></button>`,
            event: function() {
                $(table).bootstrapTable('expandAllRows');
            }
        },
        {
            html: `<button type='button' name='${idx++}' class='btn btn-outline-secondary' title='Contraer todas las filas'><span class='fas fa-compress'></span></button>`,
            event: function() {
...
function downloadTableData(table, opts={}) {
    ...
    constructFormBody({}, {
        title: opts.title || 'Exportar datos de tabla',
        fields: {
            format: {
                label: 'Formato',
    ...
    $.fn.bootstrapTable.locales['en-US-custom'] = {
        ...
        formatShowingRows: function(pageFrom, pageTo, totalRows) {
            ...
            if (totalRows === undefined || isNaN(totalRows)) {
                return 'Mostrar todas las filas';
            } else {
                return `Mostrando ${pageFrom} para ${pageTo} de ${totalRows} filas`;
            }
        },
