        ...
        `/api/build/${build_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Czy na pewno przerwać tę budowę?
                </div>`;
                ...
                inventreeGet(`/api/part/${build.part}/serial-numbers/`, {}, {
                    success: function(data) {
                        if (data.next) {
                            ...
                            fields.serial_numbers.placeholder = `Ostatni numer seryjny: ${data.latest}`;
                        }
                    },
                    async: false,
                });
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Numer Seryjny: ${output.serial}`;
    } else {
        output_html += `Ilość: ${output.quantity}`;
    ...
    buttons += makeRemoveButton('button-row-remove', pk, 'Usuń wiersz');
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.batch) {
                        text += ` <small>(Partia: ${row.batch})</small>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.location_detail) {
                        ...
                        return '<i>Lokacje nie są ustawione</i>';
                    }
                }
            },
            {
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `W Zamówieniu: ${formatDecimal(row.on_order)}`);
                    }
