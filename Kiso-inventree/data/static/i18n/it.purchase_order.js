...
function purchaseOrderFields(options={}) {
    ...
    if (options.duplicate_order) {
        fields.duplicate_order = {
            ...
            help_text: 'Selezione l\'ordine di acquisto da duplicare',
        };
        ...
        fields.duplicate_line_items = {
            ...
            group: 'duplicate',
            type: 'boolean',
            label: 'Duplica linee degli elementi',
            help_text: 'Duplica tutte le linee elementi dall\'ordine selezionato',
        };
        ...
        fields.duplicate_extra_lines = {
            ...
            group: 'duplicate',
            type: 'boolean',
            label: 'Duplica Linee Extra',
            help_text: 'Duplica elementi linee extra dall\'ordine selezionato',
        };
    }
...
function editPurchaseOrder(pk, options={}) {
    ...
    constructForm(`/api/order/po/${pk}/`, {
        fields: fields,
        title: 'Modifica ordine d\'acquisto',
...
function createPurchaseOrder(options={}) {
    ...
    constructForm('/api/order/po/', {
        ...
        title: options.title || 'Crea ordine d\'acquisto',
    });
}
        ...
        part: {
            ...
            onEdit: function(value, name, field, opts) {
                ...
                inventreeGet(`/api/company/part/${value}/`,
                    ...
                    if (pack_quantity != 1) {
                        var txt = `<span class='fas fa-info-circle icon-blue'></span> Quantità Confezione: ${formatDecimal(pack_quantity)} ${units}`;
            ...
            secondary: {
                method: 'POST',
                title: 'Aggiungi fornitore articolo',
...
function completePurchaseOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Completa Ordine D\'Acquisto',
            confirm: true,
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Contrassegnare questo ordine come completato?
                </div>`;

                if (opts.context.is_complete) {
                    html += `
                    <div class='alert alert-block alert-success'>
                        Tutti gli elementi della riga sono stati ricevuti
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Questo ordine ha elementi di riga che non sono stati contrassegnati come ricevuti.</br>
                        Completare questo ordine significa che l\'ordine e gli elementi della riga non saranno più modificabili.
                    </div>`;
                }
...
function cancelPurchaseOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Annulla Ordine di Acquisto',
            confirm: true,
            preFormContent: function(opts) {
                var html = `
                <div class='alert alert-info alert-block'>
                    Sei sicuro di voler annullare questo ordine di acquisto?
                </div>`;

                if (!opts.context.can_cancel) {
                    html += `
                    <div class='alert alert-danger alert-block'>
                        Questo ordine d\'acquisto non può essere cancellato
                    </div>`;
                }
...
function issuePurchaseOrder(order_id, options={}) {
    ...
    let html = `
    <div class='alert alert-block alert-warning'>
    Dopo aver effettuato questo ordine, gli elementi della riga non saranno più modificabili.
    </div>`;

    constructForm(`/api/order/po/${order_id}/issue/`, {
        method: 'POST',
        title: 'Problema Ordine di Acquisto',
        confirm: true,
        preFormContent: html,
        onSuccess: function(response) {
            handleFormSuccess(response, options);
...
function orderParts(parts_list, options={}) {
    ...
    if (parts.length == 0) {
        showAlertDialog(
            'Seleziona Articoli',
            'Deve essere selezionata almeno un articolo acquistabile',
        );
        return;
    }
    ...
    function renderPart(part, opts={}) {
        ...
        var supplier_part_prefix = `
            <button type='button' class='input-group-text button-row-new-sp' pk='${pk}' title='Nuovo articolo fornitore'>
        ...
        var purchase_order_prefix = `
            <button type='button' class='input-group-text button-row-new-po' pk='${pk}' title='Nuovo ordine d\'acquisto'>
                <span class='fas fa-plus-circle icon-green'></span>
            </button>
        `;
    ...
    <table class='table table-striped table-condensed' id='order-parts-table'>
        <thead>
            <tr>
                <th>Articolo</th>
                <th style='min-width: 300px;'>Articolo Fornitore</th>
                <th style='min-width: 300px;'>Ordine D\'Acquisto</th>
                <th style='min-width: 50px;'>Quantità</th>
                <th><!-- Actions --></th>
            </tr>
        </thead>
        <tbody>
            ...
            parts.forEach(function(part) {
                ...
                var onSupplierPartChanged = function(value, name, field, opts) {
                    ...
                    if (value != null) {
                        inventreeGet(
                            ...
                            if (pack_quantity != 1) {
                                var txt = `<span class='fas fa-info-circle icon-blue'></span> Quantità Confezione: ${pack_quantity} ${units}`;
                ...
                var supplier_part_field = {
                    ...
                    noResults: function(query) {
                        return 'Nessun fornitore articolo corrispondente';
                    }
                };
                ...
                initializeRelatedField({
                    ...
                    noResults: function(query) {
                        return 'Nessun ordine di acquisto corrispondente trovato';
                    }
                }, null, opts);
...
function receivePurchaseOrderItems(order_id, line_items, options={}) {
    ...
    if (line_items.length == 0) {
        ...
        showAlertDialog(
            'Seleziona Linee Elementi',
            'È necessario selezionare almeno una linea elemento',
        );
        return;
    }
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        if (native_pack_quantity != 1) {
            ...
            <div class='alert alert-small alert-block alert-info'>
                Quantità Confezione: ${pack_quantity}<br>
                Quantità Ricevuta: <span class='pack_received_quantity' id='items_received_quantity_${pk}'>${received}</span> ${units}
            </div>`;
        }
        ...
        var batch_input = constructField(
            `items_batch_code_${pk}`,
            {
                ...
                required: false,
                label: 'Codice Lotto',
                help_text: 'Inserisci il codice univoco per gli articoli in arrivo',
                icon: 'fa-layer-group',
            },
            {
                hideLabels: true,
        ...
        var sn_input = constructField(
            `items_serial_numbers_${pk}`,
            {
                ...
                help_text: 'Inserisci i numeri di serie per gli articoli stock in arrivo',
                icon: 'fa-hashtag',
            },
            {
                hideLabels: true,
        ...
        if (global_settings.BARCODE_ENABLE) {
            buttons += makeIconButton('fa-qrcode', 'button-row-add-barcode', pk, 'Aggiungi codice a barre');
            buttons += makeIconButton('fa-unlink icon-red', 'button-row-remove-barcode', pk, 'Rimuovi il codice a barre', {hidden: true});
        }

        buttons += makeIconButton('fa-sitemap', 'button-row-add-location', pk, 'Specifica la posizione', {
    ...
    <table class='table table-condensed' id='order-receive-table'>
        <thead>
            <tr>
                ...
                <th>Codice ordine</th>
                <th>Ricevuto</th>
                <th style='min-width: 50px;'>Quantità da Ricevere</th>
    ...
    constructForm(`/api/order/po/${order_id}/receive/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Conferma la ricezione degli elementi',
        title: 'Ricevi Elementi Ordine D\'Acquisto',
        afterRender: function(fields, opts) {
            ...
            if (global_settings.BARCODE_ENABLE) {
                $(opts.modal).find('.button-row-add-barcode').click(function() {
                    ...
                    barcodeDialog('Scansiona codice a barre', {
                        details: 'Scansiona il codice a barre sull\'elemento in arrivo (non deve corrispondere a nessun articolo disponibile esistente)',
                        onScan: function(response, barcode_options) {
                            ...
                            showBarcodeMessage(barcode_options.modal, 'Il codice a barre corrisponde a un elemento esistente');
                        },
                        onError400: function(response, barcode_options) {
                            if (response.barcode_data && response.barcode_hash) {
                                ...
                                showBarcodeMessage(barcode_options.modal, 'Dati codice a barre non validi');
                            }
                        }
                    });
                });
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'L\'Ordine è in ritardo');
                    }
...
function deletePurchaseOrderLineItems(items, options={}) {
    ...
    <table class='table table-striped table-condensed'>
        <tr>
            ...
            <th>Descrizione</th>
            <th>SKU</th>
            <th>Codice articolo produttore (MPN)</th>
            <th>Quantità</th>
        </tr>
        ${rows}
    </table>
...
function loadPurchaseOrderLineItemTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        sidePagination: 'server',
        formatNoMatches: function() {
            return 'Nessuna linea elementi trovata';
        },
        queryParams: filters,
        original: options.params,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Quantità Confezione: ${pack_quantity} - Quantità Totale: ${total}${units}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.target_date) {
                        ...
                        if (row.overdue) {
                            html += makeIconBadge('fa-calendar-times icon-red', 'Questo elemento è in ritardo');
                        }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (options.allow_receive && row.received < row.quantity) {
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-line-receive', pk, 'Ricevi linea elemento');
                    }

                    if (options.allow_edit) {
                        buttons += makeCopyButton('button-line-duplicate', pk, 'Duplica linea elemento');
                        buttons += makeEditButton('button-line-edit', pk, 'Modifica linea elemento');
                        buttons += makeDeleteButton('button-line-delete', pk, 'Cancella linea elemento');
                    }
