...
function newBuildOrder(options={}) {
    ...
    constructForm(`/api/build/`, {
        ...
        follow: true,
        method: 'POST',
        title: 'Crea Ordine di Produzione',
...
function cancelBuildOrder(build_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Annulla Ordine Di Produzione',
            confirm: true,
            fields: {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Sei sicuro di voler annullare questa produzione?
                </div>`;

                if (opts.context.has_allocated_stock) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Gli elementi di magazzino è stata assegnata a questo ordine di produzione
                    </div>`;
                }
                ...
                if (opts.context.incomplete_outputs) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Ci sono output incompleti rimanenti per questo ordine di produzione
                    </div>`;
                }
    ...
    constructForm(`/api/build/${build_id}/finish/`, {
        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                L\'ordine di produzione è pronto per essere completato'
                </div>`;
            } else {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>L\'ordine di produzione ha output incompleti</strong><br>
                    Questo ordine di produzione non può essere completato in quanto ci sono output incompleti
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    <strong>L\'Ordine di Produzione è incompleto</strong>
                    </div>
                    `;
                }

                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>La giacenza richiesta non è stata completamente assegnata</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>La quantità di produzione richiesta non è stata completata</div>`;
                }
            }
        ...
        reload: true,
        confirm: true,
        title: 'Completa l\'Ordine di Produzione',
        method: 'POST',
    });
}
        ...
        {},
        {
            success: function(build) {
                ...
                inventreeGet(`/api/part/${build.part}/serial-numbers/`, {}, {
                    success: function(data) {
                        if (data.next) {
                            fields.serial_numbers.placeholder = `Il prossimo numero di serie disponibile è: ${data.next}`;
                        } else if (data.latest) {
                            fields.serial_numbers.placeholder = `Ultimo Numero Di Serie: ${data.latest}`;
                        }
                    },
                    async: false,
                });
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        La distinta base contiene articoli tracciabili.<br>
                        Gli outputs della produzione devono essere generati singolarmente.
                    </div>
                    `;
                }
                ...
                if (trackable) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Gli articoli tracciabili possono avere numeri di serie specificati<br>
                        Inserisci i numeri seriali per generare più output di produzione
                    </div>
                    `;
                }
                ...
                constructForm(`/api/build/${build_id}/create-output/`, {
                    method: 'POST',
                    title: 'Crea Output di Produzione',
                    confirm: true,
                    fields: fields,
                    preFormContent: html,
                    onSuccess: function(response) {
...
function makeBuildOutputButtons(output_id, build_info, options={}) {
    ...
    if (options.has_tracked_lines) {
        ...
        html += makeIconButton(
            ...
            output_id,
            'Assegna gli elementi di magazzino a questo output di produzione',
        );
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Numero Seriale: ${output.serial}`;
    } else {
        output_html += `Quantità: ${output.quantity}`;
    ...
    buttons += makeRemoveButton('button-row-remove', pk, 'Elimina riga');
...
function completeBuildOutputs(build_id, outputs, options={}) {
    ...
    if (outputs.length == 0) {
        showAlertDialog(
            'Seleziona Output di produzione',
            'Almeno un output di produzione deve essere selezionato',
        );
        return;
    }
    ...
    constructForm(`/api/build/${build_id}/complete/`, {
        ...
        confirm: true,
        title: 'Completa l\'output di produzione',
...
function deleteBuildOutputs(build_id, outputs, options={}) {
    ...
    constructForm(`/api/build/${build_id}/delete-outputs/`, {
        ...
        fields: {},
        confirm: true,
        title: 'Cancella l\'output di produzione',
        afterRender: function(fields, opts) {
            // Setup callbacks to remove outputs
            $(opts.modal).find('.button-row-remove').click(function() {
                var pk = $(this).attr('pk');
...
function loadBuildOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nessuna allocazione per l\'ordine di produzione trovato';
        },
        columns: [
            {
                field: 'pk',
...
function loadBuildOutputTable(build_info, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nessun output di produzione attivo trovato';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        text = `Numero Seriale: ${row.serial}`;
                    } else {
                        text = `Quantità: ${row.quantity}`;
                    ...
                    if (row.batch) {
                        text += ` <small>(Lotto: ${row.batch})</small>`;
                    }
...
function allocateStockToBuild(build_id, line_items, options={}) {
    ...
    if (line_items.length == 0) {
        ...
        showAlertDialog(
            'Seleziona Articoli',
            'È necessario selezionare almeno un articolo da assegnare',
        );
    ...
    function renderBuildLineRow(build_line, quantity) {
        ...
        var quantity_input = constructField(
            `items_quantity_${pk}`,
            {
                ...
                min_value: 0,
                value: quantity || 0,
                title: 'Specificare il quantitativo assegnato allo stock',
                required: true,
            },
            {
                hideLabels: true,
    ...
    if (table_entries.length == 0) {
        ...
        showAlertDialog(
            'Tutti gli articoli assegnati',
            'Tutti gli articoli selezionati sono stati completamente assegnati',
        );
    ...
    html += constructField(
        'take_from',
        {
            ...
            help_text: 'Seleziona la posizione di origine (lascia vuoto per prendere da tutte le posizioni)',
            required: false,
        },
        {},
    );
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Articolo</th>
                <th>Allocato</th>
                <th style='min-width: 250px;'>Articoli in magazzino</th>
                <th>Quantità</th>
                <th></th>
            </tr>
        </thead>
    ...
    constructForm(`/api/build/${build_id}/allocate/`, {
        ...
        fields: {},
        preFormContent: html,
        title: 'Assegna gli Elementi Stock all\'Ordine di Produzione',
        afterRender: function(fields, options) {
            ...
            var take_from_field = {
                ...
                noResults: function(query) {
                    return 'Nessuna posizione di magazzino corrispondente';
                },
            };
                ...
                initializeRelatedField(
                    {
                        ...
                        noResults: function(query) {
                            return 'Nessun elemento corrispondente trovato';
                        }
                    },
                    null,
                    options,
...
function autoAllocateStockToBuild(build_id, bom_items=[], options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
    <strong>Assegna Automaticamente Scorte</strong><br>
    Gli elementi in magazzino saranno automaticamente assegnati a questo ordine di produzione, secondo le linee guida fornite:
    <ul>
        <li>Se viene specificata una posizione, le scorte saranno assegnate solo da quella ubicazione</li>
        <li>Se lo stock è considerato intercambiabile, sarà assegnato dal primo luogo in cui viene trovato</li>
        <li>Se lo stock sostitutivo è ammesso, sarà utilizzato nel caso in cui lo stock dell\'articolo primario non possa essere trovato</li>
    </ul>
    </div>
    `;
    ...
    constructForm(`/api/build/${build_id}/auto-allocate/`, {
        method: 'POST',
        fields: fields,
        title: 'Assegna Elementi di Magazzino',
        confirm: true,
        preFormContent: html,
        onSuccess: function(response) {
            if (options.onSuccess) {
...
function loadBuildTable(table, options) {
    ...
    $(table).inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Nessuna produzione corrispondente alla ricerca';
        },
        url: '/api/build/',
        queryParams: filters,
        groupBy: false,
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'L\'ordine di produzione è in ritardo');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.location_detail) {
                        ...
                        return '<i>Nessuna posizione impostata</i>';
                    }
                }
            },
            {
                ...
                formatter: function(value, row) {
                    let buttons = '';
                    buttons += makeEditButton('button-allocation-edit', row.pk, 'Modifica allocazione magazzino');
                    buttons += makeDeleteButton('button-allocation-delete', row.pk, 'Elimina posizione giacenza');
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.allow_variants) {
                        html += makeIconBadge('fa-sitemap', 'Variante stock consentita');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.consumable) {
                        icons += `<span class='fas fa-info-circle icon-blue float-right' title='Elementi consumabili'></span>`;
                    } else {
                        if (available < (row.quantity - row.allocated)) {
                            icons += makeIconBadge('fa-times-circle icon-red', 'Scorte insufficienti disponibili');
                        } else {
                            icons += makeIconBadge('fa-check-circle icon-green', 'Scorte sufficienti disponibili');
                        }

                        if (available <= 0) {
                            icons += `<span class='badge rounded-pill bg-danger'>Nessuna Scorta Disponibile</span>`;
                        } else {
                            let extra = '';
                            if ((row.available_substitute_stock > 0) && (row.available_variant_stock > 0)) {
                                extra = 'Include variante e scorte sostitutive';
                            } else if (row.available_variant_stock > 0) {
                                extra = 'Comprende varianti magazzino';
                            } else if (row.available_substitute_stock > 0) {
                                extra = 'Comprende le scorte sostitutive';
                            }
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `Ordinato: ${formatDecimal(row.on_order)}`);
                    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.allocated < row.quantity) {
                        ...
                        if (row.part_detail.assembly) {
                            buttons += makeIconButton('fa-tools icon-blue', 'button-build', pk, 'Produci scorta');
                        }

                        // Add a button to "purchase" stock for this line
                        if (row.part_detail.purchaseable) {
                            buttons += makeIconButton('fa-shopping-cart icon-blue', 'button-buy', pk, 'Ordina scorta');
                        }

                        // Add a button to "allocate" stock for this line
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-allocate', pk, 'Assegna scorta');
                    }
