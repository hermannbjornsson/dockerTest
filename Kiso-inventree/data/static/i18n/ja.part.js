...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>在庫切れ</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>在庫切れ</span>`;
    }

    rows += `<tr><td><b>在庫</b></td><td>${stock}</td></tr>`;
