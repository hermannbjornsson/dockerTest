...
function createSalesOrder(options={}) {
    ...
    constructForm('/api/order/so/', {
        method: 'POST',
        fields: fields,
        title: 'Vevői rendelés létrehozása',
...
function editSalesOrder(order_id, options={}) {
    ...
    constructForm(`/api/order/so/${order_id}/`, {
        ...
        title: 'Vevői rendelés szerkesztése',
    ...
    inventreeGet(`/api/order/so/shipment/${shipment_id}/`, {}, {
        success: function(shipment) {
            ...
            if (!allocations || allocations.length == 0) {
                ...
                Ehhez a szállítmányhoz nincs készlet hozzárendelve
                </div>
                `;
            } else {
                html = `
                A következő készlet tételek ki lesznek szállítva
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        stock = `Sorozatszám: ${allocation.serial}`;
                    } else {
                        stock = `Mennyiség: ${allocation.quantity}`;
                    }
            ...
            constructForm(`/api/order/so/shipment/${shipment_id}/ship/`, {
                method: 'POST',
                title: `Függő szállítmányok kiszállítása ${shipment.reference}`,
                fields: {
                    shipment_date: {
                ...
                preFormContent: html,
                confirm: true,
                confirmMessage: 'Szállítmány megerősítése',
...
function completePendingShipments(order_id, options={}) {
    ...
    if (allocated_shipments.length > 0) {
        ...
        if (!pending_shipments.length) {
            html += `
            Nincs függő szállítmány
            `;
        } else {
            html += `
            A függő a szállítmányokhoz nincs készlet hozzárendelve
            `;
        }
        ...
        constructForm(`/api/order/so/shipment/0/ship/`, {
            method: 'POST',
            title: 'Függő szállítmányok kiszállítása',
            preFormContent: html,
...
function completeSalesOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Vevői rendelés befejezése, minden kiszállítva',
            confirm: true,
            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.pending_shipments) {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    A rendelés nem jelölhető késznek mivel függő szállítmányok vannak<br>
                    </div>`;
                }
                ...
                if (!opts.context.is_complete) {
                    ...
                    Ez a rendelés olyan sortételeket tartalmaz amik még nem teljesítettek.<br>
...
function issueSalesOrder(order_id, options={}) {
    ...
    let html = `
    <div class='alert alert-block alert-info'>
    Vissza lett igazolva ez a vevői rendelés?
    </div>`;

    constructForm(`/api/order/so/${order_id}/issue/`, {
        method: 'POST',
        title: 'Vevői rendelés visszaigazolása',
        confirm: true,
        preFormContent: html,
        onSuccess: function(response) {
            handleFormSuccess(response, options);
...
function cancelSalesOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Vevő rendelés törlése',
            confirm: true,
            preFormContent: function(opts) {
                var html = `
                <div class='alert alert-block alert-warning'>
                A rendelés törlésével annak adatai a továbbiakban már nem lesznek szerkeszthetők.
                </div>`;
...
function loadSalesOrderTable(table, options) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nem található vevői rendelés';
        },
...
function loadSalesOrderShipmentTable(table, options={}) {
    ...
    function makeShipmentActions(row) {
        ...
        html += makeEditButton('button-shipment-edit', pk, 'Szállítmány szerkesztése');

        if (!options.shipped) {
            html += makeIconButton('fa-truck icon-green', 'button-shipment-ship', pk, 'Szállítmány kiszállítása');
        }

        var enable_delete = row.allocations && row.allocations.length == 0;

        html += makeDeleteButton('button-shipment-delete', pk, 'Szállítmány törlése', {disabled: !enable_delete});
    ...
    function setupShipmentCallbacks() {
        ...
        $(table).find('.button-shipment-edit').click(function() {
            ...
            constructForm(`/api/order/so/shipment/${pk}/`, {
                fields: fields,
                title: 'Szállítmány szerkesztése',
                refreshTable: table,
            });
        });
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nincs egyező szállímány';
        },
        columns: [
            {
                visible: false,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (value) {
                        ...
                        return '<em>Nincs kiszállítva</em>';
                    }
                }
            },
            {
...
function allocateStockToSalesOrder(order_id, line_items, options={}) {
    ...
    constructForm(`/api/order/so/${order_id}/allocate/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Készlet foglalás megerősítése',
        title: 'Készlet foglalása a vevői rendeléshez',
...
function loadSalesOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        original: options.params,
        formatNoMatches: function() {
            return 'Nincs vevői rendeléshez történő foglalás';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    var text = `Készlet tétel ${row.item}`;
    ...
    function setupCallbacks() {
        ...
        table.find('.button-allocation-edit').click(function() {
            ...
            constructForm(
                `/api/order/so-allocation/${pk}/`,
                {
                    ...
                    title: 'Készlet foglalások szerkesztése',
        ...
        table.find('.button-allocation-delete').click(function() {
            ...
            constructForm(
                ...
                {
                    method: 'DELETE',
                    confirmMessage: 'Törlési művelet megerősítése',
                    title: 'Készlet foglalások törlése',
                    refreshTable: options.table,
                }
            );
        });
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `Mennyiség: ${row.quantity}`;

                    if (item && item.serial != null && row.quantity == 1) {
                        text = `Sorozatszám: ${item.serial}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        return `<em>Vevőnek kiszállítva - ${row.shipment_date}</em>`;
                    } else if (row.location) {
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || 'Hely',
                            `/stock/location/${row.location}/`
                        );
                    } else {
                        return `<em>Készlethely nincs megadva</em>`;
                    }
                },
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        html += `<span class='badge bg-success badge-right'>Kiszállítva</span>`;
                    } else {
                        html += makeEditButton('button-allocation-edit', pk, 'Készlet foglalások szerkesztése');
                        html += makeDeleteButton('button-allocation-delete', pk, 'Készlet foglalások törlése');
                    }
        ...
        },
        {
            ...
            formatter: function(value, row) {
                if (row.target_date) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times', 'Ez a sortétel késésben van');
                    }
        ...
        columns.push(
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (available > 0) {
                        ...
                        html += `<span class='badge rounded-pill bg-danger'>Nincs szabad</span>`;
                    }

                    if (required > 0) {
                        if (available >= required) {
                            html += makeIconBadge('fa-check-circle icon-green', 'Van elegendő');
                        } else {
                            html += makeIconBadge('fa-times-circle icon-red', 'Nincs elegendő');
                        }
                    }
        ...
        formatter: function(value, row, index, field) {
            ...
            if (row.part && row.part_detail) {
                ...
                if (options.allow_edit && (row.shipped < row.quantity)) {
                    if (part.trackable) {
                        buttons += makeIconButton('fa-hashtag icon-green', 'button-add-by-sn', pk, 'Sorozatszámok kiosztása');
                    }
                    buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-add', pk, 'Lefoglalt készlet');
                    if (part.purchaseable) {
                        buttons += makeIconButton('fa-shopping-cart', 'button-buy', row.part, 'Készletrendelés');
                    }

                    if (part.assembly) {
                        buttons += makeIconButton('fa-tools', 'button-build', row.part, 'Gyártási készlet');
                    }
                }
            }

            buttons += makeIconButton('fa-dollar-sign icon-green', 'button-price', pk, 'Árszámítás');

            if (options.allow_edit) {
                buttons += makeCopyButton('button-duplicate', pk, 'Sortétel másolása');
                buttons += makeEditButton('button-edit', pk, 'Sortétel szerkesztése');
            }

            if (options.allow_delete) {
                var delete_disabled = false;

                var title = 'Sortétel törlése';

                if (row.shipped) {
                    delete_disabled = true;
                    title = 'Nem törölhető mivel a tételek ki lettek szállítva';
                } else if (row.allocated) {
                    delete_disabled = true;
                    title = 'Nem törölhető mivel tételek vannak lefoglalva';
                }
