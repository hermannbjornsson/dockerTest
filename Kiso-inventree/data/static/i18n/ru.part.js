...
function partGroups() {
    ...
    return {
        attributes: {
            title: 'Атрибуты детали',
            collapsible: true,
        },
        create: {
            title: 'Настройки создания детали',
            collapsible: true,
        },
        duplicate: {
            title: 'Настройки дублирования детали',
            collapsible: true,
        },
        initial_stock: {
            title: 'Initial Stock',
...
function createPartCategory(options={}) {
    ...
    constructForm('/api/part/category/', {
        fields: fields,
        method: 'POST',
        title: 'Создать категорию деталей',
        follow: true,
        persist: true,
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        ...
        persist: true,
        persistMessage: 'Создать ещё одну деталь после этой',
        successMessage: 'Деталь создана успешно',
...
function duplicatePart(pk, options={}) {
    ...
    var title = 'Дублировать деталь';

    if (options.variant) {
        title = 'Создать разновидность детали';
    }
                ...
                },
                {
                    ...
                    success: function(response) {
                        if (response.starred) {
                            ...
                            $(options.button).attr('title', 'Вы подписаны на уведомления для данного элемента');

                            showMessage('Вы подписались на уведомления для данного элемента', {
                                style: 'success',
                            });
                        } else {
                            $(options.button).removeClass('fas fa-bell icon-green').addClass('fa fa-bell-slash');
                            $(options.button).attr('title', 'Включить уведомления для данного элемента');

                            showMessage('Вы отписались от уведомлений для данного элемента', {
                                style: 'warning',
                            });
                        }
                    }
...
function makePartIcons(part) {
    ...
    if (part.trackable) {
        html += makeIconBadge('fa-directions', 'Отслеживаемая деталь');
    }
    ...
    if (part.is_template) {
        html += makeIconBadge('fa-clone', 'Деталь-шаблон');
    }
...
function loadPartVariantTable(table, partId, options={}) {
    ...
    table.inventreeTable({
        ...
        queryParams: filters,
        formatNoMatches: function() {
            return 'Разновидности не найдены';
        },
        columns: cols,
        treeEnable: true,
        rootParentId: partId,
...
function createPartParameter(part_id, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Добавить параметр';
...
function editPartParameter(param_id, options={}) {
    options.fields = partParameterFields();
    options.title = 'Редактировать параметр';
    options.focus = 'data';
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-parameter-edit', pk, 'Редактировать параметр');
                    html += makeDeleteButton('button-parameter-delete', pk, 'Удалить параметр');
...
function loadPartParameterTemplateTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Шаблоны параметров детали не найдены';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    buttons += makeEditButton('template-edit', row.pk, 'Редактировать шаблон');
                    buttons += makeDeleteButton('template-delete', row.pk, 'Удалить шаблон');
...
function loadPartPurchaseOrderTable(table, part_id, options={}) {
    ...
    $(table).inventreeTable({
        ...
        uniqueId: 'pk',
        formatNoMatches: function() {
            return 'Заказов на закупку не найдено';
        },
        onPostBody: function() {
...
function partGridTile(part) {
    ...
    rows += `<tr><td><b>Склад</b></td><td>${stock}</td></tr>`;
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Количество: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
