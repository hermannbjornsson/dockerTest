            ...
            if (!allocations || allocations.length == 0) {
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        stock = `Seri Numara: ${allocation.serial}`;
                    } else {
                        stock = `Miktar: ${allocation.quantity}`;
                    }
...
function allocateStockToSalesOrder(order_id, line_items, options={}) {
    ...
    constructForm(`/api/order/so/${order_id}/allocate/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Stok tahsisini onayla',
    ...
    function setupCallbacks() {
        ...
        table.find('.button-allocation-delete').click(function() {
            ...
            constructForm(
                ...
                {
                    method: 'DELETE',
                    confirmMessage: 'Silme İşlemini Onayla',
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `Miktar: ${row.quantity}`;

                    if (item && item.serial != null && row.quantity == 1) {
                        text = `Seri Numara: ${item.serial}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        ...
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || 'Konum',
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        html += `<span class='badge bg-success badge-right'>Sevk edildi</span>`;
                    } else {
                        html += makeEditButton('button-allocation-edit', pk, 'Stok tahsisini düzenle');
                        html += makeDeleteButton('button-allocation-delete', pk, 'Stok tahsisini sil');
                    }
        ...
        formatter: function(value, row, index, field) {
            ...
            if (row.part && row.part_detail) {
                ...
                if (options.allow_edit && (row.shipped < row.quantity)) {
                    if (part.trackable) {
                        buttons += makeIconButton('fa-hashtag icon-green', 'button-add-by-sn', pk, 'Seri numaralarını tahsis et');
                    }
            ...
            inventreeGet(`/api/order/so-line/${pk}/`, {},
                {
                    success: function(response) {
                        ...
                        constructForm(`/api/order/so/${options.order}/allocate-serials/`, {
                            method: 'POST',
                            title: 'Seri Numaralarını Tahsis Et',
                            fields: {
                                line_item: {
                                    value: pk,
                                    hidden: true,
