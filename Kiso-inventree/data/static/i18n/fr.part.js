...
function partGroups() {
    ...
    return {
        attributes: {
            title: 'Attributs de la pièce',
            collapsible: true,
        },
        create: {
            title: 'Options de création de pièce',
            collapsible: true,
        },
        duplicate: {
            title: 'Options de duplication de pièces',
            collapsible: true,
        },
        initial_stock: {
            title: 'Initial Stock',
        ...
        initial_supplier: {
            title: 'Données initiales du fournisseur',
            collapsible: true,
...
function partFields(options={}) {
    ...
    var fields = {
        category: {
            secondary: {
                title: 'Ajouter une catégorie de pièce',
                fields: function() {
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        ...
        successMessage: 'Composant créé avec succès',
...
function duplicatePart(pk, options={}) {
    ...
    if (options.variant) {
        title = 'Créer une variante de pièce';
    }
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            elements.push(`Stock bas: ${part.total_in_stock}`);
    ...
    if (part.ordering) {
        elements.push(`En Commande: ${part.ordering}`);
    }
...
function makePartIcons(part) {
    ...
    if (part.trackable) {
        html += makeIconBadge('fa-directions', 'Pièce traçable');
    }

    if (part.virtual) {
        html += makeIconBadge('fa-ghost', 'Pièce virtuelle');
    }
    ...
    if (part.salable) {
        html += makeIconBadge('fa-dollar-sign', 'Pièce vendable');
    }
...
function loadPartVariantTable(table, partId, options={}) {
    ...
    table.inventreeTable({
        ...
        queryParams: filters,
        formatNoMatches: function() {
            return 'Aucune variante trouvée';
        },
        columns: cols,
        treeEnable: true,
        rootParentId: partId,
...
function createPartParameter(part_id, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Ajouter un paramètre';
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Nombre de paquet: ${row.pack_quantity} - Total Quantity: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let total = value * row.supplier_part_detail.pack_quantity_native;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Nombre de paquet: ${row.pack_quantity} - Total Quantity: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Reçu</span>`;
...
function partGridTile(part) {
    ...
    if (part.ordering) {
        rows += `<tr><td><b>En Commande</b></td><td>${part.ordering} ${units}</td></tr>`;
    }
...
function makePartActions(table) {
    ...
    return [
        ...
        {
            label: 'order',
            title: 'Commander des composants',
    ...
    $(table).inventreeTable({
        ...
        buttons: options.gridView ? [
            ...
            },
            {
                icon: 'fas fa-th',
                attributes: {
                    title: 'Afficher sous forme de grille',
                    id: 'view-part-grid',
                },
                event: () => {
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Quantité: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
