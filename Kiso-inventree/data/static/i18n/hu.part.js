...
function partGroups() {
    ...
    return {
        attributes: {
            title: 'Alkatrész tulajdonságok',
            collapsible: true,
        },
        create: {
            title: 'Alkatrész létrehozási opciók',
            collapsible: true,
        },
        duplicate: {
            title: 'Alkatrész másolási opciók',
            collapsible: true,
        },
        initial_stock: {
            title: 'Kezdeti készlet',
            collapsible: true,
            hidden: !global_settings.PART_CREATE_INITIAL,
        },
        initial_supplier: {
            title: 'Kezdeti beszállítói adatok',
            collapsible: true,
...
function partFields(options={}) {
    ...
    var fields = {
        category: {
            secondary: {
                title: 'Alkatrész kategória hozzáadása',
                fields: function() {
...
function categoryFields(options={}) {
    let fields = {
        parent: {
            help_text: 'Felsőbb szintű alkatrész kategória',
            required: false,
        },
        name: {},
        description: {},
...
function createPartCategory(options={}) {
    ...
    constructForm('/api/part/category/', {
        fields: fields,
        method: 'POST',
        title: 'Alkatrész kategória létrehozása',
        follow: true,
        persist: true,
        persistMessage: 'Új kategória létrehozása ez után',
        successMessage: 'Alkatrész kategória létrehozva'
    });
}
...
function editCategory(pk) {
    ...
    constructForm(url, {
        fields: fields,
        title: 'Alkatrész kategória szerkesztése',
        reload: true,
    });
}
...
function deletePartCategory(pk, options={}) {
    ...
    Biztos hogy törölni szeretnéd ezt az alkatrész kategóriát?
    </div>`;
    var subChoices = [
        {
            value: 0,
            display_name: 'Áthelyezés fentebbi kategóriába',
        },
        {
            value: 1,
            display_name: 'Törlés',
    ...
    constructForm(url, {
        title: 'Alkatrész kategória törlése',
        method: 'DELETE',
        fields: {
            'delete_parts': {
                label: 'A kategóriában lévő alkatrészek kezelése',
                choices: subChoices,
                type: 'choice'
            },
            'delete_child_categories': {
                label: 'Alkategóriák kezelése',
                choices: subChoices,
                type: 'choice'
            },
        },
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        ...
        groups: partGroups(),
        title: 'Alkatrész létrehozása',
        persist: true,
        persistMessage: 'Új alkatrész létrehozása ez után',
        successMessage: 'Alkatrész sikeresen létrehozva',
...
function editPart(pk) {
    ...
    constructForm(url, {
        ...
        reload: true,
        successMessage: 'Alkatrész módosítva',
    });
}
...
function duplicatePart(pk, options={}) {
    ...
    var title = 'Alkatrész másolása';

    if (options.variant) {
        title = 'Alkatrész változat létrehozása';
    }
    ...
    inventreeGet(`/api/part/${pk}/`, {}, {
        success: function(part) {
            if (part.active) {
                showAlertDialog(
                    'Aktív alkatrész',
                    'Alkatrész nem törölhető mivel még aktív',
                    {
                        alert_style: 'danger',
                    }
                );
            ...
            Ezen alkatrész törlése nem vonható vissza
            <ul>
            <li>Ennek az alkatrésznek a teljes készlete törölve lesz</li>
            <li>Ez az alkatrész minden alkatrészjegyzékből törölve lesz</li>
            <li>Ehhez az alkatrészhez rendelt minden beszállítói és gyártói információ törölve lesz</li>
            </div>`;
                ...
                },
                {
                    ...
                    success: function(response) {
                        if (response.starred) {
                            ...
                            $(options.button).attr('title', 'Értesítések beállítva erre a tételre');

                            showMessage('Értesítések beállítva erre a tételre', {
                                style: 'success',
                            });
                        } else {
                            $(options.button).removeClass('fas fa-bell icon-green').addClass('fa fa-bell-slash');
                            $(options.button).attr('title', 'Értesítések kérése erre a tételre');

                            showMessage('Értesítések letiltva erre a tételre', {
                                style: 'warning',
                            });
                        }
                    }
...
function validateBom(part_id, options={}) {
    ...
    Az alkatrészjegyzék jóváhagyása minden sortételt jóvá fog hagyni
    </div>
    `;
    ...
    constructForm(`/api/part/${part_id}/bom-validate/`, {
        ...
        preFormContent: html,
        title: 'Alkatrészjegyzék jóváhagyása',
        reload: options.reload,
        onSuccess: function(response) {
            showMessage('Alkatrészjegyzék jóvá lett hagyva');
        }
    });
}
...
function duplicateBom(part_id, options={}) {
    ...
    constructForm(`/api/part/${part_id}/bom-copy/`, {
        ...
        confirm: true,
        title: 'Alkatrészjegyzék másolása',
        onSuccess: function(response) {
            if (options.success) {
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        elements.push(`Nincs készlet`);
    }

    // Check for items on order
    if (part.ordering) {
        elements.push(`Rendelve: ${part.ordering}`);
    }

    // Check for items being built
    if (part.building) {
        elements.push(`Gyártásban: ${part.building}`);
    }
    ...
    if (units && !options.no_units) {
        output += `<span class='badge rounded-pill text-muted bg-muted ${classes}'>Me: ${units}</span> `;
    }
...
function makePartIcons(part) {
    ...
    if (part.trackable) {
        html += makeIconBadge('fa-directions', 'Követésre kötelezett alkatrész');
    }

    if (part.virtual) {
        html += makeIconBadge('fa-ghost', 'Virtuális alkatrész');
    }

    if (part.is_template) {
        html += makeIconBadge('fa-clone', 'Sablon alkatrész');
    }

    if (part.assembly) {
        html += makeIconBadge('fa-tools', 'Gyártmány alkatrész');
    }

    if (part.starred) {
        html += makeIconBadge('fa-bell icon-green', 'Értesítésre beállított alkatrész');
    }

    if (part.salable) {
        html += makeIconBadge('fa-dollar-sign', 'Értékesíthető alkatrész');
    }

    if (!part.active) {
        html += `<span class='badge badge-right rounded-pill bg-warning'>Inaktív</span> `;
    }
...
function generateStocktakeReport(options={}) {
    ...
    Új leltár riport ütemezése. Amint elkészül, az új leltár riport letölthető lesz.
    </div>
    `;
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Leltár riport létrehozása',
            preFormContent: content,
            fields: fields,
            onSuccess: function(response) {
                showMessage('Leltár riport beütemezve', {
                    style: 'success',
                });
            }
        }
...
function loadPartStocktakeTable(partId, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nincs elérhető leltár előzmény';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.allow_edit) {
                        html += makeEditButton('button-edit-stocktake', row.pk, 'Leltár bejegyzés szerkesztése');
                    }

                    if (options.allow_delete) {
                        html += makeDeleteButton('button-delete-stocktake', row.pk, 'Leltár bejegyzés törlése');
                    }
        ...
        onPostBody: function() {
            ...
            $(table).find('.button-edit-stocktake').click(function() {
                ...
                constructForm(`/api/part/stocktake/${pk}/`, {
                    ...
                    title: 'Leltár bejegyzés szerkesztése',
                    refreshTable: table,
                });
            });
            ...
            $(table).find('.button-delete-stocktake').click(function() {
                ...
                constructForm(`/api/part/stocktake/${pk}/`, {
                    method: 'DELETE',
                    title: 'Leltár bejegyzés törlése',
                    refreshTable: table,
                });
            });
        }
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.variant_stock > 0) {
                    text = `<em>${text}</em>`;
                    text += `<span title='Változatokkal együtt' class='fas fa-info-circle float-right icon-blue'></span>`;
                }
...
function createPartParameter(part_id, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Paraméter hozzáadása';
...
function editPartParameter(param_id, options={}) {
    options.fields = partParameterFields();
    options.title = 'Paraméter szerkesztése';
    options.focus = 'data';
...
function loadPartParameterTable(table, options) {
    ...
    $(table).inventreeTable({
        ...
        groupBy: false,
        formatNoMatches: function() {
            return 'Nem található paraméter';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-parameter-edit', pk, 'Paraméter szerkesztése');
                    html += makeDeleteButton('button-parameter-delete', pk, 'Paraméter törlése');
...
function loadPartParameterTemplateTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nincs alkatrész paraméter sablon';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    buttons += makeEditButton('template-edit', row.pk, 'Sablon szerkesztése');
                    buttons += makeDeleteButton('template-delete', row.pk, 'Sablon törlése');
    ...
    $(table).on('click', '.template-edit', function() {
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                ...
                title: 'Alkatrész paraméter sablon módosítása',
                refreshTable: table,
            }
        );
    });
    ...
    $(table).on('click', '.template-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
            Az összes erre a sablonra hivatkozó paraméter is törlésre kerül
        </div>`;
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                method: 'DELETE',
                preFormContent: html,
                title: 'Alkatrész paraméter sablon törlése',
                refreshTable: table,
            }
        );
    });
...
function loadPartPurchaseOrderTable(table, part_id, options={}) {
    ...
    $(table).inventreeTable({
        ...
        uniqueId: 'pk',
        formatNoMatches: function() {
            return 'Nem található beszerzési rendelés';
        },
        onPostBody: function() {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Csomagolási mennyiség: ${row.pack_quantity} - Teljes mennyiség: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let total = value * row.supplier_part_detail.pack_quantity_native;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Csomagolási mennyiség: ${row.pack_quantity} - Teljes mennyiség: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Beérkezett</span>`;
                    } else if (row.order_detail && row.order_detail.status == 20) {
                        let html = '';
                        var pk = row.pk;

                        html += makeIconButton('fa-sign-in-alt', 'button-line-receive', pk, 'Sortétel bevételezése');
...
function loadRelatedPartsTable(table, part_id, options={}) {
    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                let html = '';
                html += makeDeleteButton('button-related-delete', row.pk, 'Alkatrész kapcsolatok törlése');
        ...
        onPostBody: function() {
            $(table).find('.button-related-delete').click(function() {
                ...
                constructForm(`/api/part/related/${pk}/`, {
                    method: 'DELETE',
                    title: 'Alkatrész kapcsolatok törlése',
                    refreshTable: table,
                });
            });
        },
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>Nincs készlet</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>Nincs készlet</span>`;
    }

    rows += `<tr><td><b>Készlet</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>Rendelve</b></td><td>${part.ordering} ${units}</td></tr>`;
    }

    if (part.building) {
        rows += `<tr><td><b>Gyártásban</b></td><td>${part.building} ${units}</td></tr>`;
    }
...
function setPartCategory(data, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
        Kategória beállítása a kiválasztott alkatrészekhez
    </div>
    `;

    constructForm('/api/part/change_category/',{
        title: 'Alkatrész kategória beállítása',
        method: 'POST',
        preFormContent: html,
        fields: {
            category: {},
...
function loadPartTable(table, url, options={}) {
    ...
    if (!options.disableFilters) {
        ...
        setupFilterList('parts', $(table), options.filterTarget, {
            ...
            singular_name: 'alkatrész',
            plural_name: 'alkatrészek',
            custom_actions: [
                {
                    label: 'parts',
                    icon: 'fa-tools',
    ...
    $(table).inventreeTable({
        ...
        buttons: options.gridView ? [
            ...
            },
            {
                icon: 'fas fa-th',
                attributes: {
                    title: 'Megjelenítés rácsnézetként',
                    id: 'view-part-grid',
                },
                event: () => {
...
function loadPartCategoryTable(table, options) {
    ...
    table.inventreeTable({
        ...
        method: 'get',
        formatNoMatches: function() {
            return 'Nem találhatóak alkategóriák';
        },
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                        ...
                        if (row.subRequested) {
                            ...
                            html += `
                                <a href='#' pk='${row.pk}' class='load-sub-category'>
                                    <span class='fas fa-sync-alt' title='Alkategóriák betöltése'></span>
                                </a> `;
                        }
                    }
                    ...
                    if (row.starred) {
                        html += makeIconBadge('fa-bell icon-green', 'Értesítésre beállított kategória');
                    }
...
function loadPartTestTemplateTable(table, options) {
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Nincs a lekérdezéssel egyező teszt sablon';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part == part) {
                        ...
                        html += makeEditButton('button-test-edit', pk, 'Teszt eredmény szerkesztése');
                        html += makeDeleteButton('button-test-delete', pk, 'Teszt eredmény törlése');

                        return wrapButtons(html);
                    } else {
                        var text = 'Ez a teszt a szülő alkatrészhez lett felvéve';
        ...
        onPostBody: function() {
            ...
            table.find('.button-test-edit').click(function() {
                ...
                constructForm(url, {
                    ...
                    title: 'Teszt eredmény sablon szerkesztése',
                    onSuccess: function() {
                        table.bootstrapTable('refresh');
                    },
                });
            ...
            table.find('.button-test-delete').click(function() {
                ...
                constructForm(url, {
                    method: 'DELETE',
                    title: 'Teszt eredmény sablon törlése',
                    refreshTable: table,
                });
            });
        }
...
function loadPartSchedulingChart(canvas_id, part_id) {
        ...
        {},
        {
            ...
            success: function(response) {
                ...
                for (var idx = 0; idx < response.length; idx++) {
                    ...
                    if (date == null) {
                        date_string = '<em>Nincs megadva dátum</em>';
                        date_string += makeIconBadge('fa-exclamation-circle icon-red', 'Nincs megadva dátum');
                    } else if (date < today) {
                        date_string += makeIconBadge('fa-exclamation-circle icon-yellow', 'A megadott dátum a múltban van');
                    }

                    var quantity_string = entry.quantity + entry.speculative_quantity;

                    if (entry.speculative_quantity != 0) {
                        quantity_string += makeIconBadge('fa-question-circle icon-blue', 'Spekulatív');
                    }
    ...
    if (n_entries < 1) {
        ...
        var message = `
        <div class='alert alert-block alert-info'>
            Az alkatrészhez nem áll rendelkezésre ütemezési információ.
        </div>`;

        if (was_error) {
            message = `
                <div class='alert alert-block alert-danger'>
                    Hiba az alkatrész ütemezési információinak betöltésekor.
                </div>
            `;
        }
    ...
    var data = {
        datasets: [
            {
                label: 'Ütemezett készlet mennyiség',
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Mennyiség: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
