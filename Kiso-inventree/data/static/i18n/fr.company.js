    ...
    inventreeGet(`/api/company/part/${part}/`, {}, {
        success: function(data) {
            ...
            constructForm('/api/company/part/', {
                method: 'POST',
                fields: fields,
                title: 'Dupliquer la pièce du fournisseur',
                data: data,
...
function editSupplierPart(part, options={}) {
    ...
    constructForm(`/api/company/part/${part}/`, {
        fields: fields,
        title: options.title || 'Modifier la pièce du fournisseur',
        onSuccess: options.onSuccess
    });
}
...
function createSupplierPartPriceBreak(part_id, options={}) {
    ...
    constructForm('/api/company/price-break/', {
        fields: fields,
        method: 'POST',
        title: 'Ajouter un prix de rupture',
        onSuccess: function(response) {
            handleFormSuccess(response, options);
        }
    });
    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.is_customer) {
                    html += `<span title='Client' class='fas fa-user-tie float-right'></span>`;
                }

                if (row.is_manufacturer) {
                    html += `<span title='Fabricant' class='fas fa-industry float-right'></span>`;
                }

                if (row.is_supplier) {
                    html += `<span title='Fournisseur' class='fas fa-building float-right'></span>`;
                }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-manufacturer-part-edit', pk, 'Modifier la pièce du fabricant');
                    html += makeDeleteButton('button-manufacturer-part-delete', pk, 'Supprimer la pièce de fabricant');
...
function makeSupplierPartActions(options={}) {
    return [
        ...
        {
            label: 'delete',
            title: 'Supprimer les pièces du fournisseur',
            icon: 'fa-trash-alt icon-red',
            permission: 'purchase_order.delete',
            callback: function(data) {
...
function loadSupplierPartTable(table, url, options) {
    ...
    setupFilterList('supplierpart', $(table), '#filter-list-supplier-part', {
        custom_actions: [
            {
                label: 'supplier-part',
                title: 'Actions de la pièce du fournisseur',
                icon: 'fa-tools',
