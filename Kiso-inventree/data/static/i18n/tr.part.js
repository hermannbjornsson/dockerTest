    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            ...
            if (options.hideTotalStock) {
                elements.push(`Mevcut: ${part.unallocated_stock}`);
            } else {
                elements.push(`Mevcut: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
...
function makePartIcons(part) {
    ...
    if (part.is_template) {
        html += makeIconBadge('fa-clone', 'Şablon Parça');
    }
    ...
    if (!part.active) {
        html += `<span class='badge badge-right rounded-pill bg-warning'>Pasif</span> `;
    }
...
function loadPartParameterTemplateTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Parça parametre şablonu bulunamadı';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    buttons += makeEditButton('template-edit', row.pk, 'Şablonu Düzenle');
                    buttons += makeDeleteButton('template-delete', row.pk, 'Şablonu Sil');
    ...
    $(table).on('click', '.template-edit', function() {
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                ...
                title: 'Parça Parametre Şablonu Düzenle',
                refreshTable: table,
            }
        );
    });
    ...
    $(table).on('click', '.template-delete', function() {
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                method: 'DELETE',
                preFormContent: html,
                title: 'Parça Parametre Şablonu Sil',
                refreshTable: table,
            }
        );
    });
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>Stok Yok</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>Stok Yok</span>`;
    }

    rows += `<tr><td><b>Stok</b></td><td>${stock}</td></tr>`;
...
function loadPartTestTemplateTable(table, options) {
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Sorgu ile eşleşen test şablonu bulunamadı';
        },
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Miktar: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
