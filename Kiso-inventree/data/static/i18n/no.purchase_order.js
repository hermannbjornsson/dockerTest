...
function createPurchaseOrder(options={}) {
    ...
    constructForm('/api/order/po/', {
        ...
        title: options.title || 'Opprett Innkjøpsordre',
    });
}
        ...
        part: {
            ...
            onEdit: function(value, name, field, opts) {
                ...
                inventreeGet(`/api/company/part/${value}/`,
                    ...
                    if (pack_quantity != 1) {
                        var txt = `<span class='fas fa-info-circle icon-blue'></span> Pakkeantall: ${formatDecimal(pack_quantity)} ${units}`;
...
function orderParts(parts_list, options={}) {
    ...
    <table class='table table-striped table-condensed' id='order-parts-table'>
        <thead>
            <tr>
                <th>Del</th>
                <th style='min-width: 300px;'>Leverandørdel</th>
                <th style='min-width: 300px;'>Innkjøpsordre</th>
                <th style='min-width: 50px;'>Antall</th>
                <th><!-- Actions --></th>
            </tr>
        </thead>
        <tbody>
                ...
                var onSupplierPartChanged = function(value, name, field, opts) {
                    ...
                    if (value != null) {
                        inventreeGet(
                            ...
                            if (pack_quantity != 1) {
                                var txt = `<span class='fas fa-info-circle icon-blue'></span> Pakkeantall: ${pack_quantity} ${units}`;
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        var batch_input = constructField(
            `items_batch_code_${pk}`,
            {
                ...
                required: false,
                label: 'Batchkode',
                help_text: 'Angi batchkode for innkommende lagervarer',
                icon: 'fa-layer-group',
            },
            {
                hideLabels: true,
        ...
        var sn_input = constructField(
            `items_serial_numbers_${pk}`,
            {
                ...
                required: false,
                label: 'Serienummer',
                help_text: 'Angi serienummer for innkommende lagervarer',
                icon: 'fa-hashtag',
            },
            {
                hideLabels: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Pakkeantall: ${pack_quantity} - Totalt Antall: ${total}${units}'></span>`;
                    }
