        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>Pedido de produção tem saídas incompletas</strong><br>
                ...
                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>Estoque obrigatório não foi totalmente alocado</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>Quantidade de produção requerida não foi concluída</div>`;
                }
            }
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Número de Sério: ${output.serial}`;
    } else {
        output_html += `Quantidade: ${output.quantity}`;
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        text = `Número de Sério: ${row.serial}`;
                    } else {
                        text = `Quantidade: ${row.quantity}`;
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Peça</th>
                <th>Alocado</th>
                <th style='min-width: 250px;'>Item de estoque</th>
                <th>Quantidade</th>
                <th></th>
            </tr>
        </thead>
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `No pedido: ${formatDecimal(row.on_order)}`);
                    }
