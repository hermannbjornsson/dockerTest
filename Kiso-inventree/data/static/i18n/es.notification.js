        ...
        columns: [
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    if (enableDelete) {
                        bDel = `<button title='Eliminar notificación' class='notification-delete btn btn-outline-secondary' type='button' pk='${row.pk}'><span class='fas fa-trash-alt icon-red'></span></button>`;
                    }
...
function getReadEditButton(pk, state, small=false) {
    ...
    if (state) {
        bReadText = 'Marcar como no leído';
        bReadIcon = 'fas fa-bookmark icon-red';
        bReadTarget = 'unread';
    } else {
        bReadText = 'Marcar como leído';
        ...
        },
        {
            success: function(response) {
                if (response.length == 0) {
                    html = `<p class='text-muted'><em>No hay notificaciones sin leer</em><span class='fas fa-check-circle icon-green float-right'></span></p>`;
                } else {
                    // build up items
...
function closeNotificationPanel() {
    $('#notification-center').html(`<p class='text-muted'>Las notificaciones cargarán aquí</p>`);
}
