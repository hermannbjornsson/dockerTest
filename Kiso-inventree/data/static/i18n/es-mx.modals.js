...
function createNewModal(options={}) {
    ...
    if (!options.hideCloseButton) {
        buttons += `<button type='button' class='btn btn-secondary' id='modal-form-close' data-bs-dismiss='modal'>Cancelar</button>`;
    }
    ...
    modalSetTitle(modal_name, options.title || 'Título del Formulario');
    modalSetSubmitText(modal_name, options.submitText || 'Enviar');
    modalSetCloseText(modal_name, options.closeText || 'Cancelar');
...
function loadingMessageContent() {
    ...
    return `<span class='glyphicon glyphicon-refresh glyphicon-refresh-animate'></span> Esperando al servidor...`;
}
        ...
        <div class='panel'>
            <div class='panel panel-heading'>
                <div class='panel-title'>
                    <a data-bs-toggle='collapse' href="#collapse-error-info">Mostrar Información de Error</a>
                </div>
            </div>
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.title = title;
    options.submitText = options.accept_text || 'Aceptar';
    options.closeText = options.cancel_text || 'Cancelar';
...
function openModal(options) {
    ...
    if (options.title) {
        ...
        modalSetTitle(modal, 'Cargando Datos...');
    }
    ...
    modalSubmit(modal, function() {
        $(modal).find('.js-modal-form').ajaxSubmit({
                ...
                if ('form_valid' in response) {
                    ...
                    if (response.form_valid) {
                        ...
                        if (response.html_form) {
                            ...
                            $(modal).modal('hide');
                            showAlertDialog('Respuesta no válida del servidor', 'Datos del formulario faltantes de la respuesta del servidor');
                        }
                    }
                } else {
                    $(modal).modal('hide');
            ...
            error: function(xhr) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Error al publicar datos del formulario', renderErrorMessage(xhr));
            },
            complete: function() {
                // TODO
            }
    ...
    var ajax_data = {
        ...
        success: function(response) {
            ...
            if (response.html_form) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Respuesta del servidor inválida', 'Respuesta JSON faltan datos del formulario');
            }
        },
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    'Sin Respuesta',
                    'No hay respuesta del servidor InvenTree',
                );
            } else if (xhr.status == 400) {
                showAlertDialog(
                    'Error 400: Solicitud Incorrecta',
                    'El servidor devolvió el código de error 400',
                );
            } else if (xhr.status == 401) {
                showAlertDialog(
                    'Error 401: No autenticado',
                    'Credenciales de autenticación no suministradas',
                );
                ...
                showAlertDialog(
                    ...
                    'Tiempo de espera de conexión agotado al solicitar datos del servidor',
                );
            } else {
                showAlertDialog('Error al solicitar datos del formulario', renderErrorMessage(xhr));
            }
