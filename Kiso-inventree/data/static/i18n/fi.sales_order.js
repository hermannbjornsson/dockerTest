            ...
            if (!allocations || allocations.length == 0) {
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        stock = `Sarjanumero: ${allocation.serial}`;
                    } else {
                        stock = `Määrä: ${allocation.quantity}`;
                    }
...
function allocateStockToSalesOrder(order_id, line_items, options={}) {
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Osa</th>
                <th style='min-width: 250px;'>Varastotuote</th>
                <th>Määrä</th>
                <th></th>
        </thead>
        <tbody>
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    var text = `Varastotuote ${row.item}`;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `Määrä: ${row.quantity}`;

                    if (item && item.serial != null && row.quantity == 1) {
                        text = `Sarjanumero: ${item.serial}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        ...
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || 'Sijainti',
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        html += `<span class='badge bg-success badge-right'>Lähetetty</span>`;
                    } else {
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        text = `Sarjanumero: ${row.serial}`;
                    } else {
                        text = `Määrä: ${row.quantity}`;
                    }
