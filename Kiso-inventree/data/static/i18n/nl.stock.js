...
function editStockLocation(pk, options={}) {
    ...
    options.title = 'Bewerk Voorraadlocatie';
...
function deleteStockLocation(pk, options={}) {
    ...
    constructForm(url, {
        title: 'Verwijder Voorraadlocatie',
        method: 'DELETE',
        fields: {
            'delete_stock_items': {
...
function createNewStockItem(options={}) {
    ...
    options.title = 'Nieuw Voorraadartikel';
    options.method = 'POST';
    ...
    options.successMessage = 'Voorraaditem gemaakt';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                var details = `
                <br>Hoeveelheid: ${response.quantity}
                <br>Serienummers: ${response.serial_numbers}
                `;
...
function assignStockToCustomer(items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.serial && item.quantity == 1) {
            ...
            quantity = `Hoeveelheid: ${item.quantity}`;
        }
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        text = 'Verbruikt door productieorder';
        ...
        text = '<i>Geen voorraadlocatie ingesteld</i>';
        url = '';
    }
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Samengevoegde voorraadartikelen', {
                            style: 'success',
                        });
                    }
                });
    ...
    col = {
        ...
        formatter: function(value, row) {
            ...
            if (row.sales_order) {
                ...
                html += makeIconBadge('fa-truck', 'Voorraadartikel toegewezen aan verkooporder');
            } else if (row.customer) {
                // StockItem has been assigned to a customer
        ...
        formatter: function(details, row) {
            ...
            if (details.purchaseorder) {
                ...
                if (details.purchaseorder_detail) {
                    ...
                    html += `<i>Inkooporder bestaat niet meer</i>`;
                }
