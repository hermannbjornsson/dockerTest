...
function editSupplierPart(part, options={}) {
    ...
    constructForm(`/api/company/part/${part}/`, {
        fields: fields,
        title: options.title || 'Rediger Leverandørdel',
        onSuccess: options.onSuccess
    });
}
...
function deleteSupplierParts(parts, options={}) {
    ...
    Alle valgte leverandørdeler vil slettes
    </div>
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.is_manufacturer) {
                    html += `<span title='Produsent' class='fas fa-industry float-right'></span>`;
                }

                if (row.is_supplier) {
                    html += `<span title='Leverandør' class='fas fa-building float-right'></span>`;
                }
...
function loadManufacturerPartTable(table, url, options) {
    ...
    $(table).inventreeTable({
        ...
        groupBy: false,
        formatNoMatches: function() {
            return 'Ingen produsentdeler funnet';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-manufacturer-part-edit', pk, 'Endre produsentdel');
                    html += makeDeleteButton('button-manufacturer-part-delete', pk, 'Slett produsentdel');
...
function loadSupplierPartTable(table, url, options) {
    ...
    setupFilterList('supplierpart', $(table), '#filter-list-supplier-part', {
        custom_actions: [
            {
                label: 'supplier-part',
                title: 'Handlinger for leverandørdeler',
                icon: 'fa-tools',
    ...
    $(table).inventreeTable({
        ...
        sortable: true,
        formatNoMatches: function() {
            return 'Ingen leverandørdeler funnet';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-supplier-part-edit', pk, 'Rediger leverandørdel');
                    html += makeDeleteButton('button-supplier-part-delete', pk, 'Slett leverandørdel');
...
function loadSupplierPriceBreakTable(options={}) {
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Ingen informasjon om prisbrudd funnet';
        },
        onPostBody: function() {
            setupCallbacks();
        },
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    buttons += makeEditButton('button-price-break-edit', row.pk, 'Rediger prisbrudd');
                    buttons += makeDeleteButton('button-price-break-delete', row.pk, 'Slett prisbrudd');
