        ...
        columns: [
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    if (enableDelete) {
                        bDel = `<button title='Értesítés törlése' class='notification-delete btn btn-outline-secondary' type='button' pk='${row.pk}'><span class='fas fa-trash-alt icon-red'></span></button>`;
                    }
...
function getReadEditButton(pk, state, small=false) {
    ...
    if (state) {
        bReadText = 'Megjelölés olvasatlanként';
        bReadIcon = 'fas fa-bookmark icon-red';
        bReadTarget = 'unread';
    } else {
        bReadText = 'Megjelölés olvasottként';
        ...
        },
        {
            success: function(response) {
                if (response.length == 0) {
                    html = `<p class='text-muted'><em>Nincs olvasatlan értesítés</em><span class='fas fa-check-circle icon-green float-right'></span></p>`;
                } else {
                    // build up items
...
function closeNotificationPanel() {
    $('#notification-center').html(`<p class='text-muted'>Az értesítések itt fognak megjelenni</p>`);
}
