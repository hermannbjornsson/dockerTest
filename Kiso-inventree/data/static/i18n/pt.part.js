...
function partGroups() {
    ...
    return {
        ...
        initial_supplier: {
            title: 'Dados Iniciais de Fornecedor',
            collapsible: true,
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            ...
            if (options.hideTotalStock) {
                elements.push(`Disponível: ${part.unallocated_stock}`);
            } else {
                elements.push(`Disponível: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
        ...
        elements.push(`Sem Estoque`);
    }

    // Check for items on order
    if (part.ordering) {
        elements.push(`No pedido: ${part.ordering}`);
    }

    // Check for items being built
    if (part.building) {
        elements.push(`Produzindo: ${part.building}`);
    }
...
function makePartIcons(part) {
    ...
    if (!part.active) {
        html += `<span class='badge badge-right rounded-pill bg-warning'>Inativo</span> `;
    }
...
function generateStocktakeReport(options={}) {
    ...
    Programar geração de um novo relatório de balanço. Uma vez concluído, o relatório de estoque estará disponível para baixar.
    </div>
    `;
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Gerar Relatório de Balanço',
            preFormContent: content,
            fields: fields,
            onSuccess: function(response) {
                showMessage('Relatório de balanço agendado', {
                    style: 'success',
                });
            }
        }
...
function loadPartStocktakeTable(partId, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nenhuma informação de balanço disponível';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.allow_edit) {
                        html += makeEditButton('button-edit-stocktake', row.pk, 'Editar Lançamento de Balanço');
                    }

                    if (options.allow_delete) {
                        html += makeDeleteButton('button-delete-stocktake', row.pk, 'Apagar Lançamento de Balanço');
                    }
        ...
        onPostBody: function() {
            ...
            $(table).find('.button-edit-stocktake').click(function() {
                ...
                constructForm(`/api/part/stocktake/${pk}/`, {
                    ...
                    title: 'Editar Lançamento de Balanço',
                    refreshTable: table,
                });
            });
            ...
            $(table).find('.button-delete-stocktake').click(function() {
                ...
                constructForm(`/api/part/stocktake/${pk}/`, {
                    method: 'DELETE',
                    title: 'Apagar Lançamento de Balanço',
                    refreshTable: table,
                });
            });
        }
...
function createPartParameter(part_id, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Adicionar parâmetro';
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Quantidade de embalagens: ${row.pack_quantity} - Quantidade Total: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let total = value * row.supplier_part_detail.pack_quantity_native;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Quantidade de embalagens: ${row.pack_quantity} - Quantidade Total: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Recebido</span>`;
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>Sem Estoque</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>Sem Estoque</span>`;
    }

    rows += `<tr><td><b>Estoque</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>No pedido</b></td><td>${part.ordering} ${units}</td></tr>`;
    }

    if (part.building) {
        rows += `<tr><td><b>Produzindo</b></td><td>${part.building} ${units}</td></tr>`;
    }
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Quantidade: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
