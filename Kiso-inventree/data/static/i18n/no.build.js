...
function editBuildOrder(pk) {
    ...
    constructForm(`/api/build/${pk}/`, {
        fields: fields,
        reload: true,
        title: 'Rediger produksjonsordre',
    });
}
...
function newBuildOrder(options={}) {
    ...
    constructForm(`/api/build/`, {
        ...
        follow: true,
        method: 'POST',
        title: 'Opprett Produksjonsordre',
...
function cancelBuildOrder(build_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Kanseller Produksjonsordre',
            confirm: true,
            fields: {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Er du sikker du vil kansellere produksjonen?
                </div>`;

                if (opts.context.has_allocated_stock) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Lagervarer har blitt tildelt til denne Produksjonsordren
                    </div>`;
                }
                ...
                if (opts.context.incomplete_outputs) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Det er fortsatt ufullstendige artikler i denne produksjonsordren
                    </div>`;
                }
    ...
    constructForm(`/api/build/${build_id}/finish/`, {
        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                Produksjonsordren er klar til å fullføres'
                </div>`;
            } else {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>Produksjonsordren har uferdige artikler</strong><br>
                    Denne produksjonsordren kan ikke fullføres da det fortsatt er ufullstendige artikler
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    <strong>Produksjonsordren er ufullstendig</strong>
                    </div>
                    `;
                }

                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>Nøvendig lagerbeholdning er ikke fullt tildelt</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>Nødvendig produksjonsmengde er ikke nådd</div>`;
                }
            }
        ...
        reload: true,
        confirm: true,
        title: 'Fullføre Produksjonsordre',
        method: 'POST',
    });
}
        ...
        {},
        {
            success: function(build) {
                ...
                inventreeGet(`/api/part/${build.part}/serial-numbers/`, {}, {
                    success: function(data) {
                        if (data.next) {
                            fields.serial_numbers.placeholder = `Neste tilgjengelige serienummer: ${data.next}`;
                        } else if (data.latest) {
                            fields.serial_numbers.placeholder = `Siste serienummer: ${data.latest}`;
                        }
                    },
                    async: false,
                });
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Stykklisten inneholder sporbare deler.<br>
                        Produksjonsartikler må genereres individuelt.
                    </div>
                    `;
                }
                ...
                if (trackable) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Sporbare varer kan ha serienummer angitt<br>
                        Angi serienumre for å generere flere single produksjonsartikler
                    </div>
                    `;
                }
                ...
                constructForm(`/api/build/${build_id}/create-output/`, {
                    method: 'POST',
                    title: 'Opprett Produksjonsartikkel',
                    confirm: true,
                    fields: fields,
                    preFormContent: html,
                    onSuccess: function(response) {
...
function makeBuildOutputButtons(output_id, build_info, options={}) {
    ...
    if (options.has_tracked_lines) {
        ...
        html += makeIconButton(
            ...
            output_id,
            'Tildel lagervarer til denne produksjonsartikkelen',
        );
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        ...
        output_html += `Antall: ${output.quantity}`;
    ...
    buttons += makeRemoveButton('button-row-remove', pk, 'Fjern rad');
...
function completeBuildOutputs(build_id, outputs, options={}) {
    ...
    if (outputs.length == 0) {
        showAlertDialog(
            'Velg Produksjonsartikler',
            'Minst en produksjonsartikkel må velges',
        );
        return;
    }
    ...
    constructForm(`/api/build/${build_id}/complete/`, {
        ...
        confirm: true,
        title: 'Fullfør Produksjonsartikler',
...
function deleteBuildOutputs(build_id, outputs, options={}) {
    ...
    constructForm(`/api/build/${build_id}/delete-outputs/`, {
        ...
        fields: {},
        confirm: true,
        title: 'Slett Produksjonsartikler',
        afterRender: function(fields, opts) {
            // Setup callbacks to remove outputs
            $(opts.modal).find('.button-row-remove').click(function() {
                var pk = $(this).attr('pk');
...
function loadBuildOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Ingen tildelinger til produksjonsordre funnet';
        },
        columns: [
            {
                field: 'pk',
...
function loadBuildOutputTable(build_info, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Ingen aktive produksjonsartikler funnet';
        },
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.batch) {
                        text += ` <small>(Parti: ${row.batch})</small>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.location_detail) {
                        ...
                        return '<i>Ingen plassering satt</i>';
                    }
                }
            },
            {
                ...
                formatter: function(value, row) {
                    let buttons = '';
                    buttons += makeEditButton('button-allocation-edit', row.pk, 'Rediger lagertildeling');
                    buttons += makeDeleteButton('button-allocation-delete', row.pk, 'Slett lagertildeling');
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.consumable) {
                        icons += `<span class='fas fa-info-circle icon-blue float-right' title='Forbruksvare'></span>`;
                    } else {
                        ...
                        if (available <= 0) {
                            icons += `<span class='badge rounded-pill bg-danger'>Ingen lagerbeholdning tilgjengelig</span>`;
                        } else {
                            let extra = '';
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `I bestilling: ${formatDecimal(row.on_order)}`);
                    }
