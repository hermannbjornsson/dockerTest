...
function generateAvailableFilterList(tableKey) {
    ...
    html += `<option value=''>Velg filter</option>`;
...
function makeCustomActionGroup(action_group, table) {
    ...
    let title = action_group.title || 'Handlinger';
...
function makeBarcodeActions(barcode_actions, table) {
    ...
    let html = `
    <div class='btn-group' role='group'>
    <button id='barcode-actions' title='Strekkodehandlinger' class='btn btn-outline-secondary dropdown-toggle' type='button' data-bs-toggle='dropdown'>
...
function setupFilterList(tableKey, table, target, options={}) {
    ...
    if (report_button || labels_button) {
        ...
        if (labels_button) {
            print_buttons += `<li><a class='dropdown-item' href='#' id='print-labels-${tableKey}'><span class='fas fa-tag'></span> Skriv ut etiketter</a></li>`;
        }

        if (report_button) {
            print_buttons += `<li><a class='dropdown-item' href='#' id='print-report-${tableKey}'><span class='fas fa-file-pdf'></span> Skriv ut rapporter</a></li>`;
        }
    ...
    element.find(`#${add}`).click(function clicked() {
        ...
        if (!addClicked) {
            ...
            html += `<button title='Opprett filter' class='btn btn-outline-secondary filter-button' id='${make}'><span class='fas fa-plus'></span></button>`;
            html += `</div>`;
...
function getFilterOptionValue(tableKey, filterKey, valueKey) {
    ...
    if (filter.type == 'bool') {
        if (value == '1') return 'sant';
        if (value == '0') return 'usant';
