...
function createPurchaseOrder(options={}) {
    ...
    constructForm('/api/order/po/', {
        ...
        title: options.title || 'Создать заказ на закупку',
    });
}
...
function orderParts(parts_list, options={}) {
    ...
    <table class='table table-striped table-condensed' id='order-parts-table'>
        <thead>
            <tr>
                <th>Детали</th>
                <th style='min-width: 300px;'>Деталь поставщика</th>
                <th style='min-width: 300px;'>Заказ на закупку</th>
                <th style='min-width: 50px;'>Количество</th>
                <th><!-- Actions --></th>
            </tr>
        </thead>
        <tbody>
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        var batch_input = constructField(
            `items_batch_code_${pk}`,
            {
                ...
                required: false,
                label: 'Код партии',
                help_text: 'Введите код партии для поступающих единиц хранения',
                icon: 'fa-layer-group',
            },
            {
                hideLabels: true,
