    ...
    getApiEndpointOptions(url, function(OPTIONS) {
        ...
        case 'POST':
            if (canCreate(OPTIONS)) {
                ...
                showMessage('Aktion verboten', {
                    style: 'danger',
                    details: 'Erstellvorgang nicht erlaubt',
        ...
        case 'PATCH':
            if (canChange(OPTIONS)) {
                ...
                showMessage('Aktion verboten', {
                    style: 'danger',
                    details: 'Updatevorgang nicht erlaubt',
                    icon: 'fas fa-user-times',
                });
        ...
        case 'DELETE':
            if (canDelete(OPTIONS)) {
                ...
                showMessage('Aktion verboten', {
                    style: 'danger',
                    details: 'Löschvorgang nicht erlaubt',
                    icon: 'fas fa-user-times',
                });
        ...
        case 'GET':
            if (canView(OPTIONS)) {
                ...
                showMessage('Aktion verboten', {
                    style: 'danger',
                    details: 'Anzeigevorgang nicht erlaubt',
                    icon: 'fas fa-user-times',
                });
...
function insertPersistButton(options) {
    ...
    var message = options.persistMessage || 'Dieses Formular offen lassen';
        ...
        if (field) {
            ...
            case 'decimal':
                if (!validateFormField(name, options)) {
                    ...
                    data_errors[name] = ['Gib eine gültige Nummer ein'];
                }
                break;
            default:
                break;
...
function constructField(name, parameters, options={}) {
    ...
    if (extra) {
        ...
        if (!parameters.required && !options.hideClearButton) {
            html += `
            <span class='input-group-text form-clear' id='clear_${field_name}' title='Eingabe leeren'>
...
function selectImportFields(url, data={}, options={}) {
    ...
    var headers = `<tr><th>Dateispalte</th><th>Feldname</th></tr>`;
