...
function createManufacturerPart(options={}) {
    ...
    constructForm('/api/company/part/manufacturer/', {
        fields: fields,
        method: 'POST',
        title: 'Aggiungi Articolo Produttore',
...
function editManufacturerPart(part, options={}) {
    ...
    constructForm(url, {
        fields: fields,
        title: 'Modifica Articolo Produttore',
        onSuccess: options.onSuccess
    });
}
    ...
    inventreeGet(`/api/company/part/${part}/`, {}, {
        success: function(data) {
            ...
            constructForm('/api/company/part/', {
                method: 'POST',
                fields: fields,
                title: 'Duplica Articoli Fornitore',
                data: data,
...
function editSupplierPart(part, options={}) {
    ...
    constructForm(`/api/company/part/${part}/`, {
        fields: fields,
        title: options.title || 'Modifica fornitore articolo',
        onSuccess: options.onSuccess
    });
}
...
function deleteSupplierParts(parts, options={}) {
    ...
    Tutte gli articoli del fornitore selezionati saranno eliminati
    </div>
    ...
    constructForm('/api/company/part/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Cancella Articoli Fornitore',
        preFormContent: html,
        form_data: {
            items: ids,
        },
...
function createSupplierPartPriceBreak(part_id, options={}) {
    ...
    constructForm('/api/company/price-break/', {
        fields: fields,
        method: 'POST',
        title: 'Aggiungi riduzione prezzo',
        onSuccess: function(response) {
            handleFormSuccess(response, options);
        }
    });
...
function loadCompanyTable(table, url, options={}) {
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.is_manufacturer) {
                    html += `<span title='Produttore' class='fas fa-industry float-right'></span>`;
                }

                if (row.is_supplier) {
                    html += `<span title='Fornitore' class='fas fa-building float-right'></span>`;
                }
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nessuna informazione azienda trovata';
        },
        showColumns: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.allow_edit) {
                        html += makeEditButton('btn-contact-edit', pk, 'Modifica contatto');
                    }

                    if (options.allow_delete) {
                        html += makeDeleteButton('btn-contact-delete', pk, 'Elimina contatto');
                    }
...
function deleteManufacturerParts(selections, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Tutti gli articoli del produttore selezionati saranno eliminati
    </div>
    <table class='table table-striped table-condensed'>
    <tr>
        <th>Articolo</th>
    ...
    constructForm('/api/company/part/manufacturer/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Elimina Articoli Produttore',
        preFormContent: html,
        form_data: {
            items: ids,
        },
...
function deleteManufacturerPartParameters(selections, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Tutti i parametri selezionati saranno cancellati
    </div>
    <table class='table table-striped table-condensed'>
    <tr>
        <th>Nome</th>
...
function loadManufacturerPartTable(table, url, options) {
    ...
    $(table).inventreeTable({
        ...
        groupBy: false,
        formatNoMatches: function() {
            return 'Nessun articolo produttore trovato';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-manufacturer-part-edit', pk, 'Modifica articolo produttore');
                    html += makeDeleteButton('button-manufacturer-part-delete', pk, 'Cancella articolo produttore');
...
function loadSupplierPartTable(table, url, options) {
    ...
    setupFilterList('supplierpart', $(table), '#filter-list-supplier-part', {
        custom_actions: [
            {
                label: 'supplier-part',
                title: 'Azioni Articolo Fornitore',
                icon: 'fa-tools',
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-supplier-part-edit', pk, 'Modifica articolo fornitore');
                    html += makeDeleteButton('button-supplier-part-delete', pk, 'Elimina articolo fornitore');
...
function loadSupplierPriceBreakTable(options={}) {
    ...
    function setupCallbacks() {
        table.find('.button-price-break-delete').click(function() {
            ...
            constructForm(`/api/company/price-break/${pk}/`, {
                method: 'DELETE',
                title: 'Elimina riduzione di prezzo',
                refreshTable: table,
            });
        });
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nessuna informazione di riduzione di prezzo trovata';
        },
        onPostBody: function() {
            setupCallbacks();
        },
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    buttons += makeEditButton('button-price-break-edit', row.pk, 'Modifica riduzione di prezzo');
                    buttons += makeDeleteButton('button-price-break-delete', row.pk, 'Cancella riduzione di prezzo');
