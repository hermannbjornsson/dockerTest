...
function serializeStockItem(pk, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Lagerartikel serialisieren';
    ...
    if (options.part) {
        ...
        inventreeGet(`/api/part/${options.part}/serial-numbers/`, {}, {
            success: function(data) {
                if (data.next) {
                    options.fields.serial_numbers.placeholder = `Nächste verfügbare Seriennummer: ${data.next}`;
                } else if (data.latest) {
                    options.fields.serial_numbers.placeholder = `Letzte Seriennummer: ${data.latest}`;
                }
            },
            async: false,
        });
    ...
    options.confirmMessage = 'Lager-Serialisierung bestätigen';
...
function stockLocationFields(options={}) {
    var fields = {
        parent: {
            help_text: 'Übergeordneter Lagerort',
            required: false,
        },
        name: {},
        description: {},
        ...
        external: {},
        icon: {
            help_text: `Icon (optional) - alle verfügbaren Icons einsehbar auf <a href="https://fontawesome.com/v5/search?s=solid" target="_blank" rel="noopener noreferrer">Font Awesome</a>.`,
...
function editStockLocation(pk, options={}) {
    ...
    options.title = 'Lagerartikel-Ort bearbeiten';
...
function createStockLocation(options={}) {
    ...
    options.method = 'POST';
    options.fields = stockLocationFields(options);
    options.title = 'Neuer Lagerstandort';
...
function deleteStockLocation(pk, options={}) {
    ...
    Sind Sie sicher, dass Sie diesen Lagerort löschen wollen?
    </div>
    `;
    ...
    var subChoices = [
        {
            value: 0,
            display_name: 'Zum übergeordneten Lagerbestand verschieben',
        },
        {
            value: 1,
    ...
    constructForm(url, {
        title: 'Bestand-Lagerort löschen',
        method: 'DELETE',
        fields: {
            'delete_stock_items': {
                label: 'Aktion für Lagerartikel in diesem Lagerort',
                choices: subChoices,
                type: 'choice'
            },
            'delete_sub_locations': {
                label: 'Aktion für Unter-Lagerorte',
                choices: subChoices,
                type: 'choice'
            },
        },
...
function stockItemFields(options={}) {
    var fields = {
                    ...
                    if (data.trackable) {
                        ...
                        inventreeGet(`/api/part/${data.pk}/serial-numbers/`, {}, {
                            success: function(data) {
                                var placeholder = '';
                                if (data.next) {
                                    placeholder = `Nächste verfügbare Seriennummer: ${data.next}`;
                                } else if (data.latest) {
                                    placeholder = `Letzte Seriennummer: ${data.latest}`;
                                }
                        ...
                        setFormInputPlaceholder('serial_numbers', 'Dieser Teil kann nicht serialisiert werden', opts);
                    }
        ...
        quantity: {
            help_text: 'Ausgangsmenge für diesen Lagerartikel eingeben',
        },
        serial_numbers: {
            icon: 'fa-hashtag',
            type: 'string',
            label: 'Seriennummer',
            help_text: 'Seriennummern für neue Lagerartikel eingeben (oder leer lassen)',
            required: false,
        },
        serial: {
            icon: 'fa-hashtag',
...
function duplicateStockItem(pk, options) {
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            showAlertOrCache('Lagerartikel dupliziert', true, {style: 'success'});
    ...
    inventreeGet(`/api/stock/${pk}/`, {}, {
        success: function(data) {
            ...
            options.method = 'POST';
            options.title = 'Bestand duplizieren';
...
function deleteStockItem(pk, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Sind Sie sicher, dass Sie diesen Lagerartikel löschen wollen?
    </div>`;

    constructForm(url, {
        method: 'DELETE',
        title: 'Lagerartikel löschen',
        preFormContent: html,
        onSuccess: function(response) {
            handleFormSuccess(response, options);
        }
...
function editStockItem(pk, options={}) {
    ...
    options.title = 'Lagerartikel bearbeiten';
...
function createNewStockItem(options={}) {
    ...
    options.title = 'Neuer Lagerartikel';
    options.method = 'POST';
    ...
    options.successMessage = 'Lagerartikel erstellt';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                addCachedAlert('Neuer Lagerartikel erstellt', {
                    icon: 'fas fa-boxes',
                });
                ...
                var details = `
                <br>Anzahl: ${response.quantity}
                <br>Seriennummer: ${response.serial_numbers}
                `;

                showMessage('Mehrere Lagerartikel erstellt', {
                    icon: 'fas fa-boxes',
                    details: details,
                });
...
function findStockItemBySerialNumber(part_id) {
    ...
    constructFormBody({}, {
        title: 'Seriennummer finden',
        fields: {
            serial: {
                label: 'Seriennummer',
                help_text: 'Seriennummer eingeben',
...
function assignStockToCustomer(items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.serial && item.quantity == 1) {
            quantity = `Seriennummer: ${item.serial}`;
        } else {
            quantity = `Anzahl: ${item.quantity}`;
        }
    ...
    constructForm('/api/stock/assign/', {
        ...
        confirm: true,
        confirmMessage: 'Bestand Zuweisung bestätigen',
        title: 'Einem Kunden zuordnen',
...
function mergeStockItems(items, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    <h5>Achtung: Das Zusammenführen kann nicht rückgängig gemacht werden</h5>
    <strong>Einige Informationen gehen verloren, wenn Artikel zusammengeführt werden:</strong>
    <ul>
        <li>Lagerartikelverlauf wird für zusammengeführte Lagerartikel gelöscht</li>
        <li>Lieferantenteil-Informationen werden für zusammengeführte Artikel gelöscht</li>
    </ul>
    </div>
    `;
    ...
    constructForm('/api/stock/merge/', {
        ...
        confirm: true,
        confirmMessage: 'Zusammenführung der Artikel bestätigen',
...
function adjustStock(action, items, options={}) {
    ...
    case 'move':
        ...
        break;
    case 'count':
        formTitle = 'Bestand zählen';
        actionTitle = 'Anzahl';
        url = '/api/stock/count/';
        break;
    case 'take':
        formTitle = 'Bestand entfernen';
        actionTitle = 'Entfernen';
        url = '/api/stock/remove/';
        break;
    case 'add':
        formTitle = 'Bestand hinzufügen';
        actionTitle = 'Hinzufügen';
        url = '/api/stock/add/';
        break;
    case 'delete':
        formTitle = 'Bestand löschen';
        allowSerializedStock = true;
        break;
    default:
        break;
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantity += ` - <small>Losnummer: ${item.batch}</small>`;
        }
        ...
        if (actionTitle != null) {
            actionInput = constructField(
                `items_quantity_${pk}`,
                {
                    ...
                    max_value: maxValue,
                    value: value,
                    title: readonly ? 'Menge von serialisiertem Bestand kann nicht bearbeitet werden' : 'Bestandsanzahl angeben',
                    required: true,
                },
                {
                    hideLabels: true,
    ...
    constructForm(url, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Bestands-Anpassung bestätigen',
        title: formTitle,
        afterRender: function(fields, opts) {
            // Add button callbacks to remove rows
            $(opts.modal).find('.button-stock-item-remove').click(function() {
...
function passFailBadge(result) {
    ...
    if (result) {
        return `<span class='badge badge-right rounded-pill bg-success'>ERFOLGREICH</span>`;
    } else {
        return `<span class='badge badge-right rounded-pill bg-danger'>FEHLGESCHLAGEN</span>`;
    }
}

function noResultBadge() {
    return `<span class='badge badge-right rounded-pill bg-info'>KEIN ERGEBNIS</span>`;
}
...
function loadStockTestResultsTable(table, options) {
    ...
    function makeButtons(row, grouped) {
        ...
        if (row.requires_attachment == false && row.requires_value == false && !row.result) {
            ...
            html += makeIconButton('fa-check-circle icon-green', 'button-test-tick', row.test_name, 'Test bestanden');
        }

        html += makeIconButton('fa-plus icon-green', 'button-test-add', row.test_name, 'Testergebnis hinzufügen');

        if (!grouped && row.result != null) {
            var pk = row.pk;
            html += makeEditButton('button-test-edit', pk, 'Testergebnis bearbeiten');
            html += makeDeleteButton('button-test-delete', pk, 'Testergebnis löschen');
        }
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine Testergebnisse gefunden';
        },
        queryParams: filters,
        original: params,
        onPostBody: function() {
    ...
    $(table).on('click', '.button-test-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
        <strong>Testergebnis löschen:</strong> ${row.test_name || row.test || row.key}
        </div>`;
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        text = 'Durch Bauauftrag verbraucht';
        ...
        text = '<i>Kein Lagerort gesetzt</i>';
        url = '';
    }
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Lagerartikel zusammengeführt', {
                            style: 'success',
                        });
                    }
                });
...
function loadStockTable(table, options) {
    ...
    col = {
        field: 'IPN',
        title: 'IPN (Interne Produktnummer)',
        sortName: 'part__IPN',
        visible: options.params['part_detail'],
        switchable: options.params['part_detail'],
        formatter: function(value, row) {
    ...
    col = {
        ...
        formatter: function(value, row) {
            ...
            if (row.is_building) {
                html += makeIconBadge('fa-tools', 'Lagerartikel wird produziert');
            }

            if (row.sales_order) {
                // Stock item has been assigned to a sales order
                html += makeIconBadge('fa-truck', 'Lagerartikel wurde Auftrag zugewiesen');
            } else if (row.customer) {
                // StockItem has been assigned to a customer
                html += makeIconBadge('fa-user', 'Lagerartikel wurde Kunden zugewiesen');
            } else if (row.allocated) {
                if (row.serial != null && row.quantity == 1) {
                    html += makeIconBadge('fa-bookmark icon-yellow', 'Serialisierter Lagerartikel wurde zugewiesen');
                } else if (row.allocated >= row.quantity) {
                    html += makeIconBadge('fa-bookmark icon-yellow', 'Lagerartikel wurde vollständig zugewiesen');
                } else {
                    html += makeIconBadge('fa-bookmark', 'Lagerartikel wurde teilweise zugewiesen');
                }
            } else if (row.belongs_to) {
                html += makeIconBadge('fa-box', 'Lagerartikel in anderem Element verbaut');
            } else if (row.consumed_by) {
                html += makeIconBadge('fa-tools', 'Stock item has been consumed by a build order');
            }

            if (row.expired) {
                html += makeIconBadge('fa-calendar-times icon-red', 'Lagerartikel ist abgelaufen');
            } else if (row.stale) {
                html += makeIconBadge('fa-stopwatch', 'Lagerartikel läuft demnächst ab');
            }

            // Special stock status codes
            if (row.status == stockCodes.REJECTED.key) {
                html += makeIconBadge('fa-times-circle icon-red', 'Lagerartikel abgewiesen');
            } else if (row.status == stockCodes.LOST.key) {
                html += makeIconBadge('fa-question-circle', 'Lagerartikel verloren');
            } else if (row.status == stockCodes.DESTROYED.key) {
                html += makeIconBadge('fa-skull-crossbones', 'Lagerartikel zerstört');
            }

            if (row.quantity <= 0) {
                html += `<span class='badge badge-right rounded-pill bg-danger'>gelöscht</span>`;
            }
    ...
    col = {
        ...
        formatter: function(value, row) {
            ...
            if (row.supplier_part_detail) {
                ...
                text = `<i>Zuliefererteil nicht angegeben</i>`;
            }
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Keine zur Anfrage passenden Lagerartikel';
        },
        ...
        formatter: function(details, row) {
            ...
            if (details.location) {
                ...
                if (details.location_detail) {
                    ...
                    html += `<i>Standort nicht mehr vorhanden</i>`;
                }
            ...
            if (details.purchaseorder) {
                ...
                if (details.purchaseorder_detail) {
                    ...
                    html += `<i>Bestellung existiert nicht mehr</i>`;
                }
            ...
            if (details.customer) {
                ...
                if (details.customer_detail) {
                    ...
                    html += `<i>Kunde existiert nicht mehr</i>`;
                }
            ...
            if (details.stockitem) {
                ...
                if (details.stockitem_detail) {
                    ...
                    html += `<i>Lagerartikel existiert nicht mehr</i>`;
                }
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            if (value) {
                ...
                return `<i>Keine Benutzerinformation</i>`;
            }
        }
    });
...
function loadInstalledInTable(table, options) {
    ...
    */
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine installierten Elemente';
        },
        columns: [
            {
                field: 'part',
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeIconButton('fa-unlink', 'button-uninstall', pk, 'Lagerartikel entfernen');
        ...
        `/api/stock/${installed_item_id}/uninstall/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                if (installed_item_id == null) {
                    ...
                    Zu deinstallierende Lagerartikel auswählen
                    </div>`;
                }
...
function installStockItem(stock_item_id, part_id, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
        <strong>Einen weiteren Lagerartikel in dieses Teil installiert</strong><br>
        Lagerartikel können nur installiert werden wenn folgende Kriterien erfüllt werden:<br>
        <ul>
            <li>Der Lagerartikel ist auf ein Teil verknüpft das in der Stückliste für diesen Lagerartikel ist</li>
            <li>Dieser Lagerartikel ist aktuell vorhanden</li>
            <li>Der Lagerbestand ist nicht bereits in einem anderen Bestand installiert</li>
            <li>Der Lagerbestand wird entweder mit einem Batch-Code oder mit Seriennummer verfolgt</li>
        </ul>
    </div>`;
        ...
        `/api/stock/${stock_item_id}/install/`,
        {
            ...
            fields: {
                part: {
                    ...
                    required: 'true',
                    label: 'Teil',
                    help_text: 'Teil zur Installation auswählen',
                    model: 'part',
                    api_url: '/api/part/',
                    auto_fill: true,
                    filters: {
            ...
            confirm: true,
            title: 'Lagerartikel installieren',
            preFormContent: html,
            onSuccess: function(response) {
                if (options.onSuccess) {
