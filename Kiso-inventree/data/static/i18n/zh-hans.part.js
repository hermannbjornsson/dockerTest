...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            ...
            if (options.hideTotalStock) {
                elements.push(`空闲: ${part.unallocated_stock}`);
            } else {
                elements.push(`空闲: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
    ...
    if (part.ordering) {
        elements.push(`订购中: ${part.ordering}`);
    }
...
function makePartIcons(part) {
    ...
    if (part.trackable) {
        html += makeIconBadge('fa-directions', '可追溯商品');
    }

    if (part.virtual) {
        html += makeIconBadge('fa-ghost', '虚拟商品');
    }
    ...
    if (part.salable) {
        html += makeIconBadge('fa-dollar-sign', '可销售商品');
    }
        ...
        columns: [
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    buttons += makeEditButton('template-edit', row.pk, '编辑模板');
                    buttons += makeDeleteButton('template-delete', row.pk, '删除模板');
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `包装数量: ${row.pack_quantity} - 总数量: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let total = value * row.supplier_part_detail.pack_quantity_native;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='包装数量: ${row.pack_quantity} - 总数量: ${total}'></span>`;
                    }
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>无库存</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>无库存</span>`;
    }

    rows += `<tr><td><b>库存</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>订购中</b></td><td>${part.ordering} ${units}</td></tr>`;
    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part == part) {
                        ...
                        html += makeEditButton('button-test-edit', pk, '编辑测试结果');
                        html += makeDeleteButton('button-test-delete', pk, '删除测试结果');
        ...
        {},
        {
            ...
            success: function(response) {
                ...
                for (var idx = 0; idx < response.length; idx++) {
                    ...
                    if (date == null) {
                        date_string = '<em>无指定日期</em>';
                        date_string += makeIconBadge('fa-exclamation-circle icon-red', '无指定日期');
                    } else if (date < today) {
                        date_string += makeIconBadge('fa-exclamation-circle icon-yellow', '指定的日期已过');
                    }
