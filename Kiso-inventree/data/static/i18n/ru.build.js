...
function editBuildOrder(pk) {
    ...
    constructForm(`/api/build/${pk}/`, {
        fields: fields,
        reload: true,
        title: 'Редактировать заказ на сборку',
    });
}
        ...
        `/api/build/${build_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Вы уверены, что хотите отменить эту сборку?
                </div>`;
        ...
        {},
        {
            success: function(build) {
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Спецификация содержит отслеживаемые детали.<br>
                ...
                if (trackable) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Отслеживаемые детали могут иметь серийные номера<br>
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Серийный номер: ${output.serial}`;
    } else {
        output_html += `Количество: ${output.quantity}`;
    ...
    buttons += makeRemoveButton('button-row-remove', pk, 'Удалить строку');
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.batch) {
                        text += ` <small>(Партия: ${row.batch})</small>`;
                    }
