...
function makeBarcodeInput(placeholderText='', hintText='') {
    ...
    placeholderText = placeholderText || 'Vonalkód beolvasása ide a kódolvasó használatával';

    hintText = hintText || 'Add meg a vonalkódot';
    ...
    <div class='form-group'>
        <label class='control-label' for='barcode'>Vonalkód</label>
        <div class='controls'>
            <div class='input-group'>
                ...
                <button title='Vonalkód beolvasása webkamerával' id='barcode_scan_btn' type='button' class='btn btn-secondary' onclick='onBarcodeScanClicked()' style='display: none;'>
                    ${makeIcon('fa-camera')}
                </button>
            </div>
...
function makeNotesField(options={}) {
    ...
    var tooltip = options.tooltip || 'Megjegyzések a készlet áthelyezéshez';
    var placeholder = options.placeholder || 'Írd be a megjegyzéseket';

    return `
    <div class='form-group'>
        <label class='control-label' for='notes'>Megjegyzések</label>
        <div class='controls'>
            <div class='input-group'>
                <span class='input-group-text'>
            ...
            error: function(xhr) {
                ...
                case 400:
                    ...
                    if (options.onError400) {
                        ...
                        showBarcodeMessage(modal, data.error || 'Kiszolgálóhiba');
                    }
                    break;
                default:
...
function showInvalidResponseError(modal, response, status) {
    showBarcodeMessage(
        modal,
        `Érvénytelen válasz a szervertől<br>Állapot: '${status}'`
    );
}
...
function barcodeDialog(title, options={}) {
    ...
    var details = options.details || 'Vonalkód beolvasása';
...
function barcodeScanDialog(options={}) {
    ...
    let title = options.title || 'Vonalkód beolvasása';
...
function unlinkBarcode(data, options={}) {
    ...
    var html = `<b>Vonalkód leválasztása</b><br>`;

    html += 'Ez törli a vonalkód hozzárendelést';

    showQuestionDialog(
        'Vonalkód leválasztása',
        html,
        {
            accept_text: 'Leválasztás',
            accept: function() {
                inventreePut(
                    '/api/barcode/unlink/',
                    data,
...
function barcodeCheckInStockItems(location_id, options={}) {
    ...
    function reloadTable() {
        ...
        items.forEach(function(item) {
            ...
            <tr pk='${item.pk}'>
                ...
                <td>${location_info}</td>
                <td>${item.quantity}</td>
                <td>${makeRemoveButton('button-item-remove', item.pk, 'Készlet tétel törlése')}</td>
            </tr>`;
        });
    ...
    barcodeDialog(
        'Készlet bevételezése adott helyre',
        {
            details: 'Készlet tétel vonalkód beolvasása, amit bevételezzünk erre a helyre',
            headerContent: table,
            preShow: function() {
                modalSetSubmitText(modal, 'Bevételezés');
                modalEnable(modal, false);
                reloadTable();
            },
            onShow: function() {
            ...
            onSubmit: function() {
                ...
                if (entries.length == 0) {
                    showBarcodeMessage(modal, 'Nincs vonalkód beolvasva', 'warning');
                    return;
                }
            ...
            onScan: function(response) {
                if ('stockitem' in response) {
                    ...
                    inventreeGet(
                        ...
                        {},
                        {
                            success: function(stockitem) {
                                ...
                                if (duplicate) {
                                    showBarcodeMessage(modal, 'Készlet tétel már beolvasva', 'warning');
                                } else {

                                    if (stockitem.location == location_id) {
                                        showBarcodeMessage(modal, 'Készlet tétel már ezen a helyen van');
                                        return;
                                    }
                                    ...
                                    showBarcodeMessage(modal, 'Hozzáadott készlet tétel', 'success');
                    ...
                    showBarcodeMessage(modal, 'Vonalkód nem egyezik egy ismert készlet tétellel sem', 'warning');
                }
            },
        }
    );
...
function barcodeCheckInStockLocations(location_id, options={}) {
    ...
    barcodeDialog(
        'Készlet tároló bevételezése adott helyre',
        {
            details: 'Készlet tároló vonalkód beolvasása, amit bevételezzünk erre a helyre',
            ...
            onScan: function(response) {
                if ('stocklocation' in response) {
                    ...
                    showBarcodeMessage(modal, 'A vonalkód nem egyezik egy ismert hellyel sem', 'warning');
                }
            }
        }
    );
...
function scanItemsIntoLocation(item_list, options={}) {
    ...
    barcodeDialog(
        'Készlet áthelyezése a leolvasott helyre',
        {
            headerContent: header,
            extraFields: extra,
            modal: modal,
                ...
                if ('stocklocation' in response) {
                    ...
                    inventreeGet(`/api/stock/location/${pk}/`, {}, {
                        ...
                        error: function() {
                            ...
                            showBarcodeMessage(
                                modal,
                                'A vonalkód nem egyezik egy ismert hellyel sem',
                                'warning',
                            );
                        }
                    });
