...
function generateAvailableFilterList(tableKey) {
    ...
    html += `<option value=''>Valitse suodatin</option>`;
...
function makeCustomActionGroup(action_group, table) {
    ...
    let title = action_group.title || 'Toiminnot';
...
function setupFilterList(tableKey, table, target, options={}) {
    ...
    if (report_button || labels_button) {
        ...
        if (report_button) {
            print_buttons += `<li><a class='dropdown-item' href='#' id='print-report-${tableKey}'><span class='fas fa-file-pdf'></span> Tulosta raportteja</a></li>`;
        }
    ...
    element.find(`#${add}`).click(function clicked() {
        ...
        if (!addClicked) {
            ...
            html += `<button title='Luo suodatin' class='btn btn-outline-secondary filter-button' id='${make}'><span class='fas fa-plus'></span></button>`;
            html += `</div>`;
...
function getFilterOptionValue(tableKey, filterKey, valueKey) {
    ...
    if (filter.type == 'bool') {
        if (value == '1') return 'tosi';
        if (value == '0') return 'epätosi';
