        ...
        `/api/build/${build_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    ¿Estás seguro de que quieres cancelar esta construcción?
                </div>`;
    ...
    constructForm(`/api/build/${build_id}/finish/`, {
        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                El pedido de construcción está listo para ser completado'
                </div>`;
            } else {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>El orden de construcción tiene salidas incompletas</strong><br>
                    This build order cannot be completed as there are incomplete outputs
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    <strong>Orden de construcción incompleta</strong>
                    </div>
                    `;
                }

                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>El stock requerido no ha sido completamente asignado</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>La cantidad de construcción requerida aún no se ha completado</div>`;
                }
            }
        ...
        reload: true,
        confirm: true,
        title: 'Completar Orden de Construcción',
        method: 'POST',
    });
}
        ...
        {},
        {
            success: function(build) {
                ...
                inventreeGet(`/api/part/${build.part}/serial-numbers/`, {}, {
                    success: function(data) {
                        if (data.next) {
                            fields.serial_numbers.placeholder = `Siguiente número de serie disponible: ${data.next}`;
                        } else if (data.latest) {
                            fields.serial_numbers.placeholder = `Último número de serie: ${data.latest}`;
                        }
                    },
                    async: false,
                });
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        La ley de materiales contiene partes rastreables.<br>
                        Las salidas de construcción deben ser generadas individualmente.
                    </div>
                    `;
                }
                ...
                if (trackable) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Las partes rastreables pueden tener números de serie especificados<br>
                        Introduzca números de serie para generar múltiples salidas de construcción única
                    </div>
                    `;
                }
...
function makeBuildOutputButtons(output_id, build_info, options={}) {
    ...
    if (options.has_tracked_lines) {
        ...
        html += makeIconButton(
            ...
            output_id,
            'Asignar artículos de stock a esta salida de trabajo',
        );
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Número de serie: ${output.serial}`;
    } else {
        output_html += `Cantidad: ${output.quantity}`;
...
function completeBuildOutputs(build_id, outputs, options={}) {
    ...
    if (outputs.length == 0) {
        showAlertDialog(
            'Seleccionar Salida de Trabajo',
            'Se debe seleccionar al menos una salida de trabajo',
        );
        return;
    }
    ...
    constructForm(`/api/build/${build_id}/complete/`, {
        ...
        confirm: true,
        title: 'Completar salidas de trabajo',
...
function loadBuildOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'No se encontraron asignaciones de órdenes de trabajo';
        },
        columns: [
            {
                field: 'pk',
...
function loadBuildOutputTable(build_info, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'No se encontraron salidas de trabajo activas';
        },
        ...
        columns: [
            ...
            {
                field: 'quantity',
                title: 'Resultado de la construcción o armado',
                switchable: false,
                sortable: true,
                formatter: function(value, row) {
                    let text = '';
                    ...
                    if (row.batch) {
                        text += ` <small>(Lote: ${row.batch})</small>`;
                    }
...
function allocateStockToBuild(build_id, line_items, options={}) {
    ...
    constructForm(`/api/build/${build_id}/allocate/`, {
        ...
        fields: {},
        preFormContent: html,
        title: 'Asignar Artículos de Stock a Orden de Trabajo',
...
function autoAllocateStockToBuild(build_id, bom_items=[], options={}) {
    ...
    constructForm(`/api/build/${build_id}/auto-allocate/`, {
        method: 'POST',
        fields: fields,
        title: 'Asignar artículos de inventario',
        confirm: true,
        preFormContent: html,
        onSuccess: function(response) {
            if (options.onSuccess) {
...
function loadBuildTable(table, options) {
    ...
    $(table).inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'No hay trabajos que coincidan con la consulta';
        },
        url: '/api/build/',
        queryParams: filters,
        groupBy: false,
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Orden de trabajo atrasada');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.location_detail) {
                        ...
                        return '<i>Ubicación no establecida</i>';
                    }
                }
            },
            {
                ...
                formatter: function(value, row) {
                    let buttons = '';
                    buttons += makeEditButton('button-allocation-edit', row.pk, 'Editar asignación de stock');
                    buttons += makeDeleteButton('button-allocation-delete', row.pk, 'Eliminar asignación de stock');
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.allow_variants) {
                        html += makeIconBadge('fa-sitemap', 'Stock de variante permitido');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `En pedido: ${formatDecimal(row.on_order)}`);
                    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.allocated < row.quantity) {
                        ...
                        if (row.part_detail.assembly) {
                            buttons += makeIconButton('fa-tools icon-blue', 'button-build', pk, 'Stock de Trabajo');
                        }

                        // Add a button to "purchase" stock for this line
                        if (row.part_detail.purchaseable) {
                            buttons += makeIconButton('fa-shopping-cart icon-blue', 'button-buy', pk, 'Pedido de stock');
                        }

                        // Add a button to "allocate" stock for this line
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-allocate', pk, 'Asignar stock');
                    }
