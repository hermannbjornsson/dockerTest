...
function validateBom(part_id, options={}) {
    ...
    Validatie van de BOM markeert ieder artikel als geldig
    </div>
    `;
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            ...
            if (options.hideTotalStock) {
                elements.push(`Beschikbaar: ${part.unallocated_stock}`);
            } else {
                elements.push(`Beschikbaar: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
    ...
    if (part.ordering) {
        elements.push(`In bestelling: ${part.ordering}`);
    }
...
function createPartParameter(part_id, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Parameter toevoegen';
...
function editPartParameter(param_id, options={}) {
    options.fields = partParameterFields();
    options.title = 'Parameter bewerken';
    options.focus = 'data';
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Ontvangen</span>`;
                    } else if (row.order_detail && row.order_detail.status == 20) {
                        let html = '';
                        var pk = row.pk;

                        html += makeIconButton('fa-sign-in-alt', 'button-line-receive', pk, 'Artikel ontvangen');
...
function partGridTile(part) {
    ...
    rows += `<tr><td><b>Voorraad</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>In bestelling</b></td><td>${part.ordering} ${units}</td></tr>`;
    }
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Hoeveelheid: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
