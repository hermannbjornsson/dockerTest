...
function constructOrderTableButtons(options={}) {
    ...
    if (!options.disableCalendarView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_calendar}' title='Kalender-Ansicht'><span class='fas fa-calendar-alt'></span></button>`,
            event: function() {
    ...
    if (!options.disableListView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_list}' title='Listen-Ansicht'><span class='fas fa-th-list'></span></button>`,
            event: function() {
                buttonCallback('list');
            }
        });
    ...
    if (!options.disableTreeView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_tree}' title='Baumansicht zeigen'><span class='fas fa-sitemap'></span></button>`,
            event: function() {
                buttonCallback('tree');
            }
        });
...
function constructExpandCollapseButtons(table, idx=0) {
    ...
    return [
        {
            html: `<button type='button' name='${idx++}' class='btn btn-outline-secondary' title='Alle Zeilen erweitern'><span class='fas fa-expand'></span></button>`,
            event: function() {
                $(table).bootstrapTable('expandAllRows');
            }
        },
        {
            html: `<button type='button' name='${idx++}' class='btn btn-outline-secondary' title='Alle Zeilen einklappen'><span class='fas fa-compress'></span></button>`,
            event: function() {
...
function downloadTableData(table, opts={}) {
    ...
    constructFormBody({}, {
        title: opts.title || 'Tabellendaten exportieren',
        fields: {
            format: {
                label: 'Format',
                help_text: 'Dateiformat wählen',
                required: true,
                type: 'choice',
                value: 'csv',
    ...
    $.fn.bootstrapTable.locales['en-US-custom'] = {
        ...
        formatShowingRows: function(pageFrom, pageTo, totalRows) {
            ...
            if (totalRows === undefined || isNaN(totalRows)) {
                return 'Alle Zeilen anzeigen';
            } else {
                return `zeige ${pageFrom} bis ${pageTo} von ${totalRows} Zeilen`;
            }
        },
        ...
        formatPaginationSwitch: function() {
            return 'Zeige/Verstecke Pagination';
        },
