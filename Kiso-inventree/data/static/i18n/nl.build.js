        ...
        `/api/build/${build_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Weet je zeker dat je de productie wilt annuleren?
                </div>`;

                if (opts.context.has_allocated_stock) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Voorraadartikelen zijn toegewezen aan deze productieorder
                    </div>`;
                }
                ...
                if (opts.context.incomplete_outputs) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Er staat incomplete productie open voor deze productieorder
                    </div>`;
                }
    ...
    constructForm(`/api/build/${build_id}/finish/`, {
        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                Productieorder is gereed om als voltooid te markeren'
                </div>`;
            } else {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>Productieorder heeft onvolledige uitvoeren</strong><br>
                    This build order cannot be completed as there are incomplete outputs
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    <strong>Productieorder is onvolledig</strong>
                    </div>
                    `;
                }

                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>Vereiste voorraad is niet volledig toegewezen</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>Vereiste productiehoeveelheid is voltooid</div>`;
                }
            }
        ...
        {},
        {
            success: function(build) {
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        De stuklijst bevat traceerbare onderdelen.<br>
                        Productieuitvoeren moeten individueel worden gegenereerd.
                    </div>
                    `;
                }
                ...
                if (trackable) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Traceerbare onderdelen kunnen een serienummer hebben<br>
                        Voer serienummers in om meerdere enkelvoudige productuitvoeren te genereren
                    </div>
                    `;
                }
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Serienummer: ${output.serial}`;
    } else {
        output_html += `Hoeveelheid: ${output.quantity}`;
...
function completeBuildOutputs(build_id, outputs, options={}) {
    ...
    constructForm(`/api/build/${build_id}/complete/`, {
        ...
        confirm: true,
        title: 'Voltooi Productieuitvoeren',
...
function deleteBuildOutputs(build_id, outputs, options={}) {
    ...
    constructForm(`/api/build/${build_id}/delete-outputs/`, {
        ...
        fields: {},
        confirm: true,
        title: 'Verwijder Productieuitvoeren',
        afterRender: function(fields, opts) {
            // Setup callbacks to remove outputs
            $(opts.modal).find('.button-row-remove').click(function() {
                var pk = $(this).attr('pk');
...
function loadBuildOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Geen productieordertoewijzingen gevonden';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (!value) {
                        return 'Locatie is niet opgegeven';
                    }
...
function loadBuildOutputTable(build_info, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Geen actieve productieuitvoeren gevonden';
        },
...
function allocateStockToBuild(build_id, line_items, options={}) {
    ...
    constructForm(`/api/build/${build_id}/allocate/`, {
        ...
        fields: {},
        preFormContent: html,
        title: 'Voorraadartikelen toewijzen aan Productieorder',
...
function autoAllocateStockToBuild(build_id, bom_items=[], options={}) {
    ...
    Voorraadartikelen zullen automatisch worden toegewezen aan de productieorder volgens de aangegeven richtlijnen:
    <ul>
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Productieorder is achterstallig');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.location_detail) {
                        ...
                        return '<i>Geen locatie ingesteld</i>';
                    }
                }
            },
            {
                ...
                formatter: function(value, row) {
                    let buttons = '';
                    buttons += makeEditButton('button-allocation-edit', row.pk, 'Voorraadtoewijzing bewerken');
                    buttons += makeDeleteButton('button-allocation-delete', row.pk, 'Voorraadtoewijzing verwijderen');
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.consumable) {
                        ...
                        if (available < (row.quantity - row.allocated)) {
                            icons += makeIconBadge('fa-times-circle icon-red', 'Onvoldoende voorraad beschikbaar');
                        } else {
                            icons += makeIconBadge('fa-check-circle icon-green', 'Genoeg voorraad beschikbaar');
                        }

                        if (available <= 0) {
                            icons += `<span class='badge rounded-pill bg-danger'>Geen Voorraad Aanwezig</span>`;
                        } else {
                            let extra = '';
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `In bestelling: ${formatDecimal(row.on_order)}`);
                    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.allocated < row.quantity) {
                        ...
                        if (row.part_detail.assembly) {
                            buttons += makeIconButton('fa-tools icon-blue', 'button-build', pk, 'Productie voorraad');
                        }

                        // Add a button to "purchase" stock for this line
                        if (row.part_detail.purchaseable) {
                            buttons += makeIconButton('fa-shopping-cart icon-blue', 'button-buy', pk, 'Voorraad order');
                        }

                        // Add a button to "allocate" stock for this line
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-allocate', pk, 'Voorraad toewijzen');
                    }
