...
function createNewStockItem(options={}) {
    ...
    options.title = 'Novo item de estoque';
    options.method = 'POST';
    ...
    options.successMessage = 'Item de estoque criado';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                var details = `
                <br>Quantidade: ${response.quantity}
                <br>Números de Série: ${response.serial_numbers}
                `;
...
function assignStockToCustomer(items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.serial && item.quantity == 1) {
            quantity = `Série: ${item.serial}`;
        } else {
            quantity = `Quantidade: ${item.quantity}`;
        }
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        text = 'Usado na ordem de produção';
...
function makeStockActions(table) {
    let actions = [
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Itens de estoque mesclados', {
                            style: 'success',
                        });
                    }
                });
        ...
        },
        {
            label: 'assign',
            icon: 'fa-user-tie',
            title: 'Disponibilizar para o cliente',
            permission: 'stock.change',
            callback: function(data) {
    ...
    cols.push({
        ...
        formatter: function(details, row) {
            ...
            if (details.buildorder) {
                html += `<tr><th>Ondem de Produção</th>`;
                html += `<td>`;
            ...
            if (details.purchaseorder) {
                html += `<tr><th>Pedido de Compra</th>`;
                html += '<td>';
