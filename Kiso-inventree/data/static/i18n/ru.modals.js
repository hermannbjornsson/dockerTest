...
function createNewModal(options={}) {
    ...
    if (!options.hideCloseButton) {
        buttons += `<button type='button' class='btn btn-secondary' id='modal-form-close' data-bs-dismiss='modal'>Отменить</button>`;
    }
    ...
    modalSetCloseText(modal_name, options.closeText || 'Отменить');
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.closeText = options.cancel_text || 'Отменить';
    ...
    modalSubmit(modal, function() {
        $(modal).find('.js-modal-form').ajaxSubmit({
            ...
            error: function(xhr) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Ошибка отправки данных формы', renderErrorMessage(xhr));
            },
            complete: function() {
                // TODO
            }
    ...
    var ajax_data = {
        ...
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    'Ошибка 401: Авторизация не выполнена',
                ...
                showAlertDialog('Ошибка запроса данных формы', renderErrorMessage(xhr));
            }
