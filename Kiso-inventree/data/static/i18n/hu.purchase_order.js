...
function purchaseOrderFields(options={}) {
    ...
    if (options.duplicate_order) {
        fields.duplicate_order = {
            ...
            label: 'Beszerzési rendelés',
            help_text: 'Válaszd ki a lemásolandó beszerzési rendelést',
        };
        ...
        fields.duplicate_line_items = {
            ...
            group: 'duplicate',
            type: 'boolean',
            label: 'Sortételek másolása',
            help_text: 'Összes sortétel másolása a kiválasztott rendelésből',
        };
        ...
        fields.duplicate_extra_lines = {
            ...
            group: 'duplicate',
            type: 'boolean',
            label: 'Egyéb tételek másolása',
            help_text: 'Összes egyéb tétel másolása a kiválasztott rendelésből',
        };
    }
...
function editPurchaseOrder(pk, options={}) {
    ...
    constructForm(`/api/order/po/${pk}/`, {
        fields: fields,
        title: 'Beszerzési rendelés szerkesztése',
...
function createPurchaseOrder(options={}) {
    ...
    constructForm('/api/order/po/', {
        ...
        title: options.title || 'Beszerzési rendelés létrehozása',
    });
}
        ...
        part: {
            ...
            onEdit: function(value, name, field, opts) {
                ...
                inventreeGet(`/api/company/part/${value}/`,
                    ...
                    if (pack_quantity != 1) {
                        var txt = `<span class='fas fa-info-circle icon-blue'></span> Csomagolási mennyiség: ${formatDecimal(pack_quantity)} ${units}`;
            ...
            secondary: {
                method: 'POST',
                title: 'Beszállítói alkatrész hozzáadása',
...
function completePurchaseOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Beszerzési rendelés befejezése',
            confirm: true,
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Rendelés befejezettnek jelölése?
                </div>`;

                if (opts.context.is_complete) {
                    html += `
                    <div class='alert alert-block alert-success'>
                        Minden sortétel megérkezett
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Ez a rendelés olyan sortételeket tartalmaz amik még nem érkeztek be.</br>
                        A rendelés befejezésével jelölésével annak adatai és sortételei a továbbiakban már nem lesznek szerkeszthetők.
                    </div>`;
                }
...
function cancelPurchaseOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Beszerzési rendelés törlése',
            confirm: true,
            preFormContent: function(opts) {
                var html = `
                <div class='alert alert-info alert-block'>
                    Biztosan törölni szeretnéd ezt a beszerzési rendelést?
                </div>`;

                if (!opts.context.can_cancel) {
                    html += `
                    <div class='alert alert-danger alert-block'>
                        Ezt a beszerzési rendelést nem lehet törölni
                    </div>`;
                }
...
function issuePurchaseOrder(order_id, options={}) {
    ...
    let html = `
    <div class='alert alert-block alert-warning'>
    A kiküldés után a sortételek már nem lesznek szerkeszthetők.
    </div>`;

    constructForm(`/api/order/po/${order_id}/issue/`, {
        method: 'POST',
        title: 'Beszerzési rendelés kiküldése',
        confirm: true,
        preFormContent: html,
        onSuccess: function(response) {
            handleFormSuccess(response, options);
...
function orderParts(parts_list, options={}) {
    ...
    if (parts.length == 0) {
        showAlertDialog(
            ...
            'Legalább egy beszerezhető alkatrészt ki kell választani',
        );
        return;
    }
    ...
    function renderPart(part, opts={}) {
        ...
        var supplier_part_prefix = `
            <button type='button' class='input-group-text button-row-new-sp' pk='${pk}' title='Új beszállítói alkatrész'>
        ...
        var purchase_order_prefix = `
            <button type='button' class='input-group-text button-row-new-po' pk='${pk}' title='Új beszerzési rendelés'>
                <span class='fas fa-plus-circle icon-green'></span>
            </button>
        `;
        ...
        buttons += makeIconButton(
            ...
            'button-row-add',
            pk,
            'Hozzáadás beszerzési rendeléshez',
        );
    ...
    <table class='table table-striped table-condensed' id='order-parts-table'>
        <thead>
            <tr>
                <th>Alkatrész</th>
                <th style='min-width: 300px;'>Beszállítói alkatrész</th>
                <th style='min-width: 300px;'>Beszerzési rendelés</th>
                <th style='min-width: 50px;'>Mennyiség</th>
                <th><!-- Actions --></th>
            </tr>
        </thead>
        <tbody>
            ...
            parts.forEach(function(part) {
                ...
                var onSupplierPartChanged = function(value, name, field, opts) {
                    ...
                    if (value != null) {
                        inventreeGet(
                            ...
                            if (pack_quantity != 1) {
                                var txt = `<span class='fas fa-info-circle icon-blue'></span> Csomagolási mennyiség: ${pack_quantity} ${units}`;
                ...
                var supplier_part_field = {
                    ...
                    noResults: function(query) {
                        return 'Nincsenek egyező beszállítói alkatrészek';
                    }
                };
                ...
                initializeRelatedField({
                    ...
                    noResults: function(query) {
                        return 'Nincsenek egyező beszerzési rendelések';
                    }
                }, null, opts);
...
function receivePurchaseOrderItems(order_id, line_items, options={}) {
    ...
    if (line_items.length == 0) {
        ...
        showAlertDialog(
            'Sortételek kiválasztása',
            'Legalább egy sortételt ki kell választani',
        );
        return;
    }
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        if (native_pack_quantity != 1) {
            ...
            <div class='alert alert-small alert-block alert-info'>
                Csomagolási mennyiség: ${pack_quantity}<br>
                Beérkezett mennyiség: <span class='pack_received_quantity' id='items_received_quantity_${pk}'>${received}</span> ${units}
            </div>`;
        }
        ...
        var batch_input = constructField(
            `items_batch_code_${pk}`,
            {
                ...
                required: false,
                label: 'Batch kód',
                help_text: 'Írd be a batch kódját a beérkezett tételeknek',
                icon: 'fa-layer-group',
            },
            {
                hideLabels: true,
        ...
        var sn_input = constructField(
            `items_serial_numbers_${pk}`,
            {
                ...
                required: false,
                label: 'Sorozatszámok',
                help_text: 'Írd be a sorozatszámokat a beérkezett tételekhez',
                icon: 'fa-hashtag',
            },
            {
                hideLabels: true,
        ...
        if (global_settings.BARCODE_ENABLE) {
            buttons += makeIconButton('fa-qrcode', 'button-row-add-barcode', pk, 'Vonalkód hozzáadása');
            buttons += makeIconButton('fa-unlink icon-red', 'button-row-remove-barcode', pk, 'Vonalkód eltávolítása', {hidden: true});
        }

        buttons += makeIconButton('fa-sitemap', 'button-row-add-location', pk, 'Add meg a helyet', {
    ...
    <table class='table table-condensed' id='order-receive-table'>
        <thead>
            <tr>
                ...
                <th>Rendelési kód</th>
                <th>Beérkezett</th>
                <th style='min-width: 50px;'>Érkező mennyiség</th>
                <th style='min-width: 150px;'>Állapot</th>
                <th></th>
            </tr>
        </thead>
        <tbody>
    ...
    constructForm(`/api/order/po/${order_id}/receive/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Bevételezés megerősítése',
        title: 'Beszerzési rendelés tételeinek bevételezése',
        afterRender: function(fields, opts) {
            ...
            if (global_settings.BARCODE_ENABLE) {
                $(opts.modal).find('.button-row-add-barcode').click(function() {
                    ...
                    barcodeDialog('Tétel vonalkód beolvasása', {
                        details: 'Beérkezett tétel vonalkódjának leolvasása (egyik meglévő készlet tétellel sem egyezhet)',
                        onScan: function(response, barcode_options) {
                            ...
                            showBarcodeMessage(barcode_options.modal, 'Ez a vonalkód már egy másik tételé');
                        },
                        onError400: function(response, barcode_options) {
                            if (response.barcode_data && response.barcode_hash) {
                                ...
                                showBarcodeMessage(barcode_options.modal, 'Érvénytelen vonalkód adat');
                            }
                        }
                    });
                });
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Rendelés késésben');
                    }
...
function deletePurchaseOrderLineItems(items, options={}) {
    ...
    Az összes kijelölt sortétel törlésre kerül
    </div>
    ...
    constructForm('/api/order/po-line/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Töröljük a kiválasztott sortételeket?',
        form_data: {
            items: ids,
        },
        preFormContent: html,
...
function loadPurchaseOrderLineItemTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        sidePagination: 'server',
        formatNoMatches: function() {
            return 'Nem találhatók sortételek';
        },
        queryParams: filters,
        original: options.params,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Csomagolási mennyiség: ${pack_quantity} - Teljes mennyiség: ${total}${units}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.target_date) {
                        ...
                        if (row.overdue) {
                            html += makeIconBadge('fa-calendar-times icon-red', 'Ez a sortétel késésben van');
                        }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (options.allow_receive && row.received < row.quantity) {
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-line-receive', pk, 'Sortétel bevételezése');
                    }

                    if (options.allow_edit) {
                        buttons += makeCopyButton('button-line-duplicate', pk, 'Sortétel másolása');
                        buttons += makeEditButton('button-line-edit', pk, 'Sortétel szerkesztése');
                        buttons += makeDeleteButton('button-line-delete', pk, 'Sortétel törlése');
                    }
