    ...
    getApiEndpointOptions(url, function(OPTIONS) {
        ...
        case 'POST':
            if (canCreate(OPTIONS)) {
                ...
                showMessage('Działanie zabronione', {
                    style: 'danger',
                    details: 'Operacja utworzenia nie jest dozwolona',
        ...
        case 'PATCH':
            if (canChange(OPTIONS)) {
                ...
                showMessage('Działanie zabronione', {
                    style: 'danger',
                    details: 'Operacja aktualizacji nie jest dozwolona',
                    icon: 'fas fa-user-times',
                });
        ...
        case 'DELETE':
            if (canDelete(OPTIONS)) {
                ...
                showMessage('Działanie zabronione', {
                    style: 'danger',
                    details: 'Operacja usuwania nie jest dozwolona',
                    icon: 'fas fa-user-times',
                });
        ...
        case 'GET':
            if (canView(OPTIONS)) {
                ...
                showMessage('Działanie zabronione', {
                    style: 'danger',
                    details: 'Operacja przeglądania nie jest dozwolona',
                    icon: 'fas fa-user-times',
                });
...
function insertPersistButton(options) {
    ...
    var message = options.persistMessage || 'Pozostaw ten formularz otwarty';
        ...
        if (field) {
            ...
            case 'decimal':
                if (!validateFormField(name, options)) {
                    ...
                    data_errors[name] = ['Wprowadź poprawny numer'];
                }
                break;
            default:
                break;
...
function constructField(name, parameters, options={}) {
    ...
    if (extra) {
        ...
        if (!parameters.required && !options.hideClearButton) {
            html += `
            <span class='input-group-text form-clear' id='clear_${field_name}' title='Wyczyść wejście'>
...
function selectImportFields(url, data={}, options={}) {
    ...
    var headers = `<tr><th>Kolumna pliku</th><th>Nazwa pola</th></tr>`;
