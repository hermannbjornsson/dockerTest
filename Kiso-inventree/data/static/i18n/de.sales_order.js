    ...
    inventreeGet(`/api/order/so/shipment/${shipment_id}/`, {}, {
        success: function(shipment) {
            ...
            if (!allocations || allocations.length == 0) {
                ...
                Dieser Sendung wurden keine Artikel zugewiesen
                </div>
                `;
            } else {
                html = `
                Die folgenden Artikel werden verschickt
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        stock = `Seriennummer: ${allocation.serial}`;
                    } else {
                        stock = `Anzahl: ${allocation.quantity}`;
                    }
            ...
            constructForm(`/api/order/so/shipment/${shipment_id}/ship/`, {
                method: 'POST',
                title: `Sendung fertigstellen ${shipment.reference}`,
                fields: {
                    shipment_date: {
                ...
                preFormContent: html,
                confirm: true,
                confirmMessage: 'Sendung bestätigen',
...
function completePendingShipments(order_id, options={}) {
    ...
    if (allocated_shipments.length > 0) {
        ...
        if (!pending_shipments.length) {
            html += `
            Keine ausstehenden Sendungen gefunden
            `;
        } else {
            html += `
            Keine Lagerartikel für offene Sendungen zugewiesen
            `;
        }
        ...
        `/api/order/so/${order_id}/complete/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Diese Bestellung als vollständig markieren?
                </div>`;

                if (opts.context.pending_shipments) {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    Auftrag kann nicht abgeschlossen werden, da unvollständige Sendungen vorhanden sind<br>
                    </div>`;
                }
                ...
                if (!opts.context.is_complete) {
                    ...
                    Dieser Auftrag enthält Positionen, die noch nicht abgeschlossen sind.<br>
                    Fertigstellen dieser Bestellung bedeutet, dass sie und ihre Positionen nicht länger bearbeitbar sind.
                    </div>`;
                }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Bestellung überfällig');
                    }
...
function loadSalesOrderShipmentTable(table, options={}) {
    ...
    function makeShipmentActions(row) {
        ...
        html += makeEditButton('button-shipment-edit', pk, 'Sendung bearbeiten');

        if (!options.shipped) {
            html += makeIconButton('fa-truck icon-green', 'button-shipment-ship', pk, 'Sendung fertigstellen');
        }

        var enable_delete = row.allocations && row.allocations.length == 0;

        html += makeDeleteButton('button-shipment-delete', pk, 'Sendung löschen', {disabled: !enable_delete});
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine passenden Sendungen gefunden';
        },
        columns: [
            {
                visible: false,
...
function allocateStockToSalesOrder(order_id, line_items, options={}) {
    ...
    constructForm(`/api/order/so/${order_id}/allocate/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Bestandszuordnung bestätigen',
        title: 'Artikel zu Kundenauftrag zuweisen',
...
function loadSalesOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        original: options.params,
        formatNoMatches: function() {
            return 'Keine Allokationen für Verkaufsaufträge gefunden';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    var text = `Lagerartikel ${row.item}`;
    ...
    function setupCallbacks() {
        ...
        table.find('.button-allocation-edit').click(function() {
            ...
            constructForm(
                `/api/order/so-allocation/${pk}/`,
                {
                    ...
                    title: 'Bestandszuordnung bearbeiten',
        ...
        table.find('.button-allocation-delete').click(function() {
            ...
            constructForm(
                ...
                {
                    method: 'DELETE',
                    confirmMessage: 'Löschvorgang bestätigen',
                    title: 'Bestands-Zuordnung löschen',
                    refreshTable: options.table,
                }
            );
        });
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `Anzahl: ${row.quantity}`;

                    if (item && item.serial != null && row.quantity == 1) {
                        text = `Seriennummer: ${item.serial}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        return `<em>an Kunde versand - ${row.shipment_date}</em>`;
                    } else if (row.location) {
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || 'Lagerort',
                            `/stock/location/${row.location}/`
                        );
                    } else {
                        return `<em>Lagerstandort nicht angegeben</em>`;
                    }
                },
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        html += `<span class='badge bg-success badge-right'>Versendet</span>`;
                    } else {
                        html += makeEditButton('button-allocation-edit', pk, 'Bestands-Zuordnung bearbeiten');
                        html += makeDeleteButton('button-allocation-delete', pk, 'Bestands-Zuordnung löschen');
                    }
...
function loadSalesOrderLineItemTable(table, options={}) {
        ...
        },
        {
            ...
            formatter: function(value, row) {
                if (row.target_date) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times', 'Diese Position ist überfällig');
                    }
        ...
        columns.push(
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (available > 0) {
                        ...
                        html += `<span class='badge rounded-pill bg-danger'>Kein Lagerbestand verfügbar</span>`;
                    }

                    if (required > 0) {
                        if (available >= required) {
                            html += makeIconBadge('fa-check-circle icon-green', 'Ausreichender Bestand verfügbar');
                        } else {
                            html += makeIconBadge('fa-times-circle icon-red', 'Unzureichender Bestand verfügbar');
                        }
                    }
        ...
        formatter: function(value, row, index, field) {
            ...
            if (row.part && row.part_detail) {
                ...
                if (options.allow_edit && (row.shipped < row.quantity)) {
                    if (part.trackable) {
                        buttons += makeIconButton('fa-hashtag icon-green', 'button-add-by-sn', pk, 'Seriennummern zuweisen');
                    }
                    buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-add', pk, 'Bestand zuweisen');
                    if (part.purchaseable) {
                        buttons += makeIconButton('fa-shopping-cart', 'button-buy', row.part, 'Bestand kaufen');
                    }

                    if (part.assembly) {
                        buttons += makeIconButton('fa-tools', 'button-build', row.part, 'Bestand bauen');
                    }
                }
            }

            buttons += makeIconButton('fa-dollar-sign icon-green', 'button-price', pk, 'Preis berechnen');

            if (options.allow_edit) {
                buttons += makeCopyButton('button-duplicate', pk, 'Position duplizieren');
                buttons += makeEditButton('button-edit', pk, 'Position bearbeiten');
            }

            if (options.allow_delete) {
                var delete_disabled = false;

                var title = 'Position löschen';

                if (row.shipped) {
                    delete_disabled = true;
                    title = 'Kann nicht gelöscht werden, da Artikel versandt wurden';
                } else if (row.allocated) {
                    delete_disabled = true;
                    title = 'Kann nicht gelöscht werden, da Artikel zugewiesen sind';
                }
            ...
            launchModalForm(
                '/order/purchase-order/pricing/',
                {
                    ...
                    buttons: [
                        {
                            name: 'update_price',
                            title: 'Stückpreis aktualisieren'
                        },
                    ],
                    success: reloadTable,
                }
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine passenden Positionen gefunden';
        },
        queryParams: filters,
        original: options.params,
        url: options.url,
