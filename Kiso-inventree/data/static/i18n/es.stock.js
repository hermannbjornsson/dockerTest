...
function serializeStockItem(pk, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Serializar Artículo de Stock';
    ...
    if (options.part) {
        ...
        inventreeGet(`/api/part/${options.part}/serial-numbers/`, {}, {
            success: function(data) {
                if (data.next) {
                    options.fields.serial_numbers.placeholder = `Siguiente número de serie disponible: ${data.next}`;
                } else if (data.latest) {
                    options.fields.serial_numbers.placeholder = `Último número de serie: ${data.latest}`;
                }
            },
            async: false,
        });
    ...
    options.confirmMessage = 'Confirmar Serialización de Stock';
...
function stockLocationFields(options={}) {
    var fields = {
        parent: {
            help_text: 'Ubicación del stock principal',
            required: false,
        },
        name: {},
        description: {},
...
function editStockLocation(pk, options={}) {
    ...
    options.title = 'Editar ubicación de stock';
...
function createStockLocation(options={}) {
    ...
    options.method = 'POST';
    options.fields = stockLocationFields(options);
    options.title = 'Nueva Ubicación de Stock';
    options.persist = true;
    options.persistMessage = 'Crear otra ubicación después de ésta';
    options.successMessage = 'Ubicación de inventario creada';
...
function deleteStockLocation(pk, options={}) {
    ...
    var subChoices = [
        {
            value: 0,
            display_name: 'Mover a la ubicación de inventario del padre',
        },
        {
            value: 1,
    ...
    constructForm(url, {
        title: 'Eliminar ubicación de stock',
        method: 'DELETE',
        fields: {
            'delete_stock_items': {
...
function stockItemFields(options={}) {
    var fields = {
                    ...
                    if (data.trackable) {
                        ...
                        inventreeGet(`/api/part/${data.pk}/serial-numbers/`, {}, {
                            success: function(data) {
                                var placeholder = '';
                                if (data.next) {
                                    placeholder = `Siguiente número de serie disponible: ${data.next}`;
                                } else if (data.latest) {
                                    placeholder = `Último número de serie: ${data.latest}`;
                                }
                        ...
                        setFormInputPlaceholder('serial_numbers', 'Esta parte no se puede serializar', opts);
                    }
        ...
        quantity: {
            help_text: 'Introduzca la cantidad inicial para este artículo de stock',
        },
        serial_numbers: {
            icon: 'fa-hashtag',
            type: 'string',
            label: 'Números de serie',
            help_text: 'Introduzca números de serie para el nuevo stock (o deje en blanco)',
            required: false,
        },
        serial: {
            icon: 'fa-hashtag',
...
function duplicateStockItem(pk, options) {
    ...
    inventreeGet(`/api/stock/${pk}/`, {}, {
        success: function(data) {
            ...
            options.method = 'POST';
            options.title = 'Duplicar artículo de stock';
...
function deleteStockItem(pk, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    ¿Está seguro que desea eliminar este artículo de stock?
    </div>`;
...
function editStockItem(pk, options={}) {
    ...
    options.title = 'Editar artículo de stock';
...
function createNewStockItem(options={}) {
    ...
    options.title = 'Nuevo artículo de stock';
    options.method = 'POST';
    ...
    options.successMessage = 'Artículo de stock creado';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                addCachedAlert('Crear nuevo artículo de stock', {
                    icon: 'fas fa-boxes',
                });
                ...
                var details = `
                <br>Cantidad: ${response.quantity}
                <br>Números de serie: ${response.serial_numbers}
                `;

                showMessage('Creados varios artículos de stock', {
                    icon: 'fas fa-boxes',
                    details: details,
                });
...
function findStockItemBySerialNumber(part_id) {
    ...
    constructFormBody({}, {
        title: 'Encontrar número serial',
        fields: {
            serial: {
                label: 'Número de serie',
                help_text: 'Introducir número de serie',
                    ...
                    success: function(response) {
                        if (response.length == 0) {
                            ...
                            handleFormErrors(
                                {
                                    'serial': [
                                        'Ningún número de serie coincidente',
                                    ]
                                }, fields, opts
                            );
                            ...
                            handleFormErrors(
                                {
                                    'serial': [
                                        'Más de un resultado encontrado',
                                    ]
                                }, fields, opts
                            );
                        } else {
...
function assignStockToCustomer(items, options={}) {
    ...
    constructForm('/api/stock/assign/', {
        ...
        confirm: true,
        confirmMessage: 'Confirmar asignación de stock',
        title: 'Asignar Stock al Cliente',
...
function mergeStockItems(items, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    <h5>Advertencia: La operación de fusión no puede ser revertida</h5>
    <strong>Alguna información se perderá al combinar artículos de stock:</strong>
    <ul>
        <li>Se eliminará el historial de transacciones de stock para artículos fusionados</li>
        <li>La información de la parte del proveedor se eliminará para los artículos fusionados</li>
    </ul>
    </div>
    `;
    ...
    constructForm('/api/stock/merge/', {
        ...
        confirm: true,
        confirmMessage: 'Confirmar fusión de artículos de stock',
        title: 'Fusionar Artículos de Stock',
        afterRender: function(fields, opts) {
            // Add button callbacks to remove rows
            $(opts.modal).find('.button-stock-item-remove').click(function() {
                var pk = $(this).attr('pk');
...
function adjustStock(action, items, options={}) {
    ...
    case 'move':
        ...
        break;
    case 'count':
        formTitle = 'Contar Stock';
        actionTitle = 'Contar';
        url = '/api/stock/count/';
        break;
    case 'take':
        formTitle = 'Eliminar Stock';
        actionTitle = 'Tomar';
        url = '/api/stock/remove/';
        break;
    case 'add':
        formTitle = 'Añadir Stock';
        actionTitle = 'Añadir';
        url = '/api/stock/add/';
        break;
    case 'delete':
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantity += ` - <small>Lote: ${item.batch}</small>`;
        }
        ...
        if (actionTitle != null) {
            actionInput = constructField(
                `items_quantity_${pk}`,
                {
                    ...
                    max_value: maxValue,
                    value: value,
                    title: readonly ? 'La cantidad no se puede ajustar para el stock serializado' : 'Especificar cantidad de stock',
                    required: true,
                },
                {
                    hideLabels: true,
    ...
    if (itemCount == 0) {
        showAlertDialog(
            'Seleccionar artículos de stock',
    ...
    constructForm(url, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Confirmar ajuste de stock',
        title: formTitle,
        afterRender: function(fields, opts) {
            // Add button callbacks to remove rows
            $(opts.modal).find('.button-stock-item-remove').click(function() {
...
function passFailBadge(result) {
    ...
    if (result) {
        return `<span class='badge badge-right rounded-pill bg-success'>PASA</span>`;
    } else {
        return `<span class='badge badge-right rounded-pill bg-danger'>FALLO</span>`;
    }
}

function noResultBadge() {
    return `<span class='badge badge-right rounded-pill bg-info'>SIN RESULTADO</span>`;
}
...
function loadStockTestResultsTable(table, options) {
    ...
    function makeButtons(row, grouped) {
        ...
        if (row.requires_attachment == false && row.requires_value == false && !row.result) {
            ...
            html += makeIconButton('fa-check-circle icon-green', 'button-test-tick', row.test_name, 'Pruebas pasadas');
        }

        html += makeIconButton('fa-plus icon-green', 'button-test-add', row.test_name, 'Añadir resultado de prueba');

        if (!grouped && row.result != null) {
            var pk = row.pk;
            html += makeEditButton('button-test-edit', pk, 'Editar resultado de prueba');
            html += makeDeleteButton('button-test-delete', pk, 'Eliminar resultado de prueba');
        }
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'No se encontraron resultados de prueba';
        },
        queryParams: filters,
        original: params,
        onPostBody: function() {
    ...
    $(table).on('click', '.button-test-add', function() {
        ...
        constructForm('/api/stock/test/', {
            ...
            title: 'Añadir Resultado de Prueba',
    ...
    $(table).on('click', '.button-test-edit', function() {
        ...
        constructForm(url, {
            ...
            title: 'Editar Resultados de Prueba',
            onSuccess: reloadTestTable,
        });
    });
    ...
    $(table).on('click', '.button-test-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
        <strong>Eliminar resultado de prueba:</strong> ${row.test_name || row.test || row.key}
        </div>`;

        constructForm(url, {
            method: 'DELETE',
            title: 'Borrar Resultado de Prueba',
            onSuccess: reloadTestTable,
            preFormContent: html,
        });
    });
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        text = 'Consumido por orden de construcción';
        ...
        text = `Instalado en el artículo de stock ${row.belongs_to}`;
        ...
        text = 'Asignado a la Orden de Venta';
        ...
        text = '<i>Ninguna ubicación de stock establecida</i>';
        url = '';
    }
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Artículos de stock combinados', {
                            style: 'success',
                        });
                    }
                });
...
function loadStockTable(table, options) {
    ...
    col = {
        ...
        formatter: function(value, row) {
            ...
            if (row.is_building) {
                html += makeIconBadge('fa-tools', 'El artículo de stock está en producción');
            }

            if (row.sales_order) {
                // Stock item has been assigned to a sales order
                html += makeIconBadge('fa-truck', 'Artículo de stock asignado al pedido de venta');
            } else if (row.customer) {
                // StockItem has been assigned to a customer
                html += makeIconBadge('fa-user', 'Artículo de stock asignado al cliente');
            } else if (row.allocated) {
                if (row.serial != null && row.quantity == 1) {
                    html += makeIconBadge('fa-bookmark icon-yellow', 'Se ha asignado un artículo de stock serializado');
                } else if (row.allocated >= row.quantity) {
                    html += makeIconBadge('fa-bookmark icon-yellow', 'Artículo de stock ha sido completamente asignado');
                } else {
                    html += makeIconBadge('fa-bookmark', 'Artículo de stock ha sido asignado parcialmente');
                }
            } else if (row.belongs_to) {
                html += makeIconBadge('fa-box', 'Artículo de stock ha sido instalado en otro artículo');
            } else if (row.consumed_by) {
                html += makeIconBadge('fa-tools', 'Stock item has been consumed by a build order');
            }

            if (row.expired) {
                html += makeIconBadge('fa-calendar-times icon-red', 'Artículo de stock ha caducado');
            } else if (row.stale) {
                html += makeIconBadge('fa-stopwatch', 'El artículo de stock caducará pronto');
            }

            // Special stock status codes
            if (row.status == stockCodes.REJECTED.key) {
                html += makeIconBadge('fa-times-circle icon-red', 'Artículo de stock ha sido rechazado');
            } else if (row.status == stockCodes.LOST.key) {
                html += makeIconBadge('fa-question-circle', 'Artículo de stock perdido');
            } else if (row.status == stockCodes.DESTROYED.key) {
                html += makeIconBadge('fa-skull-crossbones', 'Artículo de stock destruido');
            }

            if (row.quantity <= 0) {
                html += `<span class='badge badge-right rounded-pill bg-danger'>Agotado</span>`;
            }
    ...
    col = {
        ...
        formatter: function(value, row) {
            ...
            if (row.supplier_part_detail) {
                ...
                text = `<i>Parte del proveedor no especificada</i>`;
            }
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'No hay artículos de stock que coincidan con la consulta';
        },
...
function loadStockLocationTable(table, options) {
    ...
    setupFilterList(filterKey, table, filterListElement, {
        ...
        singular_name: 'ubicación de almacén',
    ...
    cols.push({
        ...
        formatter: function(details, row) {
            ...
            if (!details || !Object.keys(details).length) {
                return `<small><em>Sin cambios</em></small>`;
            }
            ...
            if (details.location) {
                ...
                if (details.location_detail) {
                    ...
                    html += `<i>Ubicación ya no existe</i>`;
                }
            ...
            if (details.purchaseorder) {
                ...
                if (details.purchaseorder_detail) {
                    ...
                    html += `<i>La orden de compra ya no existe</i>`;
                }
            ...
            if (details.salesorder) {
                ...
                if (details.salesorder_detail) {
                    ...
                    html += `<em>El pedido de venta ya no existe</em>`;
                }
            ...
            if (details.returnorder) {
                html += `<tr><th>Orden de devolución</th>`;
                html += '<td>';
                ...
                if (details.returnorder_detail) {
                    ...
                    html += `<em>El pedido de devolución ya no existe</em>`;
                }
            ...
            if (details.customer) {
                ...
                if (details.customer_detail) {
                    ...
                    html += `<i>El cliente ya no existe</i>`;
                }
            ...
            if (details.stockitem) {
                html += '<tr><th>Artículo de stock</td>';
                ...
                if (details.stockitem_detail) {
                    ...
                    html += `<i>Artículo de stock ya no existe</i>`;
                }
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            if (value) {
                ...
                return `<i>No hay información de usuario</i>`;
            }
        }
    });
...
function loadInstalledInTable(table, options) {
    ...
    */
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Ningún artículo instalado';
        },
        columns: [
            {
                field: 'part',
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeIconButton('fa-unlink', 'button-uninstall', pk, 'Desinstalar artículo de stock');
...
function uninstallStockItem(installed_item_id, options={}) {
    ...
    constructForm(
        `/api/stock/${installed_item_id}/uninstall/`,
        {
            confirm: true,
            method: 'POST',
            title: 'Desinstalar artículo de stock',
            fields: {
                location: {
                    icon: 'fa-sitemap',
                    filters: {
...
function installStockItem(stock_item_id, part_id, options={}) {
    ...
    <div class='alert alert-block alert-info'>
        ...
        Los artículos de stock sólo pueden ser instalados si cumplen con los siguientes criterios:<br>
        <ul>
        ...
        `/api/stock/${stock_item_id}/install/`,
        {
            ...
            fields: {
                part: {
                    ...
                    required: 'true',
                    label: 'Parte',
                    help_text: 'Seleccionar parte para instalar',
                    model: 'part',
                    api_url: '/api/part/',
                    auto_fill: true,
                    filters: {
            ...
            confirm: true,
            title: 'Instalar artículo de stock',
            preFormContent: html,
            onSuccess: function(response) {
                if (options.onSuccess) {
