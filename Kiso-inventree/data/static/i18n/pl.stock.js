        ...
        inventreeGet(`/api/part/${options.part}/serial-numbers/`, {}, {
            success: function(data) {
                if (data.next) {
                    ...
                    options.fields.serial_numbers.placeholder = `Ostatni numer seryjny: ${data.latest}`;
                }
            },
            async: false,
        });
...
function deleteStockLocation(pk, options={}) {
    ...
    Czy na pewno chcesz skasować tą lokację?
    </div>
    `;
                        ...
                        inventreeGet(`/api/part/${data.pk}/serial-numbers/`, {}, {
                            success: function(data) {
                                ...
                                if (data.next) {
                                    ...
                                    placeholder = `Ostatni numer seryjny: ${data.latest}`;
                                }
...
function deleteStockItem(pk, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Czy na pewno chcesz usunąć tą część?
    </div>`;
...
function createNewStockItem(options={}) {
    ...
    options.successMessage = 'Utworzono element magazynowy';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                var details = `
                <br>Ilość: ${response.quantity}
                <br>Numer seryjny: ${response.serial_numbers}
                `;
...
function assignStockToCustomer(items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.serial && item.quantity == 1) {
            quantity = `Numer seryjny: ${item.serial}`;
        } else {
            quantity = `Ilość: ${item.quantity}`;
        }
...
function adjustStock(action, items, options={}) {
    ...
    case 'take':
        ...
        break;
    case 'add':
        formTitle = 'Dodaj stan';
        actionTitle = 'Dodaj';
        url = '/api/stock/add/';
        break;
    case 'delete':
        formTitle = 'Usuń stan magazynowy';
        allowSerializedStock = true;
        break;
    default:
        break;
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantity += ` - <small>Partia: ${item.batch}</small>`;
        }
...
function noResultBadge() {
    return `<span class='badge badge-right rounded-pill bg-info'>BRAK WYNIKÓW</span>`;
}
...
function loadStockTestResultsTable(table, options) {
    ...
    function makeButtons(row, grouped) {
        ...
        html += makeIconButton('fa-plus icon-green', 'button-test-add', row.test_name, 'Dodaj wynik testu');
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        text = 'Zużyte przez kolejność kompilacji';
...
function makeStockActions(table) {
    let actions = [
        ...
        },
        {
            ...
            title: 'Przelicz stan magazynowy',
            permission: 'stock.change',
            callback: function(data) {
        ...
        },
        {
            ...
            title: 'Przenieś stan magazynowy',
            permission: 'stock.change',
            callback: function(data) {
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Scalone przedmioty magazynowe', {
                            style: 'success',
                        });
                    }
                });
        ...
        formatter: function(details, row) {
            ...
            if (details.location) {
                ...
                if (details.location_detail) {
                    ...
                    html += `<i>Lokalizacja już nie istnieje</i>`;
                }
            ...
            if (details.purchaseorder) {
                ...
                if (details.purchaseorder_detail) {
                    ...
                    html += `<i>Zamówienie zakupu już nie istnieje</i>`;
                }
            ...
            if (details.salesorder) {
                html += `<tr><th>Zamówienie zakupu</th>`;
                html += '<td>';
            ...
            if (details.customer) {
                ...
                if (details.customer_detail) {
                    ...
                    html += `<i>Klient już nie istnieje</i>`;
                }
            ...
            if (details.stockitem) {
                html += '<tr><th>Element magazynowy</td>';
                ...
                if (details.stockitem_detail) {
                    ...
                    html += `<i>Element magazynowy już nie istnieje</i>`;
                }
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            if (value) {
                ...
                return `<i>Brak informacji o użytkowniku</i>`;
            }
        }
    });
