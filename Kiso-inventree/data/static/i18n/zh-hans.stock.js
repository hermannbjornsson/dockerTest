    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                var details = `
                <br>数量: ${response.quantity}
                <br>序列号: ${response.serial_numbers}
                `;
...
function adjustStock(action, items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantity += ` - <small>批量: ${item.batch}</small>`;
        }
...
function loadStockTestResultsTable(table, options) {
    ...
    $(table).on('click', '.button-test-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
        <strong>删除测试结果:</strong> ${row.test_name || row.test || row.key}
        </div>`;
...
function loadStockTable(table, options) {
    ...
    col = {
        ...
        formatter: function(value, row) {
            ...
            if (row.is_building) {
                html += makeIconBadge('fa-tools', '库存品正在生产');
            }
