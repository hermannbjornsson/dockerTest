    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.is_customer) {
                    html += `<span title='Asiakas' class='fas fa-user-tie float-right'></span>`;
                }

                if (row.is_manufacturer) {
                    html += `<span title='Valmistaja' class='fas fa-industry float-right'></span>`;
                }

                if (row.is_supplier) {
                    html += `<span title='Toimittaja' class='fas fa-building float-right'></span>`;
                }
...
function loadContactTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        sidePagination: 'server',
        formatNoMatches: function() {
            return 'Yhteystietoja ei löytynyt';
        },
        showColumns: true,
        name: 'contacts',
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.allow_edit) {
                        html += makeEditButton('btn-contact-edit', pk, 'Muokkaa yhteystietoa');
                    }

                    if (options.allow_delete) {
                        html += makeDeleteButton('btn-contact-delete', pk, 'Poista yhteystieto');
                    }
