            ...
            if (!allocations || allocations.length == 0) {
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        stock = `일련번호: ${allocation.serial}`;
                    } else {
                        stock = `수량: ${allocation.quantity}`;
                    }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `수량: ${row.quantity}`;

                    if (item && item.serial != null && row.quantity == 1) {
                        text = `일련번호: ${item.serial}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        ...
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || '위치',
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        text = `일련번호: ${row.serial}`;
                    } else {
                        text = `수량: ${row.quantity}`;
                    }
