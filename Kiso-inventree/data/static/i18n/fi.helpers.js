...
function yesNoLabel(value, options={}) {
    ...
    if (toBool(value)) {
        text = options.pass || 'KYLLÄ';
        color = 'bg-success';
    } else {
        text = options.fail || 'EI';
        color = 'bg-warning';
    }
...
function trueFalseLabel(value, options={}) {
    options.pass = 'Tosi';
    options.fail = 'Epätosi';

    return yesNoLabel(value, options);
}


function editButton(url, text='Muokkaa') {
...
function renderClipboard(s, prepend=false) {
    ...
    let clipString = `<span class="d-none d-xl-inline"><button class="btn clip-btn" type="button" data-bs-toggle='tooltip' title='kopioi leikepöydälle'><em class="fas fa-copy"></em></button></span>`;
