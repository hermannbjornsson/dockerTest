...
function makeNotesField(options={}) {
    ...
    return `
    <div class='form-group'>
        <label class='control-label' for='notes'>Notlar</label>
        <div class='controls'>
            <div class='input-group'>
                <span class='input-group-text'>
...
function showInvalidResponseError(modal, response, status) {
    showBarcodeMessage(
        modal,
        `Invalid server response<br>Durum: '${status}'`
    );
}
                            ...
                            success: function(stockitem) {
                                ...
                                if (duplicate) {
                                    ...
                                    if (stockitem.location == location_id) {
                                        showBarcodeMessage(modal, 'Stok kalemi zaten bu konumda');
                                        return;
                                    }
                ...
                if ('stocklocation' in response) {
                    ...
                    inventreeGet(`/api/stock/location/${pk}/`, {}, {
                        ...
                        error: function() {
                            ...
                            showBarcodeMessage(
                                modal,
                                'Barkod geçerli bir konumla eşleşmiyor',
                                'warning',
                            );
                        }
                    });
