...
function createNewModal(options={}) {
    ...
    if (!options.hideSubmitButton) {
        buttons += `<button type='button' class='btn btn-${submitClass}' id='modal-form-submit'>Submit</button>`;
    }
    ...
    modalSetSubmitText(modal_name, options.submitText || 'Submit');
...
function openModal(options) {
    ...
    var close_text = options.close_text || 'Lukk';
    ...
    var ajax_data = {
        ...
        success: function(response) {
            ...
            if (response.html_form) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Ugyldig svar fra serveren', 'JSON response missing form data');
            }
        },
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    'Ingen respons',
                    'Ingen svar fra InvenTree-serveren',
                );
                ...
                showAlertDialog(
                    'Feil 401: Ikke autentisert',
                    'Autentiseringslegitimasjon ikke angitt',
                );
            } else if (xhr.status == 403) {
                showAlertDialog(
                    'Feil 403: Tilgang nektet',
                    'Du har ikke de nødvendige tillatelsene for tilgang til denne funksjonen',
                );
            } else if (xhr.status == 404) {
                showAlertDialog(
                    'Feil 404: Ressurs ikke funnet',
                    'Kan ikke finne den forespurte ressursen på serveren',
                );
            } else if (xhr.status == 408) {
                showAlertDialog(
                    'Feil 408: Tidsavbrudd',
