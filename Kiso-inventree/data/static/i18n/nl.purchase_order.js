...
function createPurchaseOrder(options={}) {
    ...
    constructForm('/api/order/po/', {
        ...
        title: options.title || 'Inkooporder aanmaken',
    });
}
...
function poLineItemFields(options={}) {
    ...
    var fields = {
        ...
        part: {
            ...
            secondary: {
                method: 'POST',
                title: 'Leveranciersonderdeel Toevoegen',
            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.is_complete) {
                    ...
                    <div class='alert alert-block alert-success'>
                        Alle artikelen zijn ontvangen
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Deze order heeft artikelen die niet zijn gemarkeerd als ontvangen.</br>
...
function cancelPurchaseOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Inkooporder annuleren',
            confirm: true,
            preFormContent: function(opts) {
                var html = `
                <div class='alert alert-info alert-block'>
                    Weet u zeker dat u deze inkooporder wilt annuleren?
                </div>`;

                if (!opts.context.can_cancel) {
                    html += `
                    <div class='alert alert-danger alert-block'>
                        Deze inkooporder kan niet geannuleerd worden
                    </div>`;
                }
...
function orderParts(parts_list, options={}) {
    ...
    function renderPart(part, opts={}) {
        ...
        var purchase_order_prefix = `
            <button type='button' class='input-group-text button-row-new-po' pk='${pk}' title='Nieuwe inkooporder'>
                <span class='fas fa-plus-circle icon-green'></span>
            </button>
        `;
    ...
    <table class='table table-striped table-condensed' id='order-parts-table'>
        <thead>
            <tr>
                <th>Onderdeel</th>
                <th style='min-width: 300px;'>Leveranciersonderdeel</th>
                <th style='min-width: 300px;'>Inkooporder</th>
                <th style='min-width: 50px;'>Hoeveelheid</th>
                <th><!-- Actions --></th>
            </tr>
        </thead>
        <tbody>
        ...
        afterRender: function(fields, opts) {
            parts.forEach(function(part) {
                ...
                initializeRelatedField({
                    ...
                    noResults: function(query) {
                        return 'Geen overeenkomende inkooporders';
                    }
                }, null, opts);
...
function receivePurchaseOrderItems(order_id, line_items, options={}) {
    ...
    if (line_items.length == 0) {
        ...
        showAlertDialog(
            'Selecteer artikelen',
            'Ten minste één artikel moet worden geselecteerd',
        );
        return;
    }
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        var sn_input = constructField(
            `items_serial_numbers_${pk}`,
            {
                ...
                required: false,
                label: 'Serienummers',
                help_text: 'Voer serienummers in voor inkomende voorraadartikelen',
                icon: 'fa-hashtag',
            },
            {
                hideLabels: true,
    ...
    constructForm(`/api/order/po/${order_id}/receive/`, {
        ...
        title: 'Ontvang Artikelen Inkooporder',
        afterRender: function(fields, opts) {
...
function loadPurchaseOrderTable(table, options) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Geen inkooporder gevonden';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.target_date) {
                        ...
                        if (row.overdue) {
                            html += makeIconBadge('fa-calendar-times icon-red', 'Dit artikel is achterstallig');
                        }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (options.allow_receive && row.received < row.quantity) {
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-line-receive', pk, 'Artikel ontvangen');
                    }

                    if (options.allow_edit) {
                        buttons += makeCopyButton('button-line-duplicate', pk, 'Artikel dupliceren');
                        buttons += makeEditButton('button-line-edit', pk, 'Artikel bewerken');
                        buttons += makeDeleteButton('button-line-delete', pk, 'Artikel verwijderen');
                    }
