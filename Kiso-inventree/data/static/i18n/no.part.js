...
function partGroups() {
    ...
    return {
        ...
        initial_stock: {
            title: 'Innledende lagerbeholdning',
            collapsible: true,
            hidden: !global_settings.PART_CREATE_INITIAL,
        },
        initial_supplier: {
            title: 'Innledende leverandørdata',
            collapsible: true,
...
function duplicatePart(pk, options={}) {
    ...
    var title = 'Dupliser del';

    if (options.variant) {
        title = 'Opprett delvariant';
    }
    ...
    inventreeGet(`/api/part/${pk}/`, {}, {
        success: function(part) {
            if (part.active) {
                showAlertDialog(
                    'Aktiv del',
                    'Delen kan ikke slettes ettersom den er aktiv',
                    {
                        alert_style: 'danger',
                    }
                );
            ...
            Sletting av denne delen kan ikke angres
            <ul>
            <li>Eventuelle lagervarer for denne delen vil bli slettet</li>
            <li>Denne delen vil bli fjernet fra eventuelle stykklister</li>
            <li>All produsent- og leverandørinformasjon for denne delen vil bli slettet</li>
            </div>`;
                ...
                },
                {
                    ...
                    success: function(response) {
                        if (response.starred) {
                            ...
                            $(options.button).attr('title', 'Du abonnerer på varsler for denne artikkelen');

                            showMessage('Du abonnerer nå på varsler for denne artikkelen', {
                                style: 'success',
                            });
                        } else {
                            $(options.button).removeClass('fas fa-bell icon-green').addClass('fa fa-bell-slash');
                            $(options.button).attr('title', 'Abonner på varsler for denne artikkelen');

                            showMessage('Du har avsluttet abonnementet på varsler for denne artikkelen', {
                                style: 'warning',
                            });
                        }
                    }
...
function validateBom(part_id, options={}) {
    ...
    Godkjenning av BOM vil merke hvert linjeelement som godkjent
    </div>
    `;
    ...
    constructForm(`/api/part/${part_id}/bom-validate/`, {
        ...
        preFormContent: html,
        title: 'Godkjenn Stykkliste',
        reload: options.reload,
        onSuccess: function(response) {
            showMessage('Godkjente Stykkliste');
        }
    });
}
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            elements.push(`Lite lager: ${part.total_in_stock}`);
        } else if (part.unallocated_stock <= 0) {
            // There is no available stock at all
            elements.push(`Ingen varer på lager`);
        } else if (part.unallocated_stock < part.in_stock) {
            // Unallocated quantity is less than total quantity
            if (options.hideTotalStock) {
                elements.push(`Tilgjengelig: ${part.unallocated_stock}`);
            } else {
                elements.push(`Tilgjengelig: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
        ...
        elements.push(`Ingen lagerbeholdning`);
    }

    // Check for items on order
    if (part.ordering) {
        elements.push(`I bestilling: ${part.ordering}`);
    }

    // Check for items being built
    if (part.building) {
        elements.push(`Produseres: ${part.building}`);
    }
    ...
    if (options.showDemandInfo) {
        ...
        if (demand) {
            elements.push(`Etterspørsel: ${demand}`);
        }
    ...
    if (units && !options.no_units) {
        output += `<span class='badge rounded-pill text-muted bg-muted ${classes}'>Enhet: ${units}</span> `;
    }
...
function makePartIcons(part) {
    ...
    if (part.trackable) {
        html += makeIconBadge('fa-directions', 'Sporbar del');
    }

    if (part.virtual) {
        html += makeIconBadge('fa-ghost', 'Virtuell del');
    }

    if (part.is_template) {
        html += makeIconBadge('fa-clone', 'Maldel');
    }

    if (part.assembly) {
        html += makeIconBadge('fa-tools', 'Sammenstilt del');
    }

    if (part.starred) {
        html += makeIconBadge('fa-bell icon-green', 'Abonnert del');
    }

    if (part.salable) {
        html += makeIconBadge('fa-dollar-sign', 'Salgbar del');
    }
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.variant_stock > 0) {
                    text = `<em>${text}</em>`;
                    text += `<span title='Inkluderer variantbeholdning' class='fas fa-info-circle float-right icon-blue'></span>`;
                }
...
function createPartParameter(part_id, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Legg til Parameter';
...
function editPartParameter(param_id, options={}) {
    options.fields = partParameterFields();
    options.title = 'Rediger Parameter';
    options.focus = 'data';
...
function loadPartParameterTable(table, options) {
    ...
    $(table).inventreeTable({
        ...
        groupBy: false,
        formatNoMatches: function() {
            return 'Ingen parametere funnet';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-parameter-edit', pk, 'Rediger parameter');
                    html += makeDeleteButton('button-parameter-delete', pk, 'Slett parameter');
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Pakkeantall: ${row.pack_quantity} - Totalt Antall: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let total = value * row.supplier_part_detail.pack_quantity_native;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Pakkeantall: ${row.pack_quantity} - Totalt Antall: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Mottatt</span>`;
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>Ingen lagerbeholdning</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>Ingen lagerbeholdning</span>`;
    }

    rows += `<tr><td><b>Lagerbeholdning</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>I bestilling</b></td><td>${part.ordering} ${units}</td></tr>`;
    }

    if (part.building) {
        rows += `<tr><td><b>Produseres</b></td><td>${part.building} ${units}</td></tr>`;
    }
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Antall: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
