...
function renderStockItem(data, parameters={}) {
    ...
    if (data.quantity == 0) {
        ...
        if (data.serial && data.quantity == 1) {
            stock_detail = `Número de Sério: ${data.serial}`;
        } else {
            if (render_available_quantity) {
                var available = data.quantity - data.allocated;
                stock_detail = `Disponível: ${available}`;
            } else {
                stock_detail = `Quantidade: ${data.quantity}`;
            }
        }

        if (data.batch) {
            stock_detail += ` - <small>Lote: ${data.batch}</small>`;
        }
    }
...
function renderPart(data, parameters={}) {
    ...
    if (user_settings.PART_SHOW_QUANTITY_IN_FORMS) {
        ...
        if (!data.active) {
            labels += `<span class='badge badge-right rounded-pill bg-danger'>Inativo</span>`;
        }
    }
    ...
    return renderModel(
        {
            ...
            textSecondary: `Remessa ${data.reference}`,
        },
        parameters
    );
}
