    ...
    if (options.part) {
        ...
        inventreeGet(`/api/part/${options.part}/serial-numbers/`, {}, {
            success: function(data) {
                if (data.next) {
                    options.fields.serial_numbers.placeholder = `Neste tilgjengelige serienummer: ${data.next}`;
                } else if (data.latest) {
                    options.fields.serial_numbers.placeholder = `Siste serienummer: ${data.latest}`;
                }
            },
            async: false,
        });
                    ...
                    if (data.trackable) {
                        ...
                        inventreeGet(`/api/part/${data.pk}/serial-numbers/`, {}, {
                            success: function(data) {
                                var placeholder = '';
                                if (data.next) {
                                    placeholder = `Neste tilgjengelige serienummer: ${data.next}`;
                                } else if (data.latest) {
                                    placeholder = `Siste serienummer: ${data.latest}`;
                                }
...
function createNewStockItem(options={}) {
    ...
    options.successMessage = 'Lagevare opprettet';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                var details = `
                <br>Antall: ${response.quantity}
                <br>Serienummer: ${response.serial_numbers}
                `;
...
function assignStockToCustomer(items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.serial && item.quantity == 1) {
            quantity = `Serienummer: ${item.serial}`;
        } else {
            quantity = `Antall: ${item.quantity}`;
        }
...
function adjustStock(action, items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantity += ` - <small>Parti: ${item.batch}</small>`;
        }
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Sammenslåtte lagervarer', {
                            style: 'success',
                        });
                    }
                });
...
function installStockItem(stock_item_id, part_id, options={}) {
    ...
    <div class='alert alert-block alert-info'>
        ...
        <ul>
            <li>Lagervaren peker til en Del som er BOMen for denne lagervaren</li>
