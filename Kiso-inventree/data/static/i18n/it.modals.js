...
function createNewModal(options={}) {
    ...
    if (!options.hideCloseButton) {
        buttons += `<button type='button' class='btn btn-secondary' id='modal-form-close' data-bs-dismiss='modal'>Annulla</button>`;
    }
    ...
    modalSetTitle(modal_name, options.title || 'Titolo modulo');
    modalSetSubmitText(modal_name, options.submitText || 'Invia');
    modalSetCloseText(modal_name, options.closeText || 'Annulla');
...
function loadingMessageContent() {
    ...
    return `<span class='glyphicon glyphicon-refresh glyphicon-refresh-animate'></span> In attesa del server...`;
}
        ...
        <div class='panel'>
            <div class='panel panel-heading'>
                <div class='panel-title'>
                    <a data-bs-toggle='collapse' href="#collapse-error-info">Informazioni sull\'errore</a>
                </div>
            </div>
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.title = title;
    options.submitText = options.accept_text || 'Accetta';
    options.closeText = options.cancel_text || 'Annulla';
...
function openModal(options) {
    ...
    if (options.title) {
        ...
        modalSetTitle(modal, 'Caricamento Dati...');
    }
    ...
    modalSubmit(modal, function() {
        $(modal).find('.js-modal-form').ajaxSubmit({
                ...
                if ('form_valid' in response) {
                    ...
                    if (response.form_valid) {
                        ...
                        if (response.html_form) {
                            ...
                            $(modal).modal('hide');
                            showAlertDialog('Risposta dal server non valida', 'Dati del modulo mancanti dalla risposta server');
                        }
                    }
                } else {
                    $(modal).modal('hide');
            ...
            error: function(xhr) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Errore nel pubblicare i dati del modulo', renderErrorMessage(xhr));
            },
            complete: function() {
                // TODO
            }
    ...
    var ajax_data = {
        ...
        success: function(response) {
            ...
            if (response.html_form) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Risposta del server non valida', 'Dati del modulo mancanti di risposta JSON');
            }
        },
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    'Nessuna Risposta',
                    'Nessuna risposta dal server InvenTree',
                );
            } else if (xhr.status == 400) {
                showAlertDialog(
                    'Errore 400: Richiesta Non Valida',
                    'Il server ha restituito codice di errore 400',
                );
                ...
                showAlertDialog(
                    ...
                    'La risorsa richiesta non può essere localizzata sul server',
                );
            } else if (xhr.status == 408) {
                showAlertDialog(
                    'Errore 408: Timeout',
                    'Timeout connessione durante la richiesta di dati dal server',
                );
            } else {
                showAlertDialog('Errore nella richiesta di dati modulo', renderErrorMessage(xhr));
            }
