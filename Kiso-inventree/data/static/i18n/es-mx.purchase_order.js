...
function purchaseOrderFields(options={}) {
    ...
    if (options.duplicate_order) {
        ...
        fields.duplicate_line_items = {
            ...
            group: 'duplicate',
            type: 'boolean',
            label: 'Duplicar artículos de línea',
        ...
        fields.duplicate_extra_lines = {
            ...
            group: 'duplicate',
            type: 'boolean',
            label: 'Duplicar líneas adicionales',
            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.is_complete) {
                    ...
                    <div class='alert alert-block alert-warning'>
                        Este pedido tiene artículos de línea que no han sido marcados como recibidos.</br>
...
function orderParts(parts_list, options={}) {
    ...
    function renderPart(part, opts={}) {
        ...
        var supplier_part_prefix = `
            <button type='button' class='input-group-text button-row-new-sp' pk='${pk}' title='Nueva parte del proveedor'>
        ...
        var purchase_order_prefix = `
            <button type='button' class='input-group-text button-row-new-po' pk='${pk}' title='Nueva orden de compra'>
                <span class='fas fa-plus-circle icon-green'></span>
            </button>
        `;
            ...
            parts.forEach(function(part) {
                ...
                var supplier_part_field = {
                    ...
                    noResults: function(query) {
                        return 'No hay partes de proveedor coincidentes';
                    }
                };
                ...
                initializeRelatedField({
                    ...
                    noResults: function(query) {
                        return 'No hay órdenes de compra coincidentes';
                    }
                }, null, opts);
...
function receivePurchaseOrderItems(order_id, line_items, options={}) {
    ...
    if (line_items.length == 0) {
        ...
        showAlertDialog(
            'Seleccionar Artículos de Línea',
            'Debe seleccionar al menos un artículo de línea',
        );
        return;
    }
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        if (native_pack_quantity != 1) {
            ...
            <div class='alert alert-small alert-block alert-info'>
                ...
                Cantidad recibida: <span class='pack_received_quantity' id='items_received_quantity_${pk}'>${received}</span> ${units}
            </div>`;
        }
        ...
        if (global_settings.BARCODE_ENABLE) {
            buttons += makeIconButton('fa-qrcode', 'button-row-add-barcode', pk, 'Agregar código de barras');
            buttons += makeIconButton('fa-unlink icon-red', 'button-row-remove-barcode', pk, 'Eliminar código de barras', {hidden: true});
        }

        buttons += makeIconButton('fa-sitemap', 'button-row-add-location', pk, 'Especificar ubicación', {
    ...
    <table class='table table-condensed' id='order-receive-table'>
        <thead>
            <tr>
                ...
                <th>Recibido</th>
                <th style='min-width: 50px;'>Cantidad a recibir</th>
    ...
    constructForm(`/api/order/po/${order_id}/receive/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Confirmar recepción de artículos',
        title: 'Recibir artículos de orden de compra',
        afterRender: function(fields, opts) {
...
function loadPurchaseOrderLineItemTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        sidePagination: 'server',
        formatNoMatches: function() {
            return 'No hay artículos de línea';
        },
        queryParams: filters,
        original: options.params,
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (options.allow_receive && row.received < row.quantity) {
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-line-receive', pk, 'Recibir ítem de línea');
                    }

                    if (options.allow_edit) {
                        buttons += makeCopyButton('button-line-duplicate', pk, 'Duplicar artículo de línea');
                        buttons += makeEditButton('button-line-edit', pk, 'Editar artículo de línea');
                        buttons += makeDeleteButton('button-line-delete', pk, 'Eliminar artículo de línea');
                    }
