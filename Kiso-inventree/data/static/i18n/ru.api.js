...
function showApiError(xhr, url) {
    ...
    case 0:
        title = 'No Response';
        message = 'No response from the InvenTree server';
        break;
    // Bad request
    case 400:
        // Note: Normally error code 400 is handled separately,
        //       and should now be shown here!
        title = 'Ошибка 400: Некорректный запрос';
        message = 'API-запрос вернул код ошибки 400';
        break;
    // Not authenticated
    case 401:
        title = 'Ошибка 401: Авторизация не выполнена';
        message = 'Authentication credentials not supplied';
        break;
    // Permission denied
    case 403:
        title = 'Ошибка 403: Доступ запрещён';
        message = 'У вас нет прав доступа к этой функции';
        break;
    // Resource not found
    case 404:
    ...
    // Method not allowed
    case 405:
        title = 'Ошибка 405: Метод не разрешён';
        message = 'HTTP method not allowed at URL';
        break;
    // Timeout
    case 408:
        title = 'Ошибка 408: Таймаут';
        message = 'Connection timeout while requesting data from server';
        break;
    default:
        title = 'Необработанная ошибка';
        message = `Код ошибки: ${xhr.status}`;
