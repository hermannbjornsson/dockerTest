...
function selectReport(reports, items, options={}) {
    ...
    if (items.length > 0) {
        ...
        ${items.length} éléments sélectionnés
        </div>`;
    }
    ...
    <form method='post' action='' class='js-modal-form' enctype='multipart/form-data'>
        <div class='form-group'>
            ...
            Sélectionner un template de reporting
            </label>
            <div class='controls'>
    ...
    modalSetTitle(modal, 'Select Test Report Template');
    ...
    inventreeGet(options.url, params, {
        success: function(response) {
            if (response.length == 0) {
                showAlertDialog(
                    'No Reports Found',
                    'No report templates found which match the selected items',
                );
                return;
            }
