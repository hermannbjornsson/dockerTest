        ...
        `/api/build/${build_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Êtes-vous sûr de vouloir annuler cette construction?
                </div>`;
        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>L\'ordre de production a des sorties incomplètes</strong><br>
                ...
                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>Le stock requis n\'a pas encore été totalement alloué</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>La quantité nécessaire n\'a pas encore été complétée</div>`;
                }
            }
        ...
        {},
        {
            success: function(build) {
                ...
                inventreeGet(`/api/part/${build.part}/serial-numbers/`, {}, {
                    success: function(data) {
                        if (data.next) {
                            fields.serial_numbers.placeholder = `Prochain numéro de série disponible: ${data.next}`;
                        } else if (data.latest) {
                            fields.serial_numbers.placeholder = `Dernier numéro de série: ${data.latest}`;
                        }
                    },
                    async: false,
                });
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        La BOM contient des pièces traçables.<br>
                ...
                if (trackable) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Les pièces traçables peuvent avoir des numéros de série spécifiés<br>
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Numéro de série: ${output.serial}`;
    } else {
        output_html += `Quantité: ${output.quantity}`;
    ...
    buttons += makeRemoveButton('button-row-remove', pk, 'Supprimer la ligne');
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        text = `Numéro de série: ${row.serial}`;
                    } else {
                        text = `Quantité: ${row.quantity}`;
                    ...
                    if (row.batch) {
                        text += ` <small>(Lot: ${row.batch})</small>`;
                    }
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Pièce</th>
                <th>Allocated</th>
                <th style='min-width: 250px;'>Article en stock</th>
                <th>Quantité</th>
                <th></th>
            </tr>
        </thead>
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `En Commande: ${formatDecimal(row.on_order)}`);
                    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.allocated < row.quantity) {
                        ...
                        if (row.part_detail.purchaseable) {
                            buttons += makeIconButton('fa-shopping-cart icon-blue', 'button-buy', pk, 'Commander des stocks');
                        }
