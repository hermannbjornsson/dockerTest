...
function makeNotesField(options={}) {
    ...
    return `
    <div class='form-group'>
        <label class='control-label' for='notes'>Anotações</label>
        <div class='controls'>
            <div class='input-group'>
                <span class='input-group-text'>
...
function showInvalidResponseError(modal, response, status) {
    showBarcodeMessage(
        modal,
        `Invalid server response<br>Situação: '${status}'`
    );
}
...
function unlinkBarcode(data, options={}) {
    ...
    var html = `<b>Desatribuir Código de Barras</b><br>`;

    html += 'This will remove the link to the associated barcode';

    showQuestionDialog(
        'Desatribuir Código de Barras',
        html,
        {
            accept_text: 'Unlink',
            accept: function() {
