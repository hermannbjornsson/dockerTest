...
function addBomItem(part_id, options={}) {
    ...
    constructForm('/api/bom/', {
        fields: fields,
        method: 'POST',
        title: 'Stücklisten-Position anlegen',
        focus: 'sub_part',
...
function constructBomUploadTable(data, options={}) {
    ...
    function constructRow(row, idx, fields) {
        ...
        buttons += makeInfoButton('button-row-data', idx, 'Zeilendaten anzeigen');
        buttons += makeRemoveButton('button-row-remove', idx, 'Zeile entfernen');
...
function downloadBomTemplate(options={}) {
    ...
    constructFormBody({}, {
        title: 'Vorlage einer Stückliste herunterladen',
        fields: {
            format: {
                label: 'Format',
                help_text: 'Dateiformat auswählen',
                required: true,
                type: 'choice',
                value: format,
...
function exportBom(part_id, options={}) {
    ...
    constructFormBody({}, {
        ...
        fields: {
            ...
            cascade: {
                label: 'Multilevel Stückliste',
                help_text: 'Stücklisten-Daten für Untergruppen einbeziehen',
                type: 'boolean',
                value: inventreeLoad('bom-export-cascading', true),
            },
            levels: {
                label: 'Ebenen',
                help_text: 'Maximale Anzahl an Ebenen für Stückliste-Export auswählen (0 = alle Ebenen)',
                type: 'integer',
                value: 0,
                required: true,
                min_value: 0,
            ...
            substitute_part_data: {
                label: 'Alternative Teile einbeziehen',
                help_text: 'Alternative Teile in exportierte Stückliste einbeziehen',
                type: 'boolean',
                value: inventreeLoad('bom-export-substitute_part_data', false),
            },
            parameter_data: {
                label: 'Parameter-Daten einschließen',
                help_text: 'Teile-Parameter in Stücklisten-Export einschließen',
                type: 'boolean',
                value: inventreeLoad('bom-export-parameter_data', false),
            },
            stock_data: {
                label: 'Bestand einschließen',
                help_text: 'Teil-Bestand in Stückliste-Export einschließen',
                type: 'boolean',
                value: inventreeLoad('bom-export-stock_data', false),
            },
            manufacturer_data: {
                label: 'Herstellerdaten einschließen',
                help_text: 'Teil-Herstellerdaten in Stückliste-Export einschließen',
                type: 'boolean',
                value: inventreeLoad('bom-export-manufacturer_data', false),
            },
            supplier_data: {
                label: 'Zulieferer einschließen',
                help_text: 'Zulieferer-Daten in Stückliste-Export einschließen',
                type: 'boolean',
                value: inventreeLoad('bom-export-supplier_data', false),
            },
            pricing_data: {
                label: 'Preisdaten einschließen',
                help_text: 'Preisinformationen in Stücklisten-Export einschließen',
                type: 'boolean',
...
function bomSubstitutesDialog(bom_item_id, substitutes, options={}) {
    ...
    function renderSubstituteRow(substitute) {
        ...
        buttons += makeRemoveButton('button-row-remove', pk, 'Ersatzteil entfernen');
    ...
    <div class='alert alert-success alert-block'>
        Wählen Sie ein neues Ersatzteil aus und fügen Sie sie mit den folgenden Eingaben hinzu
    </div>
    `;
    ...
    function addRemoveCallback(modal, element) {
        $(modal).find(element).click(function() {
            ...
            Sind Sie sicher, dass Sie dieses Ersatzteil entfernen möchten?
            </div>
            `;
    ...
    constructForm('/api/bom/substitute/', {
        ...
        preFormContent: html,
        closeText: 'Schliessen',
        submitText: 'Ersatzteil hinzufügen',
        title: 'Stücklisten Ersatzteile bearbeiten',
...
function deleteBomItems(items, options={}) {
    ...
    Alle ausgewählte Stücklistenpositionen werden gelöscht
    </div>
    ...
    constructForm('/api/bom/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Ausgewählte Stücklistenpositionen löschen?',
        form_data: {
            items: ids,
        },
        preFormContent: html,
...
function loadBomTable(table, options={}) {
    ...
    cols.push(
        {
            ...
            formatter: function(value, row) {
                    ...
                    if (row.sub_assembly_received) {
                        ...
                        html += `
                            <a href='#' pk='${row.pk}' class='load-sub-assembly' id='load-sub-assembly-${row.pk}'>
                                <span class='fas fa-sync-alt' title='Stückliste für Bauteile laden'></span>
                            </a> `;
                    }
                }
                ...
                if (row.substitutes && row.substitutes.length > 0) {
                    html += makeIconBadge('fa-exchange-alt', 'Ersatzteile verfügbar');
                }
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (row.consumable) {
                text += ` <small>(Verbrauchsmaterial)</small>`;
            }
            ...
            if (any_pricing) {
                ...
                if (complete_pricing) {
                    html += makeIconBadge(
                        ...
                        'Stücklisten-Bepreisung ist vollständig',
                    );
                } else {
                    html += makeIconBadge(
                        'fa-exclamation-circle icon-yellow',
                        'Stücklisten-Bepreisung ist unvollständig',
                    );
                }
                ...
                let html = '<em>Keine Preisinformation verfügbar</em>';
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (available_stock <= 0) {
                text += makeIconBadge('fa-times-circle icon-red', 'Kein Lagerbestand verfügbar');
            } else {
                var extra = '';
    ...
    cols.push(
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.consumable) {
                    return `<em>Verbrauchsartikel</em>`;
                }
        ...
        cols.push({
            ...
            formatter: function(value, row) {
                ...
                if (row.part == options.parent_id) {
                    ...
                    var bValidate = makeIconButton('fa-check-circle icon-green', 'bom-validate-button', row.pk, 'Stücklisten-Position kontrollieren');

                    var bValid = makeIconButton('fa-check-double icon-green', 'bom-valid-button', row.pk, 'Diese Position wurde kontrolliert', {disabled: true});

                    var bSubs = makeIconButton('fa-exchange-alt icon-blue', 'bom-substitutes-button', row.pk, 'Ersatzteile bearbeiten');

                    var bEdit = makeEditButton('bom-edit-button', row.pk, 'Stücklisten-Position bearbeiten');

                    var bDelt = makeDeleteButton('bom-delete-button', row.pk, 'Stücklisten-Position löschen');
            ...
            footerFormatter: function(data) {
                return `
                <button class='btn btn-success float-right' type='button' title='Stücklisten-Position hinzufügen' id='bom-item-new-footer'>
                    ${makeIcon('fa-plus-circle')} Stücklisten-Position hinzufügen
                </button>
                `;
            }
        });
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine Stücklisten-Position(en) gefunden';
        },
        queryParams: filters,
        original: params,
        columns: cols,
    ...
    if (options.editable) {
        ...
        table.on('click', '.bom-edit-button', function() {
            ...
            constructForm(`/api/bom/${pk}/`, {
                fields: fields,
                title: 'Stücklisten-Position bearbeiten',
                focus: 'sub_part',
                onSuccess: function() {
                    reloadBomTable(table);
                }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.parent && row.parent != 'top-level-item') {
                        html += ` <em>(Geerbt von übergeordneter Stückliste)</em>`;
                    }
