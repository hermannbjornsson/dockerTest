...
function createNewStockItem(options={}) {
    ...
    options.title = 'Новая единица хранения';
    options.method = 'POST';
    ...
    options.successMessage = 'Товар создан';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                var details = `
                <br>Количество: ${response.quantity}
                <br>Серийные номера: ${response.serial_numbers}
                `;

                showMessage('Создано несколько единиц хранения', {
                    icon: 'fas fa-boxes',
                    details: details,
                });
...
function mergeStockItems(items, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    <h5>Предупреждение: Операция объединения не может быть отменена</h5>
    <strong>Следующие данные будут потеряны в процессе объединения:</strong>
    <ul>
        <li>История складских перемещений будет удалена для объединённых элементов</li>
        <li>Информация о деталях поставщика будет удалена для объединённых элементов</li>
    </ul>
    </div>
    `;
...
function adjustStock(action, items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantity += ` - <small>Партия: ${item.batch}</small>`;
        }
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Объединенные позиции на складе', {
                            style: 'success',
                        });
                    }
                });
    ...
    cols.push({
        ...
        formatter: function(details, row) {
            ...
            if (details.purchaseorder) {
                html += `<tr><th>Заказ на закупку</th>`;
                html += '<td>';
            ...
            if (details.salesorder) {
                html += `<tr><th>Заказ покупателя</th>`;
                html += '<td>';
            ...
            if (details.stockitem) {
                html += '<tr><th>Предметы на складе</td>';
...
function installStockItem(stock_item_id, part_id, options={}) {
    ...
    constructForm(
        `/api/stock/${stock_item_id}/install/`,
        {
            ...
            confirm: true,
            title: 'Установить единицу хранения',
            preFormContent: html,
            onSuccess: function(response) {
                if (options.onSuccess) {
