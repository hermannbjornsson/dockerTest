...
function constructBomUploadTable(data, options={}) {
    ...
    function constructRow(row, idx, fields) {
        ...
        buttons += makeInfoButton('button-row-data', idx, 'Wyświetl dane wiersza');
        buttons += makeRemoveButton('button-row-remove', idx, 'Usuń wiersz');
...
function loadBomTable(table, options={}) {
    ...
    if (options.editable) {
        ...
        cols.push({
            ...
            footerFormatter: function(data) {
                return `
                <button class='btn btn-success float-right' type='button' title='Dodaj część do BOM' id='bom-item-new-footer'>
                    ${makeIcon('fa-plus-circle')} Dodaj część do BOM
                </button>
                `;
            }
        });
