...
function partGroups() {
    ...
    return {
        attributes: {
            title: 'Attributi Articolo',
            collapsible: true,
        },
        create: {
            title: 'Opzioni Creazione Articolo',
            collapsible: true,
        },
        duplicate: {
            title: 'Opzioni Duplicazione Articolo',
            collapsible: true,
        },
        initial_stock: {
        ...
        initial_supplier: {
            title: 'Dati iniziali del fornitore',
            collapsible: true,
...
function partFields(options={}) {
    ...
    var fields = {
        category: {
            secondary: {
                title: 'Aggiungi Categoria Articolo',
                fields: function() {
...
function categoryFields(options={}) {
    let fields = {
        parent: {
            help_text: 'Categoria articolo principale',
            required: false,
        },
        name: {},
        description: {},
...
function createPartCategory(options={}) {
    ...
    constructForm('/api/part/category/', {
        ...
        follow: true,
        persist: true,
        persistMessage: 'Crea una nuvoa categoria dopo questa',
...
function editCategory(pk) {
    ...
    constructForm(url, {
        fields: fields,
        title: 'Modifica Categoria Articoli',
        reload: true,
    });
}
...
function deletePartCategory(pk, options={}) {
    ...
    Sei sicuro di voler eliminare questa categoria articolo?
    </div>`;
    var subChoices = [
        {
            value: 0,
            display_name: 'Sposta nella categoria superiore',
        },
        {
            value: 1,
    ...
    constructForm(url, {
        ...
        fields: {
            'delete_parts': {
                label: 'Azione articoli in questa categoria',
                choices: subChoices,
                type: 'choice'
            },
            'delete_child_categories': {
                label: 'Azione per categorie secondarie',
                choices: subChoices,
                type: 'choice'
            },
        },
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        ...
        title: 'Crea Articolo',
        persist: true,
        persistMessage: 'Crea un altro articolo dopo questo',
...
function editPart(pk) {
    ...
    constructForm(url, {
        ...
        reload: true,
        successMessage: 'Articolo modificato',
    });
}
...
function duplicatePart(pk, options={}) {
    ...
    var title = 'Duplica articolo';

    if (options.variant) {
        title = 'Crea Varianti Articolo';
    }
    ...
    inventreeGet(`/api/part/${pk}/`, {}, {
        success: function(part) {
            if (part.active) {
                showAlertDialog(
                    'Articolo Attivo',
                    'L\'articolo non può essere eliminato poiché è attualmente attivo',
                    {
                        alert_style: 'danger',
                    }
                );
            ...
            L\'eliminazione di questo articolo non è reversibile
            <ul>
            <li>Tutte le giacenze per questo articolo verranno eliminate</li>
            <li>Questo articolo verrà eliminato da qualsiasi Fattura dei Materiali</li>
            <li>Tutte le informazioni del produttore e del fornitore per questo articolo verranno eliminate</li>
            </div>`;
                ...
                },
                {
                    ...
                    success: function(response) {
                        if (response.starred) {
                            ...
                            $(options.button).attr('title', 'Sei iscritto alle notifiche per questo elemento');

                            showMessage('Hai sottoscritto le notifiche per questo elemento', {
                                style: 'success',
                            });
                        } else {
                            $(options.button).removeClass('fas fa-bell icon-green').addClass('fa fa-bell-slash');
                            $(options.button).attr('title', 'Sottoscrivi le notifiche per questo elemento');

                            showMessage('Hai annullato l\'iscrizione alle notifiche per questo elemento', {
                                style: 'warning',
                            });
                        }
                    }
...
function validateBom(part_id, options={}) {
    ...
    La convalida della Distinta Base segnerà ogni voce di riga come valida
    </div>
    `;
    ...
    constructForm(`/api/part/${part_id}/bom-validate/`, {
        ...
        preFormContent: html,
        title: 'Convalida la distinta dei materiali',
        reload: options.reload,
        onSuccess: function(response) {
            showMessage('Valida Fattura dei Materiali');
        }
    });
}
...
function duplicateBom(part_id, options={}) {
    ...
    constructForm(`/api/part/${part_id}/bom-copy/`, {
        ...
        confirm: true,
        title: 'Copia Fattura dei Materiali',
        onSuccess: function(response) {
            if (options.success) {
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            elements.push(`In esaurimento: ${part.total_in_stock}`);
        } else if (part.unallocated_stock <= 0) {
            // There is no available stock at all
            elements.push(`Nessuno stock disponibile`);
        } else if (part.unallocated_stock < part.in_stock) {
            // Unallocated quantity is less than total quantity
            if (options.hideTotalStock) {
                elements.push(`Disponibile: ${part.unallocated_stock}`);
            } else {
                elements.push(`Disponibile: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
        ...
        elements.push(`Nessuna giacenza`);
    }

    // Check for items on order
    if (part.ordering) {
        elements.push(`Ordinato: ${part.ordering}`);
    }

    // Check for items being built
    if (part.building) {
        elements.push(`In Costruzione: ${part.building}`);
    }
    ...
    if (options.showDemandInfo) {
        ...
        if (demand) {
            elements.push(`Richieste: ${demand}`);
        }
    ...
    if (units && !options.no_units) {
        output += `<span class='badge rounded-pill text-muted bg-muted ${classes}'>Unità: ${units}</span> `;
    }
...
function makePartIcons(part) {
    ...
    if (part.trackable) {
        html += makeIconBadge('fa-directions', 'Parte tracciabile');
    }

    if (part.virtual) {
        html += makeIconBadge('fa-ghost', 'Parte virtuale');
    }

    if (part.is_template) {
        html += makeIconBadge('fa-clone', 'Modello Articolo');
    }

    if (part.assembly) {
        html += makeIconBadge('fa-tools', 'Articolo assemblato');
    }

    if (part.starred) {
        html += makeIconBadge('fa-bell icon-green', 'Parte sottoscritta');
    }

    if (part.salable) {
        html += makeIconBadge('fa-dollar-sign', 'Parte vendibile');
    }

    if (!part.active) {
        html += `<span class='badge badge-right rounded-pill bg-warning'>Inattivo</span> `;
    }
...
function generateStocktakeReport(options={}) {
    ...
    Programmare la generazione di un nuovo report inventario. Una volta completato, il report inventario sarà disponibile per il download.
    </div>
    `;
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Genera Report Inventario',
            preFormContent: content,
            fields: fields,
            onSuccess: function(response) {
                showMessage('Programma report inventario', {
                    style: 'success',
                });
            }
        }
...
function loadPartStocktakeTable(partId, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nessuna informazione sull\'inventario disponibile';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.allow_edit) {
                        html += makeEditButton('button-edit-stocktake', row.pk, 'Modifica Voce Inventario');
                    }

                    if (options.allow_delete) {
                        html += makeDeleteButton('button-delete-stocktake', row.pk, 'Elimina Voce Inventario');
                    }
        ...
        onPostBody: function() {
            ...
            $(table).find('.button-edit-stocktake').click(function() {
                ...
                constructForm(`/api/part/stocktake/${pk}/`, {
                    ...
                    title: 'Modifica Voce Inventario',
                    refreshTable: table,
                });
            });
...
function loadPartVariantTable(table, partId, options={}) {
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.variant_stock > 0) {
                    text = `<em>${text}</em>`;
                    text += `<span title='Comprende varianti magazzino' class='fas fa-info-circle float-right icon-blue'></span>`;
                }
    ...
    table.inventreeTable({
        ...
        queryParams: filters,
        formatNoMatches: function() {
            return 'Nessuna variante trovata';
        },
        columns: cols,
        treeEnable: true,
        rootParentId: partId,
...
function createPartParameter(part_id, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Aggiungi parametro';
...
function editPartParameter(param_id, options={}) {
    options.fields = partParameterFields();
    options.title = 'Modifica parametro';
    options.focus = 'data';
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-parameter-edit', pk, 'Modifica parametro');
                    html += makeDeleteButton('button-parameter-delete', pk, 'Elimina il parametro');
...
function loadPartParameterTemplateTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nessun parametro dell\'articolo templates trovato';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    buttons += makeEditButton('template-edit', row.pk, 'Modifica Template');
                    buttons += makeDeleteButton('template-delete', row.pk, 'Elimina Template');
    ...
    $(table).on('click', '.template-edit', function() {
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                ...
                title: 'Modifica Parametro Articolo Template',
                refreshTable: table,
            }
        );
    });
    ...
    $(table).on('click', '.template-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
            Ogni parametro che fa riferimento a questo modello verrà eliminato
        </div>`;
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                method: 'DELETE',
                preFormContent: html,
                title: 'Elimina Parametro Articolo Template',
                refreshTable: table,
            }
        );
    });
...
function loadPartPurchaseOrderTable(table, part_id, options={}) {
    ...
    $(table).inventreeTable({
        ...
        uniqueId: 'pk',
        formatNoMatches: function() {
            return 'Nessun ordine d\'acquisto trovato';
        },
        onPostBody: function() {
        ...
        columns: [
            ...
            {
                field: 'mpn',
                title: 'Codice articolo produttore (MPN)',
                switchable: true,
                formatter: function(value, row) {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Quantità Confezione: ${row.pack_quantity} - Quantità Totale: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let total = value * row.supplier_part_detail.pack_quantity_native;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Quantità Confezione: ${row.pack_quantity} - Quantità Totale: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Ricevuto</span>`;
                    } else if (row.order_detail && row.order_detail.status == 20) {
                        let html = '';
                        var pk = row.pk;

                        html += makeIconButton('fa-sign-in-alt', 'button-line-receive', pk, 'Ricevi linea elemento');
...
function loadRelatedPartsTable(table, part_id, options={}) {
    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                let html = '';
                html += makeDeleteButton('button-related-delete', row.pk, 'Elimina relazione tra i componenti');
        ...
        onPostBody: function() {
            $(table).find('.button-related-delete').click(function() {
                ...
                constructForm(`/api/part/related/${pk}/`, {
                    method: 'DELETE',
                    title: 'Elimina Relazione Articolo',
                    refreshTable: table,
                });
            });
        },
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>Nessuna giacenza</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>Nessuna giacenza</span>`;
    }

    rows += `<tr><td><b>Magazzino</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>Ordinato</b></td><td>${part.ordering} ${units}</td></tr>`;
    }

    if (part.building) {
        rows += `<tr><td><b>In Costruzione</b></td><td>${part.building} ${units}</td></tr>`;
    }
...
function setPartCategory(data, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
        Imposta la categoria prodotto per i prodotti selezionati
    </div>
    `;

    constructForm('/api/part/change_category/',{
        title: 'Imposta categoria articolo',
        method: 'POST',
        preFormContent: html,
        fields: {
            category: {},
    ...
    columns.push({
        ...
        formatter: function(value, row) {
            if (row.category && row.category_detail) {
                ...
                return '<em>Nessuna categoria</em>';
            }
        }
    });
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                        ...
                        if (row.subRequested) {
                            ...
                            html += `
                                <a href='#' pk='${row.pk}' class='load-sub-category'>
                                    <span class='fas fa-sync-alt' title='Carica Sotto Categorie'></span>
                                </a> `;
                        }
                    }
                    ...
                    if (row.starred) {
                        html += makeIconBadge('fa-bell icon-green', 'Categoria sottoscritta');
                    }
...
function loadPartTestTemplateTable(table, options) {
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Nessun modello di test corrispondente';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part == part) {
                        ...
                        var text = 'Questo test è definito per un articolo principale';
        ...
        onPostBody: function() {
            ...
            table.find('.button-test-edit').click(function() {
                ...
                constructForm(url, {
                    ...
                    title: 'Modifica Modello Risultato Test',
                    onSuccess: function() {
                        table.bootstrapTable('refresh');
                    },
                });
            ...
            table.find('.button-test-delete').click(function() {
                ...
                constructForm(url, {
                    method: 'DELETE',
                    title: 'Elimina Modello Risultato Test',
                    refreshTable: table,
                });
            });
        }
...
function loadPartSchedulingChart(canvas_id, part_id) {
        ...
        {},
        {
            ...
            success: function(response) {
                ...
                for (var idx = 0; idx < response.length; idx++) {
                    ...
                    if (date == null) {
                        date_string = '<em>Nessuna data specificata</em>';
                        date_string += makeIconBadge('fa-exclamation-circle icon-red', 'Nessuna data specificata');
                    } else if (date < today) {
                        date_string += makeIconBadge('fa-exclamation-circle icon-yellow', 'La data specificata è nel passato');
                    }

                    var quantity_string = entry.quantity + entry.speculative_quantity;

                    if (entry.speculative_quantity != 0) {
                        quantity_string += makeIconBadge('fa-question-circle icon-blue', 'Speculativo');
                    }
    ...
    if (n_entries < 1) {
        ...
        var message = `
        <div class='alert alert-block alert-info'>
            Nessuna informazione di pianificazione disponibile per questo prodotto.
        </div>`;

        if (was_error) {
            message = `
                <div class='alert alert-block alert-danger'>
                    Errore nel recupero delle informazioni di programmazione per questo articolo.
                </div>
            `;
        }
    ...
    var data = {
        datasets: [
            {
                label: 'Quantità Di Scorte Programmate',
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Quantità: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
