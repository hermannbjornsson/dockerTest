...
function renderStockItem(data, parameters={}) {
    ...
    if (data.quantity == 0) {
        ...
        if (data.serial && data.quantity == 1) {
            stock_detail = `Sorozatszám: ${data.serial}`;
        } else {
            if (render_available_quantity) {
                var available = data.quantity - data.allocated;
                stock_detail = `Elérhető: ${available}`;
            } else {
                stock_detail = `Mennyiség: ${data.quantity}`;
            }
        }
...
function renderPart(data, parameters={}) {
    ...
    if (user_settings.PART_SHOW_QUANTITY_IN_FORMS) {
        ...
        if (!data.active) {
            labels += `<span class='badge badge-right rounded-pill bg-danger'>Inaktív</span>`;
        }
    }
    ...
    return renderModel(
        {
            ...
            textSecondary: `Szállítmány ${data.reference}`,
        },
        parameters
    );
}
