...
function makeBarcodeInput(placeholderText='', hintText='') {
    ...
    hintText = hintText || 'Introduzca datos de código de barras';
    ...
    <div class='form-group'>
        ...
        <div class='controls'>
            <div class='input-group'>
                ...
                <button title='Escanear código de barras usando webcam conectada' id='barcode_scan_btn' type='button' class='btn btn-secondary' onclick='onBarcodeScanClicked()' style='display: none;'>
                    ${makeIcon('fa-camera')}
                </button>
            </div>
...
function makeNotesField(options={}) {
    ...
    var tooltip = options.tooltip || 'Introduzca notas opcionales para la transferencia de stock';
    var placeholder = options.placeholder || 'Escribir notas';
...
function showInvalidResponseError(modal, response, status) {
    showBarcodeMessage(
        modal,
        `Respuesta del servidor inválida<br>Estado: '${status}'`
    );
}
...
function barcodeScanDialog(options={}) {
    ...
    let title = options.title || 'Escanear código de barras';
...
function unlinkBarcode(data, options={}) {
    ...
    var html = `<b>Desvincular Código de Barras</b><br>`;
...
function barcodeCheckInStockItems(location_id, options={}) {
    ...
    function reloadTable() {
        ...
        items.forEach(function(item) {
            ...
            <tr pk='${item.pk}'>
                ...
                <td>${location_info}</td>
                <td>${item.quantity}</td>
                <td>${makeRemoveButton('button-item-remove', item.pk, 'Eliminar artículo de stock')}</td>
            </tr>`;
        });
    ...
    barcodeDialog(
        'Scan Stock Items Into Location',
        {
            ...
            headerContent: table,
            preShow: function() {
                modalSetSubmitText(modal, 'Registrar');
                modalEnable(modal, false);
                reloadTable();
            },
            onShow: function() {
                    ...
                    inventreeGet(
                        ...
                        {},
                        {
                            success: function(stockitem) {
                                ...
                                if (duplicate) {
                                    showBarcodeMessage(modal, 'Artículo de stock ya escaneado', 'warning');
                                } else {

                                    if (stockitem.location == location_id) {
                                        showBarcodeMessage(modal, 'Artículo de stock ya está en esta ubicación');
                                        return;
                                    }
                                    ...
                                    showBarcodeMessage(modal, 'Artículo de stock añadido', 'success');
                ...
                if ('stocklocation' in response) {
                    ...
                    inventreeGet(`/api/stock/location/${pk}/`, {}, {
                        ...
                        error: function() {
                            ...
                            showBarcodeMessage(
                                modal,
                                'El código de barras no coincide con una ubicación válida',
                                'warning',
                            );
                        }
                    });
