...
function partGroups() {
    ...
    return {
        attributes: {
            title: 'Atributos de Parte',
            collapsible: true,
        },
        create: {
            title: 'Opciones de Creación de Parte',
            collapsible: true,
        },
        duplicate: {
            title: 'Opciones de Duplicación de Parte',
            collapsible: true,
        },
        initial_stock: {
            title: 'Initial Stock',
        ...
        initial_supplier: {
            title: 'Datos iniciales del proveedor',
            collapsible: true,
...
function partFields(options={}) {
    ...
    var fields = {
        category: {
            secondary: {
                title: 'Añadir Categoría de Parte',
                fields: function() {
...
function categoryFields(options={}) {
    let fields = {
        parent: {
            help_text: 'Categoría superior de parte',
            required: false,
        },
        name: {},
        description: {},
...
function createPartCategory(options={}) {
    ...
    constructForm('/api/part/category/', {
        fields: fields,
        method: 'POST',
        title: 'Crear Categoría de Parte',
        follow: true,
        persist: true,
        persistMessage: 'Create new category after this one',
        successMessage: 'Categoría de partes creada'
    });
}
...
function editCategory(pk) {
    ...
    constructForm(url, {
        fields: fields,
        title: 'Editar Categoría de Parte',
        reload: true,
    });
}
...
function deletePartCategory(pk, options={}) {
    ...
    var subChoices = [
        {
            value: 0,
            display_name: 'Mover a la categoría padre',
        },
        {
            value: 1,
    ...
    constructForm(url, {
        title: 'Eliminar Categoría de Parte',
        method: 'DELETE',
        fields: {
            'delete_parts': {
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        ...
        successMessage: 'Parte creada con éxito',
...
function duplicatePart(pk, options={}) {
    ...
    var title = 'Duplicar Parte';

    if (options.variant) {
        title = 'Crear Variante de Parte';
    }
                ...
                },
                {
                    ...
                    success: function(response) {
                        if (response.starred) {
                            ...
                            $(options.button).attr('title', 'Estás suscrito a las notificaciones de este artículo');

                            showMessage('Te has suscrito a las notificaciones de este artículo', {
                                style: 'success',
                            });
                        } else {
                            $(options.button).removeClass('fas fa-bell icon-green').addClass('fa fa-bell-slash');
                            $(options.button).attr('title', 'Suscríbete a las notificaciones de este artículo');

                            showMessage('Has cancelado la suscripción a las notificaciones de este artículo', {
                                style: 'warning',
                            });
                        }
                    }
...
function validateBom(part_id, options={}) {
    ...
    Validar el BOM marcará cada artículo de línea como válido
    </div>
    `;
    ...
    constructForm(`/api/part/${part_id}/bom-validate/`, {
        ...
        preFormContent: html,
        title: 'Validar la Factura de Materiales',
        reload: options.reload,
        onSuccess: function(response) {
            showMessage('Validación de Lista de Materiales');
        }
    });
}
...
function duplicateBom(part_id, options={}) {
    ...
    constructForm(`/api/part/${part_id}/bom-copy/`, {
        ...
        confirm: true,
        title: 'Copiar Factura de Materiales',
        onSuccess: function(response) {
            if (options.success) {
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            elements.push(`Stock bajo: ${part.total_in_stock}`);
        } else if (part.unallocated_stock <= 0) {
            // There is no available stock at all
            elements.push(`Existencias no disponibles`);
        } else if (part.unallocated_stock < part.in_stock) {
            // Unallocated quantity is less than total quantity
            if (options.hideTotalStock) {
                elements.push(`Disponible: ${part.unallocated_stock}`);
            } else {
                elements.push(`Disponible: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
        ...
        elements.push(`Sin Stock`);
    }

    // Check for items on order
    if (part.ordering) {
        elements.push(`En pedido: ${part.ordering}`);
    }

    // Check for items being built
    if (part.building) {
        elements.push(`En construcción: ${part.building}`);
    }
    ...
    if (units && !options.no_units) {
        output += `<span class='badge rounded-pill text-muted bg-muted ${classes}'>Unidad: ${units}</span> `;
    }
...
function makePartIcons(part) {
    ...
    if (part.trackable) {
        html += makeIconBadge('fa-directions', 'Parte Rastreable');
    }

    if (part.virtual) {
        html += makeIconBadge('fa-ghost', 'Parte virtual');
    }
    ...
    if (part.starred) {
        html += makeIconBadge('fa-bell icon-green', 'Parte suscrita');
    }

    if (part.salable) {
        html += makeIconBadge('fa-dollar-sign', 'Parte vendible');
    }
...
function loadPartVariantTable(table, partId, options={}) {
    ...
    table.inventreeTable({
        ...
        queryParams: filters,
        formatNoMatches: function() {
            return 'No se encontraron variantes';
        },
        columns: cols,
        treeEnable: true,
        rootParentId: partId,
...
function createPartParameter(part_id, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Añadir parámetro';
...
function editPartParameter(param_id, options={}) {
    options.fields = partParameterFields();
    options.title = 'Editar parámetro';
    options.focus = 'data';
...
function loadPartParameterTemplateTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'No se encontraron plantillas de parámetros de parte';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    buttons += makeEditButton('template-edit', row.pk, 'Editar Plantilla');
                    buttons += makeDeleteButton('template-delete', row.pk, 'Eliminar Plantilla');
    ...
    $(table).on('click', '.template-edit', function() {
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                ...
                title: 'Crear plantilla Parámetro de Parte',
                refreshTable: table,
            }
        );
    });
    ...
    $(table).on('click', '.template-delete', function() {
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                method: 'DELETE',
                preFormContent: html,
                title: 'Eliminar Plantilla de Parámetros de Parte',
                refreshTable: table,
            }
        );
    });
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Cantidad de paquete: ${row.pack_quantity} - Cantidad Total: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let total = value * row.supplier_part_detail.pack_quantity_native;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Cantidad de paquete: ${row.pack_quantity} - Cantidad Total: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Recibido</span>`;
                    } else if (row.order_detail && row.order_detail.status == 20) {
                        let html = '';
                        var pk = row.pk;

                        html += makeIconButton('fa-sign-in-alt', 'button-line-receive', pk, 'Recibir ítem de línea');
...
function loadRelatedPartsTable(table, part_id, options={}) {
    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                let html = '';
                html += makeDeleteButton('button-related-delete', row.pk, 'Eliminar relación de parte');
        ...
        onPostBody: function() {
            $(table).find('.button-related-delete').click(function() {
                ...
                constructForm(`/api/part/related/${pk}/`, {
                    method: 'DELETE',
                    title: 'Eliminar Relación de Parte',
                    refreshTable: table,
                });
            });
        },
...
function loadParametricPartTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'No se encontraron partes';
        },
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>Sin Stock</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>Sin Stock</span>`;
    }

    rows += `<tr><td><b>Inventario</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>En pedido</b></td><td>${part.ordering} ${units}</td></tr>`;
    }

    if (part.building) {
        rows += `<tr><td><b>En construcción</b></td><td>${part.building} ${units}</td></tr>`;
    }
...
function setPartCategory(data, options={}) {
    ...
    constructForm('/api/part/change_category/',{
        title: 'Definir Categoría de Parte',
        method: 'POST',
        preFormContent: html,
        fields: {
            category: {},
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                        ...
                        if (row.subRequested) {
                            ...
                            html += `
                                <a href='#' pk='${row.pk}' class='load-sub-category'>
                                    <span class='fas fa-sync-alt' title='Cargar subcategorías'></span>
                                </a> `;
                        }
                    }
                    ...
                    if (row.starred) {
                        html += makeIconBadge('fa-bell icon-green', 'Categoría suscrita');
                    }
...
function loadPartTestTemplateTable(table, options) {
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'No hay plantillas de prueba que coincidan con la consulta';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part == part) {
                        ...
                        var text = 'Esta prueba está definida para una parte principal';
        ...
        onPostBody: function() {
            ...
            table.find('.button-test-edit').click(function() {
                ...
                constructForm(url, {
                    ...
                    title: 'Editar plantilla de resultado de prueba',
                    onSuccess: function() {
                        table.bootstrapTable('refresh');
                    },
                });
            ...
            table.find('.button-test-delete').click(function() {
                ...
                constructForm(url, {
                    method: 'DELETE',
                    title: 'Eliminar plantilla de resultados de prueba',
                    refreshTable: table,
                });
            });
        }
        ...
        {},
        {
            ...
            success: function(response) {
                ...
                for (var idx = 0; idx < response.length; idx++) {
                    ...
                    if (date == null) {
                        date_string = '<em>Sin fecha especificada</em>';
                        date_string += makeIconBadge('fa-exclamation-circle icon-red', 'Sin fecha especificada');
                    } else if (date < today) {
                    ...
                    if (entry.speculative_quantity != 0) {
                        quantity_string += makeIconBadge('fa-question-circle icon-blue', 'Especulativo');
                    }
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Cantidad: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
