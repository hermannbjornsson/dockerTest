...
function createManufacturerPart(options={}) {
    ...
    constructForm('/api/company/part/manufacturer/', {
        fields: fields,
        method: 'POST',
        title: 'Herstellerteil hinzufügen',
    ...
    inventreeGet(`/api/company/part/${part}/`, {}, {
        success: function(data) {
            ...
            constructForm('/api/company/part/', {
                method: 'POST',
                fields: fields,
                title: 'Zulieferer-Teil duplizieren',
                data: data,
...
function editSupplierPart(part, options={}) {
    ...
    constructForm(`/api/company/part/${part}/`, {
        fields: fields,
        title: options.title || 'Zuliefererteil bearbeiten',
        onSuccess: options.onSuccess
    });
}
...
function deleteSupplierParts(parts, options={}) {
    ...
    Alle ausgewählten Zulieferteile werden gelöscht
    </div>
    ...
    constructForm('/api/company/part/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Zuliefererteil entfernen',
        preFormContent: html,
        form_data: {
            items: ids,
        },
...
function loadCompanyTable(table, url, options={}) {
    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.is_customer) {
                    html += `<span title='Kunde' class='fas fa-user-tie float-right'></span>`;
                }

                if (row.is_manufacturer) {
                    html += `<span title='Hersteller' class='fas fa-industry float-right'></span>`;
                }

                if (row.is_supplier) {
                    html += `<span title='Zulieferer' class='fas fa-building float-right'></span>`;
                }
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine Firmeninformation gefunden';
        },
        showColumns: true,
...
function deleteManufacturerParts(selections, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Alle ausgewählten Herstellerrteile werden gelöscht
    </div>
    <table class='table table-striped table-condensed'>
    <tr>
        <th>Teil</th>
...
function deleteManufacturerPartParameters(selections, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Alle ausgewählten Parameter werden gelöscht
    </div>
    <table class='table table-striped table-condensed'>
    <tr>
        <th>Name</th>
...
function loadManufacturerPartTable(table, url, options) {
    ...
    $(table).inventreeTable({
        ...
        groupBy: false,
        formatNoMatches: function() {
            return 'Keine Herstellerteile gefunden';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-manufacturer-part-edit', pk, 'Herstellerteil bearbeiten');
                    html += makeDeleteButton('button-manufacturer-part-delete', pk, 'Herstellerteil löschen');
...
function loadSupplierPartTable(table, url, options) {
    ...
    setupFilterList('supplierpart', $(table), '#filter-list-supplier-part', {
        custom_actions: [
            {
                label: 'supplier-part',
                title: 'Zulieferer-Teil Aktionen',
                icon: 'fa-tools',
    ...
    $(table).inventreeTable({
        ...
        sortable: true,
        formatNoMatches: function() {
            return 'Keine Zuliefererteile gefunden';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-supplier-part-edit', pk, 'Zuliefererteil bearbeiten');
                    html += makeDeleteButton('button-supplier-part-delete', pk, 'Zuliefererteil entfernen');
...
function loadSupplierPriceBreakTable(options={}) {
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine Informationen zur Preisstaffel gefunden';
        },
        onPostBody: function() {
            setupCallbacks();
        },
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    buttons += makeEditButton('button-price-break-edit', row.pk, 'Preisstaffel bearbeiten');
                    buttons += makeDeleteButton('button-price-break-delete', row.pk, 'Preisstaffel löschen');
