...
function selectLabel(labels, items, options={}) {
        ...
        <div class='form-group'>
            ...
            <div class='controls'>
                <select id='id_plugin' class='select form-control' name='plugin'>
                    <option value='' title='Eksporter til PDF'>Eksporter til PDF</option>
        `;
    ...
    modalSetTitle(modal, 'Velg etikettmal');
...
function printLabels(options) {
    ...
    if (!options.items || options.items.length == 0) {
        showAlertDialog(
            'Velg artikler',
            'Ingen artikler valgt for utskrift',
        );
        return;
    }
            ...
            selectLabel(response, options.items, {
                success: function(data) {
                    ...
                    if (data.plugin) {
                        ...
                        inventreeGet(href, {}, {
                            success: function(response) {
                                showMessage('Labels sent to printer', {
                                    style: 'success',
                                });
                            }
                        });
