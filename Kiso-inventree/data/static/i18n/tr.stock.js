...
function editStockLocation(pk, options={}) {
    ...
    options.title = 'Stok konumunu düzenle';
...
function deleteStockLocation(pk, options={}) {
    ...
    Bu stok konumunu silmek istediğinizden emin misiniz?
    </div>
    `;
...
function createNewStockItem(options={}) {
    ...
    options.successMessage = 'Stok kalemi oluşturuldu';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                var details = `
                <br>Miktar: ${response.quantity}
                <br>Seri Numaraları: ${response.serial_numbers}
                `;
...
function assignStockToCustomer(items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.serial && item.quantity == 1) {
            quantity = `Seri No: ${item.serial}`;
        } else {
            quantity = `Miktar: ${item.quantity}`;
        }
...
function adjustStock(action, items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantity += ` - <small>Toplu: ${item.batch}</small>`;
        }
    ...
    constructForm(url, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Stok ayarlamasını onayla',
        title: formTitle,
        afterRender: function(fields, opts) {
            // Add button callbacks to remove rows
            $(opts.modal).find('.button-stock-item-remove').click(function() {
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        ...
        text = '<i>Stok konumu ayarlanmadı</i>';
        url = '';
    }
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Stok parçalarını birleştir', {
                            style: 'success',
                        });
                    }
                });
