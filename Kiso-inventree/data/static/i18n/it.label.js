...
function selectLabel(labels, items, options={}) {
        ...
        <div class='form-group'>
            ...
            <div class='controls'>
                <select id='id_plugin' class='select form-control' name='plugin'>
                    <option value='' title='Esporta in PDF'>Esporta in PDF</option>
        `;
    ...
    if (items.length > 0) {
        ...
        ${items.length} ${item_name} selected
        </div>`;
    }
    ...
    modalSetTitle(modal, 'Seleziona Modello Etichetta');
        ...
        success: function(response) {
            if (response.length == 0) {
                showAlertDialog(
                    ...
                    'No label templates found which match the selected items',
                );
                return;
            }
            ...
            selectLabel(response, options.items, {
                success: function(data) {
                    ...
                    if (data.plugin) {
                        ...
                        inventreeGet(href, {}, {
                            success: function(response) {
                                showMessage('Etichette inviate alla stampante', {
                                    style: 'success',
                                });
                            }
                        });
