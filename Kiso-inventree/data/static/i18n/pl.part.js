...
function createPartCategory(options={}) {
    ...
    constructForm('/api/part/category/', {
        fields: fields,
        method: 'POST',
        title: 'Utwórz nową kategorię części',
        follow: true,
        persist: true,
...
function deletePartCategory(pk, options={}) {
    ...
    Czy na pewno chcesz usunąć tę kategorię części?
    </div>`;
    var subChoices = [
        {
            value: 0,
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        ...
        title: 'Utwórz część',
        persist: true,
        persistMessage: 'Utwórz kolejną część po tej',
        successMessage: 'Część utworzona pomyślnie',
...
function editPart(pk) {
    ...
    constructForm(url, {
        ...
        title: 'Edytuj część',
        reload: true,
        successMessage: 'Część zmodyfikowana',
    });
}
...
function duplicatePart(pk, options={}) {
    ...
    var title = 'Duplikuj część';

    if (options.variant) {
        title = 'Utwórz wariant części';
    }
                ...
                },
                {
                    ...
                    success: function(response) {
                        if (response.starred) {
                            ...
                            $(options.button).attr('title', 'Masz włączone powiadomienia dla tej części');

                            showMessage('Masz włączone powiadomienia dla tej części', {
                                style: 'success',
                            });
                        } else {
                            $(options.button).removeClass('fas fa-bell icon-green').addClass('fa fa-bell-slash');
                            $(options.button).attr('title', 'Włącz powiadomienia dla tej części');

                            showMessage('Zostałeś wypisany z powiadomień dla tej części', {
                                style: 'warning',
                            });
                        }
                    }
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            ...
            if (options.hideTotalStock) {
                elements.push(`Dostępne: ${part.unallocated_stock}`);
            } else {
                elements.push(`Dostępne: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
        ...
        elements.push(`Brak w magazynie`);
    }

    // Check for items on order
    if (part.ordering) {
        elements.push(`W Zamówieniu: ${part.ordering}`);
    }
...
function makePartIcons(part) {
    ...
    if (part.starred) {
        html += makeIconBadge('fa-bell icon-green', 'Obserwowane części');
    }
    ...
    if (!part.active) {
        html += `<span class='badge badge-right rounded-pill bg-warning'>Nieaktywny</span> `;
    }
...
function loadPartVariantTable(table, partId, options={}) {
    ...
    table.inventreeTable({
        ...
        queryParams: filters,
        formatNoMatches: function() {
            return 'Nie znaleziono wariantów';
        },
        columns: cols,
        treeEnable: true,
        rootParentId: partId,
...
function loadPartParameterTable(table, options) {
    ...
    $(table).inventreeTable({
        ...
        groupBy: false,
        formatNoMatches: function() {
            return 'Nie znaleziono parametrów';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-parameter-edit', pk, 'Edytuj Parametr');
                    html += makeDeleteButton('button-parameter-delete', pk, 'Usuń parametr');
...
function loadPartParameterTemplateTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nie znaleziono szablonów parametrów części';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    buttons += makeEditButton('template-edit', row.pk, 'Edytuj szablon');
                    buttons += makeDeleteButton('template-delete', row.pk, 'Usuń szablon');
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Odebrane</span>`;
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>Brak w magazynie</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>Brak w magazynie</span>`;
    }

    rows += `<tr><td><b>Stan</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>W Zamówieniu</b></td><td>${part.ordering} ${units}</td></tr>`;
    }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.starred) {
                        html += makeIconBadge('fa-bell icon-green', 'Obserwowana kategoria');
                    }
