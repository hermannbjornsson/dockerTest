...
function constructBomUploadTable(data, options={}) {
    ...
    function constructRow(row, idx, fields) {
        ...
        buttons += makeInfoButton('button-row-data', idx, 'Vis raddata');
        buttons += makeRemoveButton('button-row-remove', idx, 'Fjern rad');
...
function downloadBomTemplate(options={}) {
    ...
    constructFormBody({}, {
        title: 'Lat ned BOM-mal',
        fields: {
            format: {
                label: 'Format',
                help_text: 'Velg filformat',
                required: true,
                type: 'choice',
                value: format,
...
function exportBom(part_id, options={}) {
    ...
    constructFormBody({}, {
        ...
        fields: {
            ...
            cascade: {
                label: 'Flernivå-BOM',
                help_text: 'Inkluder BOM-data for undersammenstillinger',
                type: 'boolean',
                value: inventreeLoad('bom-export-cascading', true),
            },
            levels: {
                label: 'Nivåer',
                help_text: 'Velg maksimalt antall BOM-nivåer å eksportere (0 = alle nivåer)',
                type: 'integer',
                value: 0,
                required: true,
                min_value: 0,
            ...
            substitute_part_data: {
                label: 'Inkluder alternative deler',
                help_text: 'Inkluder alternative deler i eksportert BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-substitute_part_data', false),
            },
            parameter_data: {
                label: 'Inkluder parameterdata',
                help_text: 'Inkluder delparameterdata i eksportert BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-parameter_data', false),
            },
            stock_data: {
                label: 'Inkluder lagerbeholdningsdata',
                help_text: 'Inkluder delbeholdningsdata i eksportert BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-stock_data', false),
            },
            manufacturer_data: {
                label: 'Inkluder Produsentdata',
                help_text: 'Inkluder delprodusentdata i eksportert BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-manufacturer_data', false),
            },
            supplier_data: {
                label: 'Inkluder Leverandørdata',
                help_text: 'Inkluder delleverandørdata i eksportert BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-supplier_data', false),
            },
            pricing_data: {
                label: 'Inkluder prisdata',
                help_text: 'Inkluder delprisdata i eksportert BOM',
                type: 'boolean',
...
function bomSubstitutesDialog(bom_item_id, substitutes, options={}) {
    ...
    function renderSubstituteRow(substitute) {
        ...
        buttons += makeRemoveButton('button-row-remove', pk, 'Fjern erstatningsdel');
    ...
    <div class='alert alert-success alert-block'>
        Velg og legg til en ny erstatningsdel ved hjelp av inntastingen under
    </div>
    `;
    ...
    function addRemoveCallback(modal, element) {
        $(modal).find(element).click(function() {
            ...
            Er du sikker på at du vil fjerne koblingen til denne erstatningsdelen?
            </div>
            `;

            constructForm(`/api/bom/substitute/${pk}/`, {
                method: 'DELETE',
                title: 'Fjern Erstatningsdel',
                preFormContent: pre,
                confirm: true,
                onSuccess: function() {
    ...
    constructForm('/api/bom/substitute/', {
        ...
        preFormContent: html,
        closeText: 'Lukk',
        submitText: 'Legg til Erstatning',
        title: 'Rediger BOM-artikkelerstatninger',
...
function deleteBomItems(items, options={}) {
    ...
    Alle valgte BOM-artikler vil bli slettet
    </div>
    ...
    constructForm('/api/bom/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Slett valgte BOM-artikler?',
        form_data: {
            items: ids,
        },
        preFormContent: html,
...
function loadBomTable(table, options={}) {
    ...
    cols.push(
        {
            ...
            formatter: function(value, row) {
                    ...
                    if (row.sub_assembly_received) {
                        ...
                        html += `
                            <a href='#' pk='${row.pk}' class='load-sub-assembly' id='load-sub-assembly-${row.pk}'>
                                <span class='fas fa-sync-alt' title='Last inn BOM for undersammenstillinger'></span>
                            </a> `;
                    }
                }
                ...
                if (row.substitutes && row.substitutes.length > 0) {
                    html += makeIconBadge('fa-exchange-alt', 'Erstatninger tilgjengelig');
                }

                if (row.allow_variants) {
                    html += makeIconBadge('fa-sitemap', 'Variantbeholdning tillatt');
                }
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (row.consumable) {
                text += ` <small>(Forbruksvare)</small>`;
            }

            if (row.optional) {
                text += ' <small>(Valgfritt)</small>';
            }
            ...
            if (any_pricing) {
                ...
                if (complete_pricing) {
                    html += makeIconBadge(
                        ...
                        'BOM-prising er komplett',
                    );
                } else {
                    html += makeIconBadge(
                        'fa-exclamation-circle icon-yellow',
                        'BOM-prising er ufullstendig',
                    );
                }
                ...
                let html = '<em>Ingen prising tilgjengelig</em>';
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (available_stock <= 0) {
                text += makeIconBadge('fa-times-circle icon-red', 'Ingen lagerbeholdning tilgjengelig');
            } else {
                var extra = '';

                if ((substitute_stock > 0) && (variant_stock > 0)) {
                    extra = 'Inkluderer variant- og erstatningsbeholdning';
                } else if (variant_stock > 0) {
                    extra = 'Inkluderer variantbeholdning';
                } else if (substitute_stock > 0) {
                    extra = 'Inkluderer erstatningsbeholdning';
                }
        ...
        cols.push({
            ...
            formatter: function(value, row) {
                ...
                if (row.part == options.parent_id) {
                    ...
                    var bValidate = makeIconButton('fa-check-circle icon-green', 'bom-validate-button', row.pk, 'Godkjenn BOM-artikkel');

                    var bValid = makeIconButton('fa-check-double icon-green', 'bom-valid-button', row.pk, 'Denne linjen er godkjent', {disabled: true});

                    var bSubs = makeIconButton('fa-exchange-alt icon-blue', 'bom-substitutes-button', row.pk, 'Rediger erstatningsdeler');

                    var bEdit = makeEditButton('bom-edit-button', row.pk, 'Rediger BOM-artikkel');

                    var bDelt = makeDeleteButton('bom-delete-button', row.pk, 'Slett BOM-artikkel');
            ...
            footerFormatter: function(data) {
                return `
                <button class='btn btn-success float-right' type='button' title='Legg til BOM-artikkel' id='bom-item-new-footer'>
                    ${makeIcon('fa-plus-circle')} Legg til BOM-artikkel
                </button>
                `;
            }
        });
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Ingen BOM-artikler funnet';
        },
        queryParams: filters,
        original: params,
        columns: cols,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.parent && row.parent != 'top-level-item') {
                        html += ` <em>(Arvet fra overordnet stykkliste)</em>`;
                    }
