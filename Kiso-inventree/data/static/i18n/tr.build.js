...
function completeBuildOrder(build_id, options={}) {
    ...
    constructForm(`/api/build/${build_id}/finish/`, {
        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    html += `
                    <div class='alert alert-block alert-danger'>
                    <strong>Yapım işi emri eksik</strong>
                    </div>
                    `;
                }

                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>Gerekli stok tamamen tahsis edilemedi</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>Gerekli yapım işi miktarı tamamlanmadı</div>`;
                }
            }
        ...
        reload: true,
        confirm: true,
        title: 'Tamamlanmış Yapım İşi Emri',
        method: 'POST',
    });
}
        ...
        {},
        {
            success: function(build) {
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Bu Malzeme Listesi takip edilebilir parçalar içeriyor.<br>
                ...
                if (trackable) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Takip edilebilir parçaların seri numaraları belirtilmiş olmalı<br>
                        Birden çok tek yapım işi çıktısı oluşturmak için seri numaraları girin
                    </div>
                    `;
                }
                ...
                constructForm(`/api/build/${build_id}/create-output/`, {
                    method: 'POST',
                    title: 'Yapım İşi Çıktısı Oluştur',
                    confirm: true,
                    fields: fields,
                    preFormContent: html,
                    onSuccess: function(response) {
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Seri Numara: ${output.serial}`;
    } else {
        output_html += `Miktar: ${output.quantity}`;
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        text = `Seri Numara: ${row.serial}`;
                    } else {
                        text = `Miktar: ${row.quantity}`;
                    ...
                    if (row.batch) {
                        text += ` <small>(Toplu: ${row.batch})</small>`;
                    }
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Parça</th>
                <th>Allocated</th>
                <th style='min-width: 250px;'>Stok Kalemi</th>
                <th>Miktar</th>
                <th></th>
            </tr>
        </thead>
    ...
    sub_table.bootstrapTable({
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    let buttons = '';
                    buttons += makeEditButton('button-allocation-edit', row.pk, 'Stok tahsisini düzenle');
                    buttons += makeDeleteButton('button-allocation-delete', row.pk, 'Stok tahsisini sil');
