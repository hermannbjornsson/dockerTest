...
function purchaseOrderFields(options={}) {
    ...
    if (options.duplicate_order) {
        fields.duplicate_order = {
            ...
            label: 'Bestellung',
            help_text: 'Bestellung zum Duplizieren auswählen',
        };
        ...
        fields.duplicate_line_items = {
            ...
            group: 'duplicate',
            type: 'boolean',
            label: 'Positionen duplizieren',
            help_text: 'Alle Positionen der ausgewählten Bestellung duplizieren',
        };
        ...
        fields.duplicate_extra_lines = {
            ...
            group: 'duplicate',
            type: 'boolean',
            label: 'Zusätzliche Zeilen duplizieren',
            help_text: 'Zusätzliche Positionen der ausgewählten Bestellung duplizieren',
        };
    }
...
function createPurchaseOrder(options={}) {
    ...
    constructForm('/api/order/po/', {
        ...
        title: options.title || 'Bestellung anlegen',
    });
}
        ...
        part: {
            ...
            onEdit: function(value, name, field, opts) {
                ...
                inventreeGet(`/api/company/part/${value}/`,
                    ...
                    if (pack_quantity != 1) {
                        var txt = `<span class='fas fa-info-circle icon-blue'></span> Packmenge: ${formatDecimal(pack_quantity)} ${units}`;
            ...
            secondary: {
                method: 'POST',
                title: 'Zuliefererteil hinzufügen',
...
function completePurchaseOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Bestellung vervollständigen',
            confirm: true,
            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.is_complete) {
                    ...
                    <div class='alert alert-block alert-success'>
                        Alle Einträge wurden erhalten
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Diese Bestellung enthält Positionen, die nicht als empfangen markiert wurden.</br>
        ...
        `/api/order/po/${order_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-info alert-block'>
                    Sind Sie sicher, dass Sie diese Bestellung abbrechen möchten?
                </div>`;
...
function orderParts(parts_list, options={}) {
    ...
    if (parts.length == 0) {
        showAlertDialog(
            'Teile auswählen',
            'Mindestens ein kaufbares Teil muss ausgewählt werden',
        );
        return;
    }
    ...
    function renderPart(part, opts={}) {
        ...
        var supplier_part_prefix = `
            <button type='button' class='input-group-text button-row-new-sp' pk='${pk}' title='Neues Zuliefererteil'>
        ...
        var purchase_order_prefix = `
            <button type='button' class='input-group-text button-row-new-po' pk='${pk}' title='Neue Bestellung'>
                <span class='fas fa-plus-circle icon-green'></span>
            </button>
        `;
    ...
    <table class='table table-striped table-condensed' id='order-parts-table'>
        <thead>
            <tr>
                <th>Teil</th>
                <th style='min-width: 300px;'>Zuliefererteil</th>
                <th style='min-width: 300px;'>Bestellung</th>
                <th style='min-width: 50px;'>Anzahl</th>
                <th><!-- Actions --></th>
            </tr>
        </thead>
        <tbody>
            ...
            parts.forEach(function(part) {
                ...
                var onSupplierPartChanged = function(value, name, field, opts) {
                    ...
                    if (value != null) {
                        inventreeGet(
                            ...
                            if (pack_quantity != 1) {
                                var txt = `<span class='fas fa-info-circle icon-blue'></span> Packmenge: ${pack_quantity} ${units}`;
                ...
                var supplier_part_field = {
                    ...
                    noResults: function(query) {
                        return 'Keine passenden Lieferantenteile';
                    }
                };
                ...
                initializeRelatedField({
                    ...
                    noResults: function(query) {
                        return 'Keine passenden Bestellungen';
                    }
                }, null, opts);
...
function receivePurchaseOrderItems(order_id, line_items, options={}) {
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        if (native_pack_quantity != 1) {
            ...
            <div class='alert alert-small alert-block alert-info'>
                Packmenge: ${pack_quantity}<br>
                Gelieferte Menge: <span class='pack_received_quantity' id='items_received_quantity_${pk}'>${received}</span> ${units}
            </div>`;
        }
        ...
        var batch_input = constructField(
            `items_batch_code_${pk}`,
            {
                ...
                required: false,
                label: 'Losnummer',
                help_text: 'Losnummer für eingehende Lagerartikel',
                icon: 'fa-layer-group',
            },
            {
                hideLabels: true,
        ...
        var sn_input = constructField(
            `items_serial_numbers_${pk}`,
            {
                ...
                required: false,
                label: 'Seriennummer',
                help_text: 'Seriennummern für eingehende Lagerartikel',
                icon: 'fa-hashtag',
            },
            {
                hideLabels: true,
    ...
    <table class='table table-condensed' id='order-receive-table'>
        <thead>
            <tr>
                ...
                <th>Bestellnummer</th>
                <th>Empfangen</th>
                <th style='min-width: 50px;'>Zu erhaltende Menge</th>
    ...
    constructForm(`/api/order/po/${order_id}/receive/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Empfang der Teile bestätigen',
        title: 'Bestellpositionen erhalten',
        afterRender: function(fields, opts) {
                ...
                $(opts.modal).find('.button-row-add-barcode').click(function() {
                    ...
                    barcodeDialog('Scan Item Barcode', {
                        ...
                        onScan: function(response, barcode_options) {
                            ...
                            showBarcodeMessage(barcode_options.modal, 'Barcode entspricht einem bereits vorhandenen Artikel');
                        },
...
function deletePurchaseOrderLineItems(items, options={}) {
    ...
    Alle ausgewählten Positionen werden gelöscht
    </div>
    ...
    <table class='table table-striped table-condensed'>
        <tr>
            <th>Teil</th>
            <th>Beschreibung</th>
            <th>SKU (Lagerbestandseinheit)</th>
            <th>MPN</th>
            <th>Anzahl</th>
        </tr>
        ${rows}
    ...
    constructForm('/api/order/po-line/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Ausgewählte Positionen löschen?',
        form_data: {
            items: ids,
        },
        preFormContent: html,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Packmenge: ${pack_quantity} - Gesamtstückzahl: ${total}${units}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.target_date) {
                        ...
                        if (row.overdue) {
                            html += makeIconBadge('fa-calendar-times icon-red', 'Diese Position ist überfällig');
                        }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (options.allow_receive && row.received < row.quantity) {
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-line-receive', pk, 'Position empfangen');
                    }

                    if (options.allow_edit) {
                        buttons += makeCopyButton('button-line-duplicate', pk, 'Position duplizieren');
                        buttons += makeEditButton('button-line-edit', pk, 'Position bearbeiten');
                        buttons += makeDeleteButton('button-line-delete', pk, 'Position löschen');
                    }
