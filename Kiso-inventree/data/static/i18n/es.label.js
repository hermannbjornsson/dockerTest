...
function selectLabel(labels, items, options={}) {
        ...
        <div class='form-group'>
            ...
            <div class='controls'>
                <select id='id_plugin' class='select form-control' name='plugin'>
                    <option value='' title='Exportar a PDF'>Exportar a PDF</option>
        `;
    ...
    if (items.length > 0) {
        ...
        ${items.length} ${item_name} seleccionado
        </div>`;
    }
    ...
    <form method='post' action='' class='js-modal-form' enctype='multipart/form-data'>
        <div class='form-group'>
            ...
            Seleccione Plantilla de Etiqueta
            </label>
            <div class='controls'>
    ...
    modalSetTitle(modal, 'Seleccione Plantilla de Etiqueta');
...
function printLabels(options) {
    ...
    if (!options.items || options.items.length == 0) {
        showAlertDialog(
            ...
            'No hay artículos seleccionados para imprimir',
        );
        return;
    }
        ...
        success: function(response) {
            if (response.length == 0) {
                showAlertDialog(
                    ...
                    'No se encontraron plantillas de etiqueta que coincidan con los artículos seleccionados',
                );
                return;
            }
            ...
            selectLabel(response, options.items, {
                success: function(data) {
                    ...
                    if (data.plugin) {
                        ...
                        inventreeGet(href, {}, {
                            success: function(response) {
                                showMessage('Etiquetas enviadas a la impresora', {
                                    style: 'success',
                                });
                            }
                        });
