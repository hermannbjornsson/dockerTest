...
function constructBomUploadTable(data, options={}) {
    ...
    function constructRow(row, idx, fields) {
        ...
        buttons += makeRemoveButton('button-row-remove', idx, 'Удалить строку');
...
function bomSubstitutesDialog(bom_item_id, substitutes, options={}) {
    ...
    var html = `
    <div class='alert alert-block'>
    <strong>Базовая деталь</strong><hr>
...
function loadBomTable(table, options={}) {
        ...
        cols.push({
            ...
            formatter: function(value, row) {
                ...
                if (row.part == options.parent_id) {
                    ...
                    var bEdit = makeEditButton('bom-edit-button', row.pk, 'Редактировать элемент BOM');

                    var bDelt = makeDeleteButton('bom-delete-button', row.pk, 'Удалить элемент BOM');
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Элементы BOM не найдены';
        },
        queryParams: filters,
        original: params,
        columns: cols,
    ...
    if (options.editable) {
        ...
        table.on('click', '.bom-edit-button', function() {
            ...
            constructForm(`/api/bom/${pk}/`, {
                fields: fields,
                title: 'Редактировать элемент BOM',
                focus: 'sub_part',
                onSuccess: function() {
                    reloadBomTable(table);
                }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.parent && row.parent != 'top-level-item') {
                        html += ` <em>(Унаследовано от родительского BOM)</em>`;
                    }
