...
function renderStockItem(data, parameters={}) {
    ...
    if (data.quantity == 0) {
        ...
        if (data.serial && data.quantity == 1) {
            stock_detail = `Número de serie: ${data.serial}`;
        } else {
            if (render_available_quantity) {
                var available = data.quantity - data.allocated;
                stock_detail = `Disponible: ${available}`;
            } else {
                stock_detail = `Cantidad: ${data.quantity}`;
            }
        }
...
function renderPart(data, parameters={}) {
    ...
    if (user_settings.PART_SHOW_QUANTITY_IN_FORMS) {
        ...
        if (!data.active) {
            labels += `<span class='badge badge-right rounded-pill bg-danger'>Inactivo</span>`;
        }
    }
    ...
    return renderModel(
        {
            ...
            textSecondary: `Envío ${data.reference}`,
        },
        parameters
    );
}
