    ...
    getApiEndpointOptions(url, function(OPTIONS) {
        ...
        case 'POST':
            if (canCreate(OPTIONS)) {
                ...
                showMessage('Művelet tiltva', {
                    style: 'danger',
                    details: 'Létrehozás nem engedélyezett',
        ...
        case 'PATCH':
            if (canChange(OPTIONS)) {
                ...
                showMessage('Művelet tiltva', {
                    style: 'danger',
                    details: 'Módosítás nem engedélyezett',
                    icon: 'fas fa-user-times',
                });
        ...
        case 'DELETE':
            if (canDelete(OPTIONS)) {
                ...
                showMessage('Művelet tiltva', {
                    style: 'danger',
                    details: 'Törlés nem engedélyezett',
                    icon: 'fas fa-user-times',
                });
        ...
        case 'GET':
            if (canView(OPTIONS)) {
                ...
                showMessage('Művelet tiltva', {
                    style: 'danger',
                    details: 'Megtekintés nem engedélyezett',
                    icon: 'fas fa-user-times',
                });
...
function insertPersistButton(options) {
    ...
    var message = options.persistMessage || 'Form nyitva tartása';
        ...
        if (field) {
            ...
            case 'decimal':
                if (!validateFormField(name, options)) {
                    ...
                    data_errors[name] = ['Adj meg egy érvényes számot'];
                }
                break;
            default:
                break;
...
function constructField(name, parameters, options={}) {
    ...
    if (extra) {
        ...
        if (!parameters.required && !options.hideClearButton) {
            html += `
            <span class='input-group-text form-clear' id='clear_${field_name}' title='Bevitel törlése'>
...
function selectImportFields(url, data={}, options={}) {
    ...
    var headers = `<tr><th>Fájl oszlop</th><th>Mező név</th></tr>`;
