...
function editSalesOrder(order_id, options={}) {
    ...
    constructForm(`/api/order/so/${order_id}/`, {
        ...
        title: 'Modifica Ordine di Vendita',
    ...
    inventreeGet(`/api/order/so/shipment/${shipment_id}/`, {}, {
        success: function(shipment) {
            ...
            if (!allocations || allocations.length == 0) {
                ...
                Nessun elemento di magazzino disponibile è stato assegnato a questa spedizione
                </div>
                `;
            } else {
                html = `
                I seguenti elementi in magazzino saranno spediti
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        stock = `Numero Seriale: ${allocation.serial}`;
                    } else {
                        stock = `Quantità: ${allocation.quantity}`;
                    }
            ...
            constructForm(`/api/order/so/shipment/${shipment_id}/ship/`, {
                method: 'POST',
                title: `Completa Spedizione ${shipment.reference}`,
                fields: {
                    shipment_date: {
                ...
                preFormContent: html,
                confirm: true,
                confirmMessage: 'Conferma Spedizione',
...
function completePendingShipments(order_id, options={}) {
    ...
    if (allocated_shipments.length > 0) {
        ...
        if (!pending_shipments.length) {
            html += `
            Nessuna spedizione in sospeso trovata
            `;
        } else {
            html += `
            Nessun elemento di magazzino disponibile è stato assegnato a questa spedizione
...
function completeSalesOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Completa Ordine Di Vendita',
            confirm: true,
            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.pending_shipments) {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    L\'ordine non può essere completato in quanto ci sono spedizioni incomplete<br>
                    </div>`;
                }
                ...
                if (!opts.context.is_complete) {
                    ...
                    Questo ordine ha elementi di riga che non sono stati completati.<br>
...
function cancelSalesOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Annulla Ordine di Vendita',
            confirm: true,
            preFormContent: function(opts) {
                var html = `
                <div class='alert alert-block alert-warning'>
                Cancellando questo ordine, l\'ordine non sarà più modificabile.
                </div>`;
...
function loadSalesOrderTable(table, options) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Non sono state trovati ordini di vendita';
        },
...
function loadSalesOrderShipmentTable(table, options={}) {
    ...
    function makeShipmentActions(row) {
        ...
        html += makeEditButton('button-shipment-edit', pk, 'Modifica spedizione');

        if (!options.shipped) {
            html += makeIconButton('fa-truck icon-green', 'button-shipment-ship', pk, 'Completa spedizione');
        }

        var enable_delete = row.allocations && row.allocations.length == 0;

        html += makeDeleteButton('button-shipment-delete', pk, 'Elimina spedizione', {disabled: !enable_delete});
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nessuna spedizione corrispondente trovata';
        },
        columns: [
            {
                visible: false,
            ...
            {
                field: 'reference',
                title: 'Riferimento della spedizione',
                switchable: false,
            },
            {
                field: 'allocations',
...
function allocateStockToSalesOrder(order_id, line_items, options={}) {
    ...
    constructForm(`/api/order/so/${order_id}/allocate/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Conferma l\'assegnazione della giacenza',
        title: 'Assegna Elementi di Magazzino all\'Ordine di Vendita',
...
function loadSalesOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        original: options.params,
        formatNoMatches: function() {
            return 'Nessun ordine di vendita trovato';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    var text = `Articoli in magazzino ${row.item}`;
    ...
    function setupCallbacks() {
        ...
        table.find('.button-allocation-edit').click(function() {
            ...
            constructForm(
                `/api/order/so-allocation/${pk}/`,
                {
                    ...
                    title: 'Modifica posizione giacenza',
        ...
        table.find('.button-allocation-delete').click(function() {
            ...
            constructForm(
                ...
                {
                    method: 'DELETE',
                    confirmMessage: 'Conferma Operazione Eliminazione',
                    title: 'Elimina posizione giacenza',
                    refreshTable: options.table,
                }
            );
        });
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `Quantità: ${row.quantity}`;

                    if (item && item.serial != null && row.quantity == 1) {
                        text = `Numero Seriale: ${item.serial}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        return `<em>Spedito al cliente - ${row.shipment_date}</em>`;
                    } else if (row.location) {
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || 'Posizione',
                            `/stock/location/${row.location}/`
                        );
                    } else {
                        return `<em>Nessun posizione specificata</em>`;
                    }
                },
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        html += `<span class='badge bg-success badge-right'>Spedito</span>`;
                    } else {
                        html += makeEditButton('button-allocation-edit', pk, 'Modifica allocazione magazzino');
                        html += makeDeleteButton('button-allocation-delete', pk, 'Elimina posizione giacenza');
                    }
...
function loadSalesOrderLineItemTable(table, options={}) {
        ...
        },
        {
            ...
            formatter: function(value, row) {
                if (row.target_date) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times', 'Questo elemento è in ritardo');
                    }
    ...
    if (options.open) {
        columns.push(
            {
                field: 'stock',
                title: 'Disponibilità in magazzino',
                formatter: function(value, row) {
                    ...
                    if (available > 0) {
                        ...
                        html += `<span class='badge rounded-pill bg-danger'>Nessuna Scorta Disponibile</span>`;
                    }

                    if (required > 0) {
                        if (available >= required) {
                            html += makeIconBadge('fa-check-circle icon-green', 'Scorte sufficienti disponibili');
                        } else {
                            html += makeIconBadge('fa-times-circle icon-red', 'Scorte insufficienti disponibili');
                        }
                    }
        ...
        formatter: function(value, row, index, field) {
            ...
            if (row.part && row.part_detail) {
                ...
                if (options.allow_edit && (row.shipped < row.quantity)) {
                    if (part.trackable) {
                        buttons += makeIconButton('fa-hashtag icon-green', 'button-add-by-sn', pk, 'Assegna Numeri di Serie');
                    }
                    buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-add', pk, 'Assegna scorta');
                    if (part.purchaseable) {
                        buttons += makeIconButton('fa-shopping-cart', 'button-buy', row.part, 'Prezzo d\'acquisto');
                    }

                    if (part.assembly) {
                        buttons += makeIconButton('fa-tools', 'button-build', row.part, 'Produci scorta');
                    }
                }
            }

            buttons += makeIconButton('fa-dollar-sign icon-green', 'button-price', pk, 'Calcola il prezzo');

            if (options.allow_edit) {
                buttons += makeCopyButton('button-duplicate', pk, 'Duplica linea elemento');
                buttons += makeEditButton('button-edit', pk, 'Modifica linea elemento');
            }

            if (options.allow_delete) {
                var delete_disabled = false;

                var title = 'Cancella linea elemento';

                if (row.shipped) {
                    delete_disabled = true;
                    title = 'Non può essere eliminato perché gli elementi sono stati spediti';
                } else if (row.allocated) {
                    delete_disabled = true;
                    title = 'Non può essere eliminato perché gli elementi sono stati assegnati';
                }
            ...
            launchModalForm(
                '/order/purchase-order/pricing/',
                {
                    ...
                    buttons: [
                        {
                            name: 'update_price',
                            title: 'Aggiorna Prezzo Unitario'
                        },
                    ],
                    success: reloadTable,
                }
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nessun elemento di riga corrispondente';
        },
        queryParams: filters,
        original: options.params,
        url: options.url,
