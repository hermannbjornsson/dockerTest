    ...
    inventreeGet(`/api/order/so/shipment/${shipment_id}/`, {}, {
        success: function(shipment) {
            ...
            if (!allocations || allocations.length == 0) {
                ...
                Geen voorraadartikelen toegewezen aan deze zending
                </div>
                `;
            } else {
                html = `
                De volgende voorraadartikelen worden verzonden
                <table class='table table-striped table-condensed'>
                    <thead>
                        <tr>
                            <th>Onderdeel</th>
                            <th>Voorraadartikel</th>
                        </tr>
                    </thead>
                    <tbody>
                `;
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        ...
                        stock = `Hoeveelheid: ${allocation.quantity}`;
                    }
            ...
            constructForm(`/api/order/so/shipment/${shipment_id}/ship/`, {
                method: 'POST',
                title: `Verzending Voltooien ${shipment.reference}`,
                fields: {
                    shipment_date: {
                ...
                preFormContent: html,
                confirm: true,
                confirmMessage: 'Verzending Bevestigen',
...
function completePendingShipments(order_id, options={}) {
    ...
    if (allocated_shipments.length > 0) {
        ...
        if (!pending_shipments.length) {
            html += `
            Geen verzendingen in behandeling gevonden
            `;
        } else {
            html += `
        ...
        `/api/order/so/${order_id}/complete/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Order markeren als voltooid?
                </div>`;

                if (opts.context.pending_shipments) {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    Bestelling kan niet worden voltooid omdat er onvolledige verzendingen aanwezig zijn<br>
                    </div>`;
                }
                ...
                if (!opts.context.is_complete) {
                    ...
                    Na het voltooien van de order zijn de order en de artikelen langer bewerkbaar.
                    </div>`;
                }
    ...
    constructForm(
        `/api/order/so/${order_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                var html = `
                <div class='alert alert-block alert-warning'>
                Na annulering van de order kan de order niet meer bewerkt worden.
                </div>`;
...
function loadSalesOrderTable(table, options) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Geen verkooporder gevonden';
        },
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Order is achterstallig');
                    }
...
function loadSalesOrderShipmentTable(table, options={}) {
    ...
    function makeShipmentActions(row) {
        ...
        html += makeEditButton('button-shipment-edit', pk, 'Verzending bewerken');

        if (!options.shipped) {
            html += makeIconButton('fa-truck icon-green', 'button-shipment-ship', pk, 'Verzending Voltooien');
        }

        var enable_delete = row.allocations && row.allocations.length == 0;

        html += makeDeleteButton('button-shipment-delete', pk, 'Verzending verwijderen', {disabled: !enable_delete});
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Geen overeenkomende verzending gevonden';
        },
        columns: [
            {
                visible: false,
...
function allocateStockToSalesOrder(order_id, line_items, options={}) {
    ...
    function renderLineItemRow(line_item, quantity) {
        ...
        var quantity_input = constructField(
            `items_quantity_${pk}`,
            {
                ...
                min_value: 0,
                value: quantity || 0,
                title: 'Specificeer voorraadtoewijzingshoeveelheid',
                required: true,
            },
            {
                hideLabels: true,
    ...
    if (table_entries.length == 0) {
        showAlertDialog(
            ...
            'Er moet op zijn minst één onderdeel toegewezen worden',
        );
    ...
    html += constructField(
        'take_from',
        {
            type: 'related field',
            label: 'Bronlocatie',
            help_text: 'Selecteer bron locatie (laat het veld leeg om iedere locatie te gebruiken)',
            required: false,
        },
        {},
    );
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Onderdeel</th>
                <th style='min-width: 250px;'>Voorraadartikel</th>
                <th>Hoeveelheid</th>
                <th></th>
        </thead>
        <tbody>
    ...
    constructForm(`/api/order/so/${order_id}/allocate/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Bevestig de voorraadtoewijzing',
        title: 'Voorraadartikel toewijzen aan Verkooporder',
        afterRender: function(fields, opts) {
            ...
            var take_from_field = {
                ...
                noResults: function(query) {
                    return 'Geen overeenkomende voorraadlocaties';
                },
            };
                ...
                initializeRelatedField(
                    {
                        ...
                        noResults: function(query) {
                            return 'Geen overeenkomende voorraadartikelen';
                        }
                    },
                    null,
                    opts
...
function loadSalesOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        original: options.params,
        formatNoMatches: function() {
            return 'Geen verkooporder toewijzingen gevonden';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    var text = `Voorraadartikel ${row.item}`;
    ...
    function setupCallbacks() {
        ...
        table.find('.button-allocation-edit').click(function() {
            ...
            constructForm(
                `/api/order/so-allocation/${pk}/`,
                {
                    ...
                    title: 'Bewerk Voorraadtoewijzing',
        ...
        table.find('.button-allocation-delete').click(function() {
            ...
            constructForm(
                ...
                {
                    method: 'DELETE',
                    confirmMessage: 'Bevestig Verwijderen',
                    title: 'Verwijder Voorraadtoewijzing',
                    refreshTable: options.table,
                }
            );
        });
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `Hoeveelheid: ${row.quantity}`;
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        return `<em>Verzonden aan klant - ${row.shipment_date}</em>`;
                    } else if (row.location) {
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || 'Locatie',
                            `/stock/location/${row.location}/`
                        );
                    } else {
                        return `<em>Voorraadlocatie niet gespecificeerd</em>`;
                    }
                },
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        html += `<span class='badge bg-success badge-right'>Verzonden</span>`;
                    } else {
                        html += makeEditButton('button-allocation-edit', pk, 'Voorraadtoewijzing bewerken');
                        html += makeDeleteButton('button-allocation-delete', pk, 'Voorraadtoewijzing verwijderen');
                    }
...
function loadSalesOrderLineItemTable(table, options={}) {
        ...
        },
        {
            ...
            formatter: function(value, row) {
                if (row.target_date) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times', 'Dit artikel is achterstallig');
                    }
        ...
        columns.push(
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (available > 0) {
                        ...
                        html += `<span class='badge rounded-pill bg-danger'>Geen Voorraad Aanwezig</span>`;
                    }

                    if (required > 0) {
                        if (available >= required) {
                            html += makeIconBadge('fa-check-circle icon-green', 'Genoeg voorraad beschikbaar');
                        } else {
                            html += makeIconBadge('fa-times-circle icon-red', 'Onvoldoende voorraad beschikbaar');
                        }
                    }
        ...
        formatter: function(value, row, index, field) {
            ...
            if (row.part && row.part_detail) {
                ...
                if (options.allow_edit && (row.shipped < row.quantity)) {
                    if (part.trackable) {
                        buttons += makeIconButton('fa-hashtag icon-green', 'button-add-by-sn', pk, 'Wijs serienummers toe');
                    }
                    buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-add', pk, 'Voorraad toewijzen');
                    if (part.purchaseable) {
                        buttons += makeIconButton('fa-shopping-cart', 'button-buy', row.part, 'Koop voorraad');
                    }

                    if (part.assembly) {
                        buttons += makeIconButton('fa-tools', 'button-build', row.part, 'Productie voorraad');
                    }
                }
            }

            buttons += makeIconButton('fa-dollar-sign icon-green', 'button-price', pk, 'Bereken prijs');

            if (options.allow_edit) {
                buttons += makeCopyButton('button-duplicate', pk, 'Artikel dupliceren');
                buttons += makeEditButton('button-edit', pk, 'Artikel bewerken');
            }

            if (options.allow_delete) {
                var delete_disabled = false;

                var title = 'Artikel verwijderen';

                if (row.shipped) {
                    delete_disabled = true;
                    title = 'Kan niet worden verwijderd omdat artikelen verzonden zijn';
                } else if (row.allocated) {
                    delete_disabled = true;
                    title = 'Kan niet worden verwijderd omdat artikelen toegewezen zijn';
                }
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Geen overeenkomende artikelen';
        },
        queryParams: filters,
        original: options.params,
        url: options.url,
