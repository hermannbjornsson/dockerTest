...
function partGroups() {
    ...
    return {
        attributes: {
            title: 'Teileigenschaften',
            collapsible: true,
        },
        create: {
            title: 'Erstellungsoptionen für Teile',
            collapsible: true,
        },
        duplicate: {
            title: 'Einstellungen für Teilkopien',
            collapsible: true,
        },
        initial_stock: {
            title: 'Initialer Lagerbestand',
            collapsible: true,
            hidden: !global_settings.PART_CREATE_INITIAL,
        },
        initial_supplier: {
            title: 'Initiale Lieferantendaten',
            collapsible: true,
...
function partFields(options={}) {
    ...
    var fields = {
        category: {
            secondary: {
                title: 'Teil-Kategorie hinzufügen',
                fields: function() {
...
function categoryFields(options={}) {
    let fields = {
        parent: {
            help_text: 'Übergeordnete Teilkategorie',
            required: false,
        },
        name: {},
        description: {},
...
function editCategory(pk) {
    ...
    constructForm(url, {
        fields: fields,
        title: 'Teil-Kategorie bearbeiten',
        reload: true,
    });
}
...
function deletePartCategory(pk, options={}) {
    ...
    var subChoices = [
        {
            value: 0,
            display_name: 'In übergeordnete Kategorie verschieben',
        },
        {
            value: 1,
    ...
    constructForm(url, {
        title: 'Teil-Kategorie löschen',
        method: 'DELETE',
        fields: {
            'delete_parts': {
                label: 'Aktion für Teile in dieser Kategorie',
                choices: subChoices,
                type: 'choice'
            },
            'delete_child_categories': {
                label: 'Aktion für Unterkategorien',
                choices: subChoices,
                type: 'choice'
            },
        },
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        ...
        persist: true,
        persistMessage: 'Ein weiteres Teil anlegen',
        successMessage: 'Teil erfolgreich angelegt',
...
function editPart(pk) {
    ...
    constructForm(url, {
        ...
        reload: true,
        successMessage: 'Teil bearbeitet',
    });
}
...
function duplicatePart(pk, options={}) {
    ...
    var title = 'Teil duplizieren';

    if (options.variant) {
        title = 'Teil-Variante anlegen';
    }
    ...
    inventreeGet(`/api/part/${pk}/`, {}, {
        success: function(part) {
            if (part.active) {
                showAlertDialog(
                    'Aktives Teil',
                    'Teil kann nicht gelöscht werden, da es derzeit aktiv ist',
                    {
                        alert_style: 'danger',
                    }
                );
            ...
            Das Löschen dieses Teils kann nicht rückgängig gemacht werden
            <ul>
            <li>Alle Lagerartikel für dieses Teil werden gelöscht</li>
            <li>Dieses Teil wird von allen Stücklisten entfernt</li>
            <li>Alle Hersteller- und Zuliefererinformationen für dieses Teil werden gelöscht</li>
            </div>`;
                ...
                },
                {
                    ...
                    success: function(response) {
                        if (response.starred) {
                            ...
                            $(options.button).attr('title', 'Sie haben Benachrichtigungen für dieses Teil abonniert');

                            showMessage('Sie haben Benachrichtigungen für dieses Teil abonniert', {
                                style: 'success',
                            });
                        } else {
                            $(options.button).removeClass('fas fa-bell icon-green').addClass('fa fa-bell-slash');
                            $(options.button).attr('title', 'Benachrichtigungen für dieses Teil abonnieren');

                            showMessage('Sie haben Benachrichtigungen für dieses Teil abgemeldet', {
                                style: 'warning',
                            });
                        }
                    }
...
function validateBom(part_id, options={}) {
    ...
    Die Stückliste zu validieren markiert jede Zeile als gültig
    </div>
    `;
    ...
    constructForm(`/api/part/${part_id}/bom-validate/`, {
        ...
        preFormContent: html,
        title: 'Stückliste prüfen',
        reload: options.reload,
        onSuccess: function(response) {
            showMessage('überprüfte Stückliste');
        }
    });
}
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            elements.push(`Bestand niedrig: ${part.total_in_stock}`);
        } else if (part.unallocated_stock <= 0) {
            // There is no available stock at all
            elements.push(`Kein Lagerbestand verfügbar`);
        } else if (part.unallocated_stock < part.in_stock) {
            // Unallocated quantity is less than total quantity
            if (options.hideTotalStock) {
                elements.push(`Verfügbar: ${part.unallocated_stock}`);
            } else {
                elements.push(`Verfügbar: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
        ...
        elements.push(`Kein Bestand`);
    }

    // Check for items on order
    if (part.ordering) {
        elements.push(`Bestellt: ${part.ordering}`);
    }

    // Check for items being built
    if (part.building) {
        elements.push(`Im Bau: ${part.building}`);
    }
    ...
    if (units && !options.no_units) {
        output += `<span class='badge rounded-pill text-muted bg-muted ${classes}'>Einheit: ${units}</span> `;
    }
...
function makePartIcons(part) {
    ...
    if (part.trackable) {
        html += makeIconBadge('fa-directions', 'Nachverfolgbares Teil');
    }

    if (part.virtual) {
        html += makeIconBadge('fa-ghost', 'virtuelles Teil');
    }

    if (part.is_template) {
        html += makeIconBadge('fa-clone', 'Vorlagenteil');
    }

    if (part.assembly) {
        html += makeIconBadge('fa-tools', 'Baugruppe');
    }

    if (part.starred) {
        html += makeIconBadge('fa-bell icon-green', 'Abonnierter Teil');
    }

    if (part.salable) {
        html += makeIconBadge('fa-dollar-sign', 'Verkäufliches Teil');
    }

    if (!part.active) {
        html += `<span class='badge badge-right rounded-pill bg-warning'>Inaktiv</span> `;
    }
...
function generateStocktakeReport(options={}) {
    ...
    Die Erstellung eines neuen Inventurberichtes planen. Nach Fertigstellung steht der Inventurbericht zum Download zur Verfügung.
    </div>
    `;
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Inventurbericht generieren',
            preFormContent: content,
            fields: fields,
            onSuccess: function(response) {
                showMessage('Inventurbericht geplant', {
                    style: 'success',
                });
            }
        }
...
function loadPartStocktakeTable(partId, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine Inventurinformationen verfügbar';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.allow_edit) {
                        html += makeEditButton('button-edit-stocktake', row.pk, 'Inventureintrag bearbeiten');
                    }

                    if (options.allow_delete) {
                        html += makeDeleteButton('button-delete-stocktake', row.pk, 'Inventureintrag löschen');
                    }
        ...
        onPostBody: function() {
            ...
            $(table).find('.button-edit-stocktake').click(function() {
                ...
                constructForm(`/api/part/stocktake/${pk}/`, {
                    ...
                    title: 'Inventureintrag bearbeiten',
                    refreshTable: table,
                });
            });
...
function loadPartVariantTable(table, partId, options={}) {
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.variant_stock > 0) {
                    text = `<em>${text}</em>`;
                    text += `<span title='Beinhaltet Variantenbestand' class='fas fa-info-circle float-right icon-blue'></span>`;
                }
    ...
    table.inventreeTable({
        ...
        queryParams: filters,
        formatNoMatches: function() {
            return 'Keine Varianten gefunden';
        },
        columns: cols,
        treeEnable: true,
        rootParentId: partId,
...
function createPartParameter(part_id, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Parameter hinzufügen';
...
function editPartParameter(param_id, options={}) {
    options.fields = partParameterFields();
    options.title = 'Parameter bearbeiten';
    options.focus = 'data';
...
function loadPartParameterTable(table, options) {
    ...
    $(table).inventreeTable({
        ...
        groupBy: false,
        formatNoMatches: function() {
            return 'Keine Parameter gefunden';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-parameter-edit', pk, 'Parameter bearbeiten');
                    html += makeDeleteButton('button-parameter-delete', pk, 'Parameter löschen');
...
function loadPartParameterTemplateTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine Teilparametervorlagen gefunden';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    buttons += makeEditButton('template-edit', row.pk, 'Vorlage bearbeiten');
                    buttons += makeDeleteButton('template-delete', row.pk, 'Vorlage löschen');
    ...
    $(table).on('click', '.template-edit', function() {
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                ...
                title: 'Teilparametervorlage bearbeiten',
                refreshTable: table,
            }
        );
    });
    ...
    $(table).on('click', '.template-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
            Alle Parameter, die diese Vorlage referenzieren, werden ebenfalls gelöscht
        </div>`;
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                method: 'DELETE',
                preFormContent: html,
                title: 'Teilparametervorlage löschen',
                refreshTable: table,
            }
        );
    });
...
function loadPartPurchaseOrderTable(table, part_id, options={}) {
    ...
    $(table).inventreeTable({
        ...
        columns: [
            ...
            {
                field: 'sku',
                title: 'SKU (Lagerbestandseinheit)',
                switchable: true,
                formatter: function(value, row) {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Packmenge: ${row.pack_quantity} - Gesamtstückzahl: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let total = value * row.supplier_part_detail.pack_quantity_native;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Packmenge: ${row.pack_quantity} - Gesamtstückzahl: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Empfangen</span>`;
                    } else if (row.order_detail && row.order_detail.status == 20) {
                        let html = '';
                        var pk = row.pk;

                        html += makeIconButton('fa-sign-in-alt', 'button-line-receive', pk, 'Position empfangen');
...
function loadRelatedPartsTable(table, part_id, options={}) {
    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                let html = '';
                html += makeDeleteButton('button-related-delete', row.pk, 'Teile-Beziehung löschen');
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>Kein Bestand</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>Kein Bestand</span>`;
    }

    rows += `<tr><td><b>Bestand</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>Bestellt</b></td><td>${part.ordering} ${units}</td></tr>`;
    }

    if (part.building) {
        rows += `<tr><td><b>Im Bau</b></td><td>${part.building} ${units}</td></tr>`;
    }
...
function setPartCategory(data, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
        Legen Sie die Teilkategorie für die ausgewählten Teile fest
    </div>
    `;

    constructForm('/api/part/change_category/',{
        title: 'Teil-Kategorie auswählen',
        method: 'POST',
        preFormContent: html,
        fields: {
            category: {},
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                        ...
                        if (row.subRequested) {
                            ...
                            html += `
                                <a href='#' pk='${row.pk}' class='load-sub-category'>
                                    <span class='fas fa-sync-alt' title='Unterkategorien laden'></span>
                                </a> `;
                        }
                    }
                    ...
                    if (row.starred) {
                        html += makeIconBadge('fa-bell icon-green', 'Abonnierte Kategorie');
                    }
...
function loadPartTestTemplateTable(table, options) {
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Keine zur Anfrage passenden Testvorlagen';
        },
        ...
        columns: [
            ...
            },
            {
                ...
                title: 'Anhang muss eingegeben werden',
                formatter: function(value) {
                    return yesNoLabel(value);
                }
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part == part) {
                        ...
                        var text = 'Dieses Testergebnis ist für ein Hauptteil';
        ...
        onPostBody: function() {
            ...
            table.find('.button-test-edit').click(function() {
                ...
                constructForm(url, {
                    ...
                    title: 'Testergebnis-Vorlage bearbeiten',
                    onSuccess: function() {
                        table.bootstrapTable('refresh');
                    },
                });
            ...
            table.find('.button-test-delete').click(function() {
                ...
                constructForm(url, {
                    method: 'DELETE',
                    title: 'Testergebnis-Vorlage löschen',
                    refreshTable: table,
                });
            });
        }
...
function loadPartSchedulingChart(canvas_id, part_id) {
        ...
        {},
        {
            ...
            success: function(response) {
                ...
                for (var idx = 0; idx < response.length; idx++) {
                    ...
                    if (date == null) {
                        date_string = '<em>Kein Datum angegeben</em>';
                        date_string += makeIconBadge('fa-exclamation-circle icon-red', 'Kein Datum angegeben');
                    } else if (date < today) {
                        date_string += makeIconBadge('fa-exclamation-circle icon-yellow', 'Das angegebene Datum liegt in der Vergangenheit');
                    }

                    var quantity_string = entry.quantity + entry.speculative_quantity;

                    if (entry.speculative_quantity != 0) {
                        quantity_string += makeIconBadge('fa-question-circle icon-blue', 'Spekulativ');
                    }
    ...
    if (n_entries < 1) {
        ...
        var message = `
        <div class='alert alert-block alert-info'>
            Keine Zeitplanung für dieses Teil vorhanden.
        </div>`;

        if (was_error) {
            message = `
                <div class='alert alert-block alert-danger'>
                    Fehler beim Abrufen der Zeitplanungsinformationen für dieses Teil.
                </div>
            `;
        }
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Anzahl: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
