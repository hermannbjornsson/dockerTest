...
function addBomItem(part_id, options={}) {
    ...
    constructForm('/api/bom/', {
        fields: fields,
        method: 'POST',
        title: 'Alkatrészjegyzék tétel létrehozása',
        focus: 'sub_part',
...
function constructBomUploadTable(data, options={}) {
    ...
    function constructRow(row, idx, fields) {
        ...
        buttons += makeInfoButton('button-row-data', idx, 'Sor adatok mutatása');
        buttons += makeRemoveButton('button-row-remove', idx, 'Sor törlése');
...
function downloadBomTemplate(options={}) {
    ...
    constructFormBody({}, {
        title: 'Alkarészjegyzék sablon letöltése',
        fields: {
            format: {
                label: 'Formátum',
                help_text: 'Fájlfomátum kiválasztása',
                required: true,
                type: 'choice',
                value: format,
...
function exportBom(part_id, options={}) {
    ...
    constructFormBody({}, {
        title: 'Alkatrészjegyzék exportálása',
        fields: {
            format: {
                label: 'Formátum',
                help_text: 'Fájlfomátum kiválasztása',
            ...
            cascade: {
                label: 'Többszintű alkatrészjegyzék',
                help_text: 'Alszerelvények alkatrészlistáinak felhasználása',
                type: 'boolean',
                value: inventreeLoad('bom-export-cascading', true),
            },
            levels: {
                label: 'Szintek',
                help_text: 'Válaszd ki a maximum alkatrészjegyzék szintet amit exportáljunk (0=összes szintet)',
                type: 'integer',
                value: 0,
                required: true,
                min_value: 0,
            ...
            substitute_part_data: {
                label: 'Alternatív alkatrészekkel együtt',
                help_text: 'Alternatív alkatrészek megjelenítése az exportált alkatrészjegyzékben',
                type: 'boolean',
                value: inventreeLoad('bom-export-substitute_part_data', false),
            },
            parameter_data: {
                label: 'Paraméter adattal együtt',
                help_text: 'Alkatrész paraméter adatok megjelenítése az exportált alkatrészjegyzékben',
                type: 'boolean',
                value: inventreeLoad('bom-export-parameter_data', false),
            },
            stock_data: {
                label: 'Készlet adatokkal együtt',
                help_text: 'Készlet adatok megjelenítése az exportált alkatrészjegyzékben',
                type: 'boolean',
                value: inventreeLoad('bom-export-stock_data', false),
            },
            manufacturer_data: {
                label: 'Gyártói adatokkal együtt',
                help_text: 'Gyártói adatok megjelenítése az exportált alkatrészjegyzékben',
                type: 'boolean',
                value: inventreeLoad('bom-export-manufacturer_data', false),
            },
            supplier_data: {
                label: 'Beszállítói adatokkal együtt',
                help_text: 'Beszállítói adatok megjelenítése az exportált alkatrészjegyzékben',
                type: 'boolean',
                value: inventreeLoad('bom-export-supplier_data', false),
            },
            pricing_data: {
                label: 'Ár adatokkal együtt',
                help_text: 'Ár adatok megjelenítése az exportált alkatrészjegyzékben',
                type: 'boolean',
...
function bomSubstitutesDialog(bom_item_id, substitutes, options={}) {
    ...
    function renderSubstituteRow(substitute) {
        ...
        buttons += makeRemoveButton('button-row-remove', pk, 'Helyettesítő alkatrész törlése');
    ...
    var html = `
    <div class='alert alert-block'>
    <strong>Kiindulási alkatrész</strong><hr>
    ...
    <div class='alert alert-success alert-block'>
        Válassz és adj hozzá új helyettesítő alkatrészt a lenti mezőben
    </div>
    `;
    ...
    function addRemoveCallback(modal, element) {
        $(modal).find(element).click(function() {
            ...
            Biztosan törölni akarod ezt a helyettesítő alkatrész hozzárendelést?
            </div>
            `;

            constructForm(`/api/bom/substitute/${pk}/`, {
                method: 'DELETE',
                title: 'Helyettesítő alkatrész törlése',
                preFormContent: pre,
                confirm: true,
                onSuccess: function() {
    ...
    constructForm('/api/bom/substitute/', {
        ...
        preFormContent: html,
        closeText: 'Bezárás',
        submitText: 'Helyettesítő hozzáadása',
        title: 'Alkatrészjegyzék tétel helyettesítők szerkesztése',
...
function deleteBomItems(items, options={}) {
    ...
    Az összes kijelölt alkatrészjegyzék tétel törlésre kerül
    </div>
    ...
    constructForm('/api/bom/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Töröljük a kiválasztott alkatrészjegyzék tételeket?',
        form_data: {
            items: ids,
        },
        preFormContent: html,
...
function loadBomTable(table, options={}) {
    ...
    cols.push(
        {
            ...
            formatter: function(value, row) {
                    ...
                    if (row.sub_assembly_received) {
                        ...
                        html += `
                            <a href='#' pk='${row.pk}' class='load-sub-assembly' id='load-sub-assembly-${row.pk}'>
                                <span class='fas fa-sync-alt' title='Alkatrészjegyzék betöltése az al-gyártmányhoz'></span>
                            </a> `;
                    }
                }
                ...
                if (row.substitutes && row.substitutes.length > 0) {
                    html += makeIconBadge('fa-exchange-alt', 'Vannak helyettesítők');
                }
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (row.consumable) {
                text += ` <small>(Fogyóeszköz)</small>`;
            }

            if (row.optional) {
                text += ' <small>(Opcionális)</small>';
            }
            ...
            if (any_pricing) {
                ...
                if (complete_pricing) {
                    html += makeIconBadge(
                        ...
                        'Alkatrészjegyzék árazása teljes',
                    );
                } else {
                    html += makeIconBadge(
                        'fa-exclamation-circle icon-yellow',
                        'Alkatrészjegyzék árazása nem teljes',
                    );
                }
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (available_stock <= 0) {
                text += makeIconBadge('fa-times-circle icon-red', 'Nincs szabad');
            } else {
                var extra = '';
    ...
    cols.push(
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.consumable) {
                    return `<em>Fogyóeszköz tétel</em>`;
                }
        ...
        cols.push({
            ...
            formatter: function(value, row) {
                ...
                if (row.part == options.parent_id) {
                    ...
                    var bValidate = makeIconButton('fa-check-circle icon-green', 'bom-validate-button', row.pk, 'Alkatrészjegyzék tétel jóváhagyása');

                    var bValid = makeIconButton('fa-check-double icon-green', 'bom-valid-button', row.pk, 'Ez a sor jóvá lett hagyva', {disabled: true});

                    var bSubs = makeIconButton('fa-exchange-alt icon-blue', 'bom-substitutes-button', row.pk, 'Helyettesítő alkatrészek szerkesztése');

                    var bEdit = makeEditButton('bom-edit-button', row.pk, 'Alkatrészjegyzék tétel szerkesztése');

                    var bDelt = makeDeleteButton('bom-delete-button', row.pk, 'Alkatrészjegyzék tétel törlése');
            ...
            footerFormatter: function(data) {
                return `
                <button class='btn btn-success float-right' type='button' title='Alkatrészjegyzék tétel hozzáadása' id='bom-item-new-footer'>
                    ${makeIcon('fa-plus-circle')} Alkatrészjegyzék tétel hozzáadása
                </button>
                `;
            }
        });
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nem találhatók alkatrészjegyzék tételek';
        },
        queryParams: filters,
        original: params,
        columns: cols,
    ...
    if (options.editable) {
        ...
        table.on('click', '.bom-edit-button', function() {
            ...
            constructForm(`/api/bom/${pk}/`, {
                fields: fields,
                title: 'Alkatrészjegyzék tétel szerkesztése',
                focus: 'sub_part',
                onSuccess: function() {
                    reloadBomTable(table);
                }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.parent && row.parent != 'top-level-item') {
                        html += ` <em>(Örökölve a szülő alkatrészjegyzéktől)</em>`;
                    }
