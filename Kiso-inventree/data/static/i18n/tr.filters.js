...
function makeCustomActionGroup(action_group, table) {
    ...
    let title = action_group.title || 'İşlemler';
...
function makeBarcodeActions(barcode_actions, table) {
    ...
    let html = `
    <div class='btn-group' role='group'>
    <button id='barcode-actions' title='Barkod işlemleri' class='btn btn-outline-secondary dropdown-toggle' type='button' data-bs-toggle='dropdown'>
...
function setupFilterList(tableKey, table, target, options={}) {
    ...
    if (report_button || labels_button) {
        let print_buttons = `
        <div class='btn-group' role='group'>
        <button id='printing-options' title='Yazdırma işlemleri' class='btn btn-outline-secondary dropdown-toggle' type='button' data-bs-toggle='dropdown'>
...
function getFilterOptionValue(tableKey, filterKey, valueKey) {
    ...
    if (filter.type == 'bool') {
        if (value == '1') return 'doğru';
        if (value == '0') return 'yanlış';
