...
function selectLabel(labels, items, options={}) {
        ...
        <div class='form-group'>
            ...
            <div class='controls'>
                <select id='id_plugin' class='select form-control' name='plugin'>
                    <option value='' title='Exportálás PDF-be'>Exportálás PDF-be</option>
        `;
    ...
    if (items.length > 0) {
        ...
        ${items.length} ${item_name} kiválasztva
        </div>`;
    }
    ...
    modalSetTitle(modal, 'Címke sablon kiválasztása');
...
function printLabels(options) {
    ...
    if (!options.items || options.items.length == 0) {
        showAlertDialog(
            ...
            'Nincs tétel kiválasztva a nyomtatáshoz',
        );
        return;
    }
    ...
    inventreeGet(options.url, params, {
        success: function(response) {
            if (response.length == 0) {
                showAlertDialog(
                    'Nem található címke',
                    'Nem található címke sablon a kiválasztott tételekhez',
                );
                return;
            }
            ...
            selectLabel(response, options.items, {
                success: function(data) {
                    ...
                    if (data.plugin) {
                        ...
                        inventreeGet(href, {}, {
                            success: function(response) {
                                showMessage('Címkék nyomtatónak elküldve', {
                                    style: 'success',
                                });
                            }
                        });
