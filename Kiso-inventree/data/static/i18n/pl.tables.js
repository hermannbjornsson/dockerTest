...
function constructOrderTableButtons(options={}) {
    ...
    if (!options.disableCalendarView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_calendar}' title='Pokaż widok kalendarza'><span class='fas fa-calendar-alt'></span></button>`,
            event: function() {
    ...
    if (!options.disableListView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_list}' title='Pokaż widok listy'><span class='fas fa-th-list'></span></button>`,
            event: function() {
                buttonCallback('list');
            }
        });
...
function downloadTableData(table, opts={}) {
    ...
    constructFormBody({}, {
        title: opts.title || 'Eksportuj dane tabeli',
        fields: {
            format: {
                label: 'Format',
                help_text: 'Wybierz format pliku',
                required: true,
                type: 'choice',
                value: 'csv',
...
(function($) {
    ...
    $.fn.bootstrapTable.locales['en-US-custom'] = {
        ...
        formatRecordsPerPage: function(pageNumber) {
            return `${pageNumber} wierszy na stronę`;
        },
        formatShowingRows: function(pageFrom, pageTo, totalRows) {

            if (totalRows === undefined || isNaN(totalRows)) {
                return 'Pokaż wszystkie wiersze';
            } else {
                return `Pokazywane ${pageFrom} do ${pageTo} z ${totalRows} wierszy`;
            }
        },
        ...
        formatPaginationSwitch: function() {
            return 'Ukryj/Pokaż stronicowanie';
        },
