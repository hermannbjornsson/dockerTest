...
function constructOrderTableButtons(options={}) {
    ...
    if (!options.disableCalendarView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_calendar}' title='Visualizzazione calendario'><span class='fas fa-calendar-alt'></span></button>`,
            event: function() {
    ...
    if (!options.disableListView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_list}' title='Visualizzazione elenco'><span class='fas fa-th-list'></span></button>`,
            event: function() {
                buttonCallback('list');
            }
        });
    ...
    if (!options.disableTreeView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_tree}' title='Visualizza vista albero'><span class='fas fa-sitemap'></span></button>`,
            event: function() {
                buttonCallback('tree');
            }
        });
...
function constructExpandCollapseButtons(table, idx=0) {
    ...
    return [
        {
            html: `<button type='button' name='${idx++}' class='btn btn-outline-secondary' title='Espandi tutte le righe'><span class='fas fa-expand'></span></button>`,
            event: function() {
                $(table).bootstrapTable('expandAllRows');
            }
        },
        {
            html: `<button type='button' name='${idx++}' class='btn btn-outline-secondary' title='Comprimi tutte le righe'><span class='fas fa-compress'></span></button>`,
            event: function() {
...
function downloadTableData(table, opts={}) {
    ...
    constructFormBody({}, {
        title: opts.title || 'Esporta Dati Tabella',
        fields: {
            format: {
                label: 'Formato',
                help_text: 'Seleziona Formato File',
                required: true,
                type: 'choice',
                value: 'csv',
    ...
    $.fn.bootstrapTable.locales['en-US-custom'] = {
        ...
        formatShowingRows: function(pageFrom, pageTo, totalRows) {
            ...
            if (totalRows === undefined || isNaN(totalRows)) {
                return 'Mostra tutte le righe';
            } else {
                return `Visualizzo ${pageFrom} a ${pageTo} di ${totalRows} righe`;
            }
        },
        ...
        formatPaginationSwitch: function() {
            return 'Mostra/nascondi la paginazione';
        },
