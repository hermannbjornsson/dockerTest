...
function addBomItem(part_id, options={}) {
    ...
    constructForm('/api/bom/', {
        fields: fields,
        method: 'POST',
        title: 'Creare un elemento della distinta base',
        focus: 'sub_part',
...
function constructBomUploadTable(data, options={}) {
    ...
    function constructRow(row, idx, fields) {
        ...
        buttons += makeInfoButton('button-row-data', idx, 'Visualizzare i dati');
        buttons += makeRemoveButton('button-row-remove', idx, 'Elimina riga');
...
function downloadBomTemplate(options={}) {
    ...
    constructFormBody({}, {
        title: 'Scarica il modello di distinta base',
        fields: {
            format: {
                label: 'Formato',
                help_text: 'Seleziona il formato del file',
                required: true,
                type: 'choice',
                value: format,
...
function exportBom(part_id, options={}) {
    ...
    constructFormBody({}, {
        ...
        fields: {
            ...
            cascade: {
                label: 'Distinta base multilivello',
                help_text: 'Includere i dati della distinta base per i sottoassiemi',
                type: 'boolean',
                value: inventreeLoad('bom-export-cascading', true),
            },
            levels: {
                label: 'Livelli',
                help_text: 'Selezionare il numero massimo di livelli di distinta base da esportare (0 = tutti i livelli)',
                type: 'integer',
                value: 0,
                required: true,
                min_value: 0,
            ...
            substitute_part_data: {
                label: 'Includere Articoli Alternativi',
                help_text: 'Includere articoli alternativi nella distinta base esportata',
                type: 'boolean',
                value: inventreeLoad('bom-export-substitute_part_data', false),
            },
            parameter_data: {
                label: 'Includere i dati dei parametri',
                help_text: 'Includere i dati dei parametri degli articoli nella distinta base esportata',
                type: 'boolean',
                value: inventreeLoad('bom-export-parameter_data', false),
            },
            stock_data: {
                label: 'Includere i dati delle scorte',
                help_text: 'Includere i dati delle scorte dei pezzi nella distinta base esportata',
                type: 'boolean',
                value: inventreeLoad('bom-export-stock_data', false),
            },
            manufacturer_data: {
                label: 'Includere i dati del produttore',
                help_text: 'Includere i dati del produttore delle parti nella distinta base esportata',
                type: 'boolean',
                value: inventreeLoad('bom-export-manufacturer_data', false),
            },
            supplier_data: {
                label: 'Includere i dati dei fornitori',
                help_text: 'Includere i dati del fornitore di parti nella distinta base esportata',
                type: 'boolean',
                value: inventreeLoad('bom-export-supplier_data', false),
            },
            pricing_data: {
                label: 'Includere i prezzi',
                help_text: 'Includere i prezzi delle parti nella distinta base esportata',
                type: 'boolean',
...
function bomSubstitutesDialog(bom_item_id, substitutes, options={}) {
    ...
    function renderSubstituteRow(substitute) {
        ...
        buttons += makeRemoveButton('button-row-remove', pk, 'Rimuovi articolo sostitutivo');
    ...
    var html = `
    <div class='alert alert-block'>
    <strong>Articolo di base</strong><hr>
    ...
    <div class='alert alert-success alert-block'>
        Seleziona e aggiungi un nuovo articolo sostitutivo utilizzando l\'input qui sotto
    </div>
    `;
    ...
    function addRemoveCallback(modal, element) {
        $(modal).find(element).click(function() {
            ...
            Sei sicuro di voler rimuovere questo collegamento all\' articolo sostitutivo?
            </div>
            `;

            constructForm(`/api/bom/substitute/${pk}/`, {
                method: 'DELETE',
                title: 'Rimuovi Articolo Sostitutivo',
                preFormContent: pre,
                confirm: true,
                onSuccess: function() {
    ...
    constructForm('/api/bom/substitute/', {
        ...
        preFormContent: html,
        closeText: 'Chiudi',
        submitText: 'Aggiungi Sostitutivo',
        title: 'Modifica Elementi Sostitutivi Distinta Base',
...
function deleteBomItems(items, options={}) {
    ...
    Tutti gli elementi selezionati della Distinta Base saranno eliminati
    </div>
    ...
    constructForm('/api/bom/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Elimina gli Elementi selezionati della Distinta Base?',
        form_data: {
            items: ids,
        },
        preFormContent: html,
...
function loadBomTable(table, options={}) {
    ...
    cols.push(
        {
            ...
            formatter: function(value, row) {
                    ...
                    if (row.sub_assembly_received) {
                        ...
                        html += `
                            <a href='#' pk='${row.pk}' class='load-sub-assembly' id='load-sub-assembly-${row.pk}'>
                                <span class='fas fa-sync-alt' title='Carica la Distinta Base per il sotto assemblaggio'></span>
                            </a> `;
                    }
                }
                ...
                if (row.substitutes && row.substitutes.length > 0) {
                    html += makeIconBadge('fa-exchange-alt', 'Sostituti Disponibili');
                }
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (row.consumable) {
                text += ` <small>(Consumabile)</small>`;
            }

            if (row.optional) {
                text += ' <small>(Opzionale)</small>';
            }
            ...
            if (any_pricing) {
                ...
                if (complete_pricing) {
                    html += makeIconBadge(
                        ...
                        'I prezzi Distinta Base sono completi',
                    );
                } else {
                    html += makeIconBadge(
                        'fa-exclamation-circle icon-yellow',
                        'I prezzi Distinta Base sono incompleti',
                    );
                }
                ...
                let html = '<em>Nessun prezzo disponibile</em>';
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (available_stock <= 0) {
                text += makeIconBadge('fa-times-circle icon-red', 'Nessuna Scorta Disponibile');
            } else {
                var extra = '';
    ...
    cols.push(
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.consumable) {
                    return `<em>Elementi consumabili</em>`;
                }
        ...
        cols.push({
            ...
            formatter: function(value, row) {
                ...
                if (row.part == options.parent_id) {
                    ...
                    var bValidate = makeIconButton('fa-check-circle icon-green', 'bom-validate-button', row.pk, 'Convalida elemento Distinta Base');

                    var bValid = makeIconButton('fa-check-double icon-green', 'bom-valid-button', row.pk, 'Questa linea è stata convalidata', {disabled: true});

                    var bSubs = makeIconButton('fa-exchange-alt icon-blue', 'bom-substitutes-button', row.pk, 'Modifica articoli sostitutivi');

                    var bEdit = makeEditButton('bom-edit-button', row.pk, 'Modifica elemento Distinta Base');

                    var bDelt = makeDeleteButton('bom-delete-button', row.pk, 'Cancella elemento Distinta Base');
            ...
            footerFormatter: function(data) {
                return `
                <button class='btn btn-success float-right' type='button' title='Aggiungi elemento Distinta Base' id='bom-item-new-footer'>
                    ${makeIcon('fa-plus-circle')} Aggiungi elemento Distinta Base
                </button>
                `;
            }
        });
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nessun elemento trovato in Distinta Base';
        },
        queryParams: filters,
        original: params,
        columns: cols,
    ...
    if (options.editable) {
        ...
        table.on('click', '.bom-edit-button', function() {
            ...
            constructForm(`/api/bom/${pk}/`, {
                fields: fields,
                title: 'Modifica elemento Distinta Base',
                focus: 'sub_part',
                onSuccess: function() {
                    reloadBomTable(table);
                }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.parent && row.parent != 'top-level-item') {
                        html += ` <em>(Ereditato dalla Distinta Base principale)</em>`;
                    }
