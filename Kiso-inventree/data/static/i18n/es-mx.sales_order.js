...
function createSalesOrderLineItem(options={}) {
    ...
    constructForm('/api/order/so-line/', {
        fields: fields,
        method: 'POST',
        title: 'Añadir artículo de línea',
        onSuccess: function(response) {
            handleFormSuccess(response, options);
        },
    });
    ...
    inventreeGet(`/api/order/so/shipment/${shipment_id}/`, {}, {
        success: function(shipment) {
            ...
            if (!allocations || allocations.length == 0) {
                ...
                No se ha asignado ningún artículo de stock a este envío
                </div>
                `;
            } else {
                html = `
                Los siguientes artículos de stock serán enviados
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        stock = `Número de serie: ${allocation.serial}`;
                    } else {
                        stock = `Cantidad: ${allocation.quantity}`;
                    }
            ...
            constructForm(`/api/order/so/shipment/${shipment_id}/ship/`, {
                method: 'POST',
                title: `Completar Envío ${shipment.reference}`,
                fields: {
                    shipment_date: {
                ...
                preFormContent: html,
                confirm: true,
                confirmMessage: 'Confirmar Envío',
...
function completePendingShipments(order_id, options={}) {
    ...
    if (allocated_shipments.length > 0) {
        ...
        if (!pending_shipments.length) {
            ...
            html += `
            No se ha asignado ningún artículo de almacén a los envíos pendientes
            `;
        }
...
function completeSalesOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Ordenes de venta completas',
            confirm: true,
            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.pending_shipments) {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    El pedido no se puede completar porque hay envíos incompletos<br>
                    </div>`;
                }
                ...
                if (!opts.context.is_complete) {
                    ...
                    Este pedido tiene artículos de línea que no han sido completados.<br>
                    Completar este pedido significa que los artículos de orden y línea ya no serán editables.
                    </div>`;
                }
...
function issueSalesOrder(order_id, options={}) {
    ...
    let html = `
    <div class='alert alert-block alert-info'>
    ¿Emitir este pedido de venta?
    </div>`;

    constructForm(`/api/order/so/${order_id}/issue/`, {
        method: 'POST',
        title: 'Emitir orden de venta',
        confirm: true,
        preFormContent: html,
        onSuccess: function(response) {
            handleFormSuccess(response, options);
    ...
    constructForm(
        `/api/order/so/${order_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                var html = `
                <div class='alert alert-block alert-warning'>
                Cancelar esta orden significa que la orden ya no será editable.
                </div>`;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'El pedido está vencido');
                    }
...
function loadSalesOrderShipmentTable(table, options={}) {
    ...
    function makeShipmentActions(row) {
        ...
        html += makeEditButton('button-shipment-edit', pk, 'Editar envío');

        if (!options.shipped) {
            html += makeIconButton('fa-truck icon-green', 'button-shipment-ship', pk, 'Completar envío');
        }

        var enable_delete = row.allocations && row.allocations.length == 0;

        html += makeDeleteButton('button-shipment-delete', pk, 'Eliminar envío', {disabled: !enable_delete});
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'No se encontraron envíos coincidentes';
        },
        columns: [
            {
                visible: false,
...
function allocateStockToSalesOrder(order_id, line_items, options={}) {
    ...
    function renderLineItemRow(line_item, quantity) {
        ...
        var quantity_input = constructField(
            `items_quantity_${pk}`,
            {
                ...
                min_value: 0,
                value: quantity || 0,
                title: 'Especificar la cantidad de asignación de stock',
                required: true,
            },
            {
                hideLabels: true,
    ...
    if (table_entries.length == 0) {
        showAlertDialog(
            'Seleccionar partes',
            'Debe seleccionar al menos una parte para asignar',
        );
    ...
    html += constructField(
        ...
        {
            type: 'related field',
            label: 'Ubicación de la fuente',
            help_text: 'Seleccionar ubicación de origen (dejar en blanco para tomar de todas las ubicaciones)',
            required: false,
        },
        {},
    );
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Parte</th>
                <th style='min-width: 250px;'>Artículo de stock</th>
                <th>Cantidad</th>
                <th></th>
        </thead>
        <tbody>
    ...
    constructForm(`/api/order/so/${order_id}/allocate/`, {
        ...
        title: 'Asignar artículos de stock a pedido de venta',
        afterRender: function(fields, opts) {
            ...
            var take_from_field = {
                ...
                noResults: function(query) {
                    return 'No hay ubicaciones de stock coincidentes';
                },
            };
                ...
                initializeRelatedField(
                    {
                        ...
                        noResults: function(query) {
                            return 'No hay artículos de stock coincidentes';
                        }
                    },
                    null,
                    opts
...
function loadSalesOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        original: options.params,
        formatNoMatches: function() {
            return 'No se encontraron asignaciones de órdenes';
        },
        columns: [
            {
                field: 'pk',
    ...
    function setupCallbacks() {
        ...
        table.find('.button-allocation-edit').click(function() {
            ...
            constructForm(
                `/api/order/so-allocation/${pk}/`,
                {
                    ...
                    title: 'Editar Asignación de Stock',
        ...
        table.find('.button-allocation-delete').click(function() {
            ...
            constructForm(
                ...
                {
                    method: 'DELETE',
                    confirmMessage: 'Confirmar Operación de Eliminar',
                    title: 'Eliminar Adjudicación de Stock',
                    refreshTable: options.table,
                }
            );
        });
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `Cantidad: ${row.quantity}`;

                    if (item && item.serial != null && row.quantity == 1) {
                        text = `Número de serie: ${item.serial}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        return `<em>Enviado al cliente - ${row.shipment_date}</em>`;
                    } else if (row.location) {
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || 'Ubicación',
                            `/stock/location/${row.location}/`
                        );
                    } else {
                        return `<em>Ubicación de stock no especificada</em>`;
                    }
                },
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        html += `<span class='badge bg-success badge-right'>Enviado</span>`;
                    } else {
                        html += makeEditButton('button-allocation-edit', pk, 'Editar asignación de stock');
                        html += makeDeleteButton('button-allocation-delete', pk, 'Eliminar asignación de stock');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        text = `Número de serie: ${row.serial}`;
                    } else {
                        text = `Cantidad: ${row.quantity}`;
                    }
...
function loadSalesOrderLineItemTable(table, options={}) {
        ...
        formatter: function(value, row, index, field) {
            ...
            if (row.part && row.part_detail) {
                ...
                if (options.allow_edit && (row.shipped < row.quantity)) {
                    if (part.trackable) {
                        buttons += makeIconButton('fa-hashtag icon-green', 'button-add-by-sn', pk, 'Asignar números de serie');
                    }
                    buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-add', pk, 'Asignar stock');
                    if (part.purchaseable) {
                        buttons += makeIconButton('fa-shopping-cart', 'button-buy', row.part, 'Comprar stock');
                    }

                    if (part.assembly) {
                        buttons += makeIconButton('fa-tools', 'button-build', row.part, 'Stock de Trabajo');
                    }
                }
            }

            buttons += makeIconButton('fa-dollar-sign icon-green', 'button-price', pk, 'Calcular precio');

            if (options.allow_edit) {
                buttons += makeCopyButton('button-duplicate', pk, 'Duplicar artículo de línea');
                buttons += makeEditButton('button-edit', pk, 'Editar artículo de línea');
            }

            if (options.allow_delete) {
                var delete_disabled = false;

                var title = 'Eliminar artículo de línea';

                if (row.shipped) {
                    delete_disabled = true;
                    title = 'No se puede eliminar ya que los artículos han sido enviados';
                } else if (row.allocated) {
                    delete_disabled = true;
                    title = 'No se puede eliminar ya que los artículos han sido asignados';
                }
            ...
            inventreeGet(`/api/order/so-line/${pk}/`, {}, {
                success: function(data) {
                    ...
                    constructForm('/api/order/so-line/', {
                        ...
                        fields: fields,
                        data: data,
                        title: 'Duplicar artículo de línea',
                        refreshTable: table,
                    });
                }
            });
            ...
            inventreeGet(`/api/order/so-line/${pk}/`, {},
                {
                    success: function(response) {
                        ...
                        constructForm(`/api/order/so/${options.order}/allocate-serials/`, {
                            method: 'POST',
                            title: 'Asignar Números de Serie',
                            fields: {
                                line_item: {
                                    value: pk,
                                    hidden: true,
            ...
            launchModalForm(
                '/order/purchase-order/pricing/',
                {
                    ...
                    buttons: [
                        {
                            name: 'update_price',
                            title: 'Actualizar precio unitario'
                        },
                    ],
                    success: reloadTable,
                }
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'No hay artículos de línea coincidentes';
        },
        queryParams: filters,
        original: options.params,
        url: options.url,
