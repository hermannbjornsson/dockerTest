...
function editManufacturerPart(part, options={}) {
    ...
    constructForm(url, {
        fields: fields,
        title: 'Редактировать деталь производителя',
        onSuccess: options.onSuccess
    });
}
...
function createSupplierPart(options={}) {
    ...
    constructForm('/api/company/part/', {
        fields: fields,
        method: 'POST',
        title: 'Добавить деталь поставщика',
        onSuccess: options.onSuccess,
        header_html: header,
    });
}
...
function editSupplierPart(part, options={}) {
    ...
    constructForm(`/api/company/part/${part}/`, {
        fields: fields,
        title: options.title || 'Редактировать деталь поставщика',
        onSuccess: options.onSuccess
    });
}
...
function deleteSupplierParts(parts, options={}) {
    ...
    Все выбранные детали поставщика будут удалены
    </div>
    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.is_customer) {
                    html += `<span title='Покупатель' class='fas fa-user-tie float-right'></span>`;
                }

                if (row.is_manufacturer) {
                    html += `<span title='Производитель' class='fas fa-industry float-right'></span>`;
                }

                if (row.is_supplier) {
                    html += `<span title='Поставщик' class='fas fa-building float-right'></span>`;
                }
...
function loadManufacturerPartTable(table, url, options) {
    ...
    $(table).inventreeTable({
        ...
        groupBy: false,
        formatNoMatches: function() {
            return 'Информация о детали производителя не найдена';
        },
        columns: [
            {
                checkbox: true,
...
function makeSupplierPartActions(options={}) {
    return [
        ...
        {
            label: 'delete',
            title: 'Удалить деталь поставщика',
            icon: 'fa-trash-alt icon-red',
            permission: 'purchase_order.delete',
            callback: function(data) {
...
function loadSupplierPartTable(table, url, options) {
    ...
    $(table).inventreeTable({
        ...
        sortable: true,
        formatNoMatches: function() {
            return 'Информация о детали поставщика не найдена';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-supplier-part-edit', pk, 'Редактировать деталь поставщика');
                    html += makeDeleteButton('button-supplier-part-delete', pk, 'Удалить деталь поставщика');
