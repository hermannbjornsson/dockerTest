...
function createNewStockItem(options={}) {
    ...
    options.successMessage = 'Το αντικείμενο αποθεμάτων δημιουργήθηκε';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                var details = `
                <br>Ποσότητα: ${response.quantity}
                <br>Σειριακοί αριθμοί: ${response.serial_numbers}
                `;
...
function assignStockToCustomer(items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.serial && item.quantity == 1) {
            ...
            quantity = `Ποσότητα: ${item.quantity}`;
        }
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        text = 'Κατανάλωση με εντολή κατασκευής';
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Έγινε συγχώνευση αποθεμάτων', {
                            style: 'success',
                        });
                    }
                });
    ...
    cols.push({
        ...
        formatter: function(details, row) {
            ...
            if (details.buildorder) {
                html += `<tr><th>Σειρά Κατασκευής</th>`;
                html += `<td>`;
            ...
            if (details.stockitem) {
                html += '<tr><th>Στοιχείο Αποθέματος</td>';
