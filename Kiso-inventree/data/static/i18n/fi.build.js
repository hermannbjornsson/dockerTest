...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Sarjanumero: ${output.serial}`;
    } else {
        output_html += `Määrä: ${output.quantity}`;
    ...
    buttons += makeRemoveButton('button-row-remove', pk, 'Poista rivi');
