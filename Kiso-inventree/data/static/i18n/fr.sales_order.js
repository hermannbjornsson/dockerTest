            ...
            if (!allocations || allocations.length == 0) {
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        stock = `Numéro de série: ${allocation.serial}`;
                    } else {
                        stock = `Quantité: ${allocation.quantity}`;
                    }
        ...
        `/api/order/so/${order_id}/complete/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.pending_shipments) {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    La commande ne peut pas être terminée car il y a des envois incomplets<br>
                    </div>`;
                }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Commande en retard');
                    }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    var text = `Article en stock ${row.item}`;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `Quantité: ${row.quantity}`;

                    if (item && item.serial != null && row.quantity == 1) {
                        text = `Numéro de série: ${item.serial}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        return `<em>Livré au client - ${row.shipment_date}</em>`;
                    } else if (row.location) {
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || 'Emplacement',
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        html += `<span class='badge bg-success badge-right'>Expédié</span>`;
                    } else {
        ...
        formatter: function(value, row, index, field) {
            ...
            if (row.part && row.part_detail) {
                ...
                if (options.allow_edit && (row.shipped < row.quantity)) {
                    if (part.trackable) {
                        buttons += makeIconButton('fa-hashtag icon-green', 'button-add-by-sn', pk, 'Allouer des numéros de série');
                    }
                    buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-add', pk, 'Allocate stock');
                    if (part.purchaseable) {
                        buttons += makeIconButton('fa-shopping-cart', 'button-buy', row.part, 'Acheter du stock');
                    }
            ...
            buttons += makeIconButton('fa-dollar-sign icon-green', 'button-price', pk, 'Calculer le prix');
            ...
            inventreeGet(`/api/order/so-line/${pk}/`, {},
                {
                    success: function(response) {
                        ...
                        constructForm(`/api/order/so/${options.order}/allocate-serials/`, {
                            method: 'POST',
                            title: 'Allouer des numéros de série',
                            fields: {
                                line_item: {
                                    value: pk,
                                    hidden: true,
