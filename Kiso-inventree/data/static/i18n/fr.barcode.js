...
function makeBarcodeInput(placeholderText='', hintText='') {
    ...
    hintText = hintText || 'Saisir les données du code-barres';
    ...
    <div class='form-group'>
        <label class='control-label' for='barcode'>Code-barres</label>
        <div class='controls'>
...
function makeNotesField(options={}) {
    ...
    var tooltip = options.tooltip || 'Saisir les notes optionnelles pour le transfert de stock';
    var placeholder = options.placeholder || 'Saisir des notes';
            ...
            error: function(xhr) {
                ...
                case 400:
                    ...
                    if (options.onError400) {
                        ...
                        showBarcodeMessage(modal, data.error || 'Erreur serveur');
                    }
                    break;
                default:
...
function showInvalidResponseError(modal, response, status) {
    showBarcodeMessage(
        modal,
        `Réponse du serveur invalide<br>État: '${status}'`
    );
}
...
function barcodeScanDialog(options={}) {
    ...
    let title = options.title || 'Scanner le code-barres';
...
function unlinkBarcode(data, options={}) {
    ...
    var html = `<b>Délier le code-barre</b><br>`;

    html += 'This will remove the link to the associated barcode';

    showQuestionDialog(
        'Délier le code-barre',
        html,
        {
            accept_text: 'Délier',
            accept: function() {
    ...
    function reloadTable() {
        ...
        items.forEach(function(item) {
            ...
            <tr pk='${item.pk}'>
                ...
                <td>${location_info}</td>
                <td>${item.quantity}</td>
                <td>${makeRemoveButton('button-item-remove', item.pk, 'Supprimer l\'article de stock')}</td>
            </tr>`;
        });
                    ...
                    inventreeGet(
                        ...
                        {},
                        {
                            success: function(stockitem) {
                                ...
                                if (duplicate) {
                                    showBarcodeMessage(modal, 'Article de stock déjà scanné', 'warning');
                                } else {

                                    if (stockitem.location == location_id) {
                                        showBarcodeMessage(modal, 'Article de stock déjà à cet emplacement');
                                        return;
                                    }
                                    ...
                                    showBarcodeMessage(modal, 'Article de stock ajouté', 'success');
                ...
                if ('stocklocation' in response) {
                    ...
                    inventreeGet(`/api/stock/location/${pk}/`, {}, {
                        ...
                        error: function() {
                            ...
                            showBarcodeMessage(
                                modal,
                                'Le code-barres ne correspond pas à un emplacement valide',
                                'warning',
                            );
                        }
                    });
