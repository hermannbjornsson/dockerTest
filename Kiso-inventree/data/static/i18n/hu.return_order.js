...
function createReturnOrder(options={}) {
    ...
    constructForm('/api/order/ro/', {
        method: 'POST',
        fields: fields,
        title: 'Visszavételi utasítás létrehozása',
...
function editReturnOrder(order_id, options={}) {
    ...
    constructForm(`/api/order/ro/${order_id}/`, {
        ...
        title: 'Visszavételi utasítás szerkesztése',
...
function issueReturnOrder(order_id, options={}) {
    ...
    constructForm(`/api/order/ro/${order_id}/issue/`, {
        method: 'POST',
        title: 'Visszavételi utasítás kiadása',
        confirm: true,
        preFormContent: html,
        onSuccess: function(response) {
            handleFormSuccess(response, options);
...
function cancelReturnOrder(order_id, options={}) {
    ...
    Biztosan törölni szeretnéd ezt a visszavételi utasítást?
    </div>`;
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Visszavételi utasítás törlése',
            confirm: true,
            preFormContent: html,
            onSuccess: function(response) {
                handleFormSuccess(response, options);
...
function completeReturnOrder(order_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Visszavételi utasítás befejezése',
            confirm: true,
            preFormContent: html,
            onSuccess: function(response) {
                handleFormSuccess(response, options);
...
function loadReturnOrderTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nem található visszavételi utasítás';
        },
...
function receiveReturnOrderItems(order_id, line_items, options={}) {
    ...
    constructForm(`/api/order/ro/${order_id}/receive/`, {
        ...
        title: 'Visszavételi utasítás tételeinek bevételezése',
    ...
    $(table).inventreeTable({
        ...
        columns: [
            ...
            },
            {
                ...
                title: 'Tétel',
                formatter: function(value, row) {
                    return renderLink(`Sorozatszám: ${row.item_detail.serial}`, `/stock/item/${row.item}/`);
                }
            },
            {
                field: 'reference',
                ...
                formatter: function(value, row) {
                    ...
                    if (options.allow_edit) {
                        ...
                        if (options.allow_receive && !row.received_date) {
                            buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-line-receive', pk, 'Tétel bevételezve');
                        }
