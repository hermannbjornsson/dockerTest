...
function makeBarcodeInput(placeholderText='', hintText='') {
    ...
    placeholderText = placeholderText || 'Scansiona il codice a barre usando uno scanner';

    hintText = hintText || 'Inserire il codice a barre';
    ...
    <div class='form-group'>
        <label class='control-label' for='barcode'>Codice a Barre</label>
        <div class='controls'>
            <div class='input-group'>
                ...
                <button title='Scansiona il codice a barre usando la webcam' id='barcode_scan_btn' type='button' class='btn btn-secondary' onclick='onBarcodeScanClicked()' style='display: none;'>
                    ${makeIcon('fa-camera')}
                </button>
            </div>
...
function makeNotesField(options={}) {
    ...
    var tooltip = options.tooltip || 'Inserire le note facoltative per il trasferimento delle scorte';
    var placeholder = options.placeholder || 'Inserire le note';
            ...
            error: function(xhr) {
                ...
                case 400:
                    ...
                    if (options.onError400) {
                        ...
                        showBarcodeMessage(modal, data.error || 'Problemi con il server');
                    }
                    break;
                default:
...
function showInvalidResponseError(modal, response, status) {
    showBarcodeMessage(
        modal,
        `Risposta del server non valida<br>Stato: '${status}'`
    );
}
...
function barcodeDialog(title, options={}) {
    ...
    var details = options.details || 'Scansione del codice a barre';
...
function barcodeScanDialog(options={}) {
    ...
    let title = options.title || 'Scansiona codice a barre';
...
function unlinkBarcode(data, options={}) {
    ...
    var html = `<b>Scollega Codice a Barre</b><br>`;

    html += 'Questo rimuoverà il collegamento al codice a barre associato';

    showQuestionDialog(
        'Scollega Codice a Barre',
        html,
        {
            accept_text: 'Scollega',
            accept: function() {
                inventreePut(
                    '/api/barcode/unlink/',
                    data,
...
function barcodeCheckInStockItems(location_id, options={}) {
    ...
    function reloadTable() {
        ...
        items.forEach(function(item) {
            ...
            <tr pk='${item.pk}'>
                ...
                <td>${location_info}</td>
                <td>${item.quantity}</td>
                <td>${makeRemoveButton('button-item-remove', item.pk, 'Rimuovere l\'articolo in magazzino')}</td>
            </tr>`;
        });
    ...
    barcodeDialog(
        'Scansione articoli di magazzino in sede',
        {
            details: 'Scansione del codice a barre dell\'articolo di magazzino per effettuare il check-in in questa sede',
            headerContent: table,
            preShow: function() {
            ...
            onSubmit: function() {
                ...
                if (entries.length == 0) {
                    showBarcodeMessage(modal, 'Non c\'è un codice a barre', 'warning');
                    return;
                }
            ...
            onScan: function(response) {
                if ('stockitem' in response) {
                    ...
                    inventreeGet(
                        ...
                        {},
                        {
                            success: function(stockitem) {
                                ...
                                if (duplicate) {
                                    showBarcodeMessage(modal, 'Articolo di magazzino già scansionato', 'warning');
                                } else {

                                    if (stockitem.location == location_id) {
                                        showBarcodeMessage(modal, 'Elemento in giacenza già in questa posizione');
                                        return;
                                    }
                                    ...
                                    showBarcodeMessage(modal, 'Aggiunta di un articolo di magazzino', 'success');
                    ...
                    showBarcodeMessage(modal, 'Il codice a barre non corrisponde a un articolo di magazzino valido', 'warning');
                }
            },
        }
    );
...
function barcodeCheckInStockLocations(location_id, options={}) {
    ...
    barcodeDialog(
        'Scansione delle scorte contenute in sede',
        {
            details: 'Scansionare il codice a barre di scorta contenuta per effettuare il check-in in questa sede',
            ...
            onScan: function(response) {
                if ('stocklocation' in response) {
                    ...
                    showBarcodeMessage(modal, 'Il codice a barre non corrisponde a una posizione di magazzino valida', 'warning');
                }
            }
        }
    );
                ...
                if ('stocklocation' in response) {
                    ...
                    inventreeGet(`/api/stock/location/${pk}/`, {}, {
                        ...
                        error: function() {
                            ...
                            showBarcodeMessage(
                                modal,
                                'Il codice a barre non corrisponde a una posizione valida',
                                'warning',
                            );
                        }
                    });
