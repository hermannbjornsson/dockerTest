...
function addBomItem(part_id, options={}) {
    ...
    constructForm('/api/bom/', {
        fields: fields,
        method: 'POST',
        title: 'Crear artículo para el BOM',
        focus: 'sub_part',
...
function constructBomUploadTable(data, options={}) {
    ...
    function constructRow(row, idx, fields) {
        ...
        buttons += makeInfoButton('button-row-data', idx, 'Mostrar datos de fila');
        buttons += makeRemoveButton('button-row-remove', idx, 'Eliminar fila');
...
function downloadBomTemplate(options={}) {
    ...
    constructFormBody({}, {
        title: 'Descargar plantilla BOM',
        fields: {
            format: {
                label: 'Formato',
                help_text: 'Seleccionar formato de archivo',
                required: true,
                type: 'choice',
                value: format,
...
function exportBom(part_id, options={}) {
    ...
    constructFormBody({}, {
        ...
        fields: {
            ...
            levels: {
                label: 'Niveles',
                help_text: 'Seleccione el número máximo de niveles BOM a exportar (0 = todos los niveles)',
                type: 'integer',
                value: 0,
                required: true,
                min_value: 0,
            ...
            substitute_part_data: {
                label: 'Incluye partes alternativas',
                help_text: 'Include alternative parts in exported BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-substitute_part_data', false),
            },
            parameter_data: {
                label: 'Incluye Parámetros de Datos',
                help_text: 'Include part parameter data in exported BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-parameter_data', false),
            },
            stock_data: {
                label: 'Incluye Datos de Stock',
                help_text: 'Incluye datos de stock de partes en BOM exportado',
                type: 'boolean',
                value: inventreeLoad('bom-export-stock_data', false),
            },
            manufacturer_data: {
                label: 'Incluir Datos del fabricante',
                help_text: 'Incluye datos del fabricante de partes en BOM exportado',
                type: 'boolean',
                value: inventreeLoad('bom-export-manufacturer_data', false),
            },
            supplier_data: {
                label: 'Incluir Datos del Proveedor',
                help_text: 'Incluye datos del proveedor de partes en BOM exportado',
                type: 'boolean',
...
function bomSubstitutesDialog(bom_item_id, substitutes, options={}) {
    ...
    function renderSubstituteRow(substitute) {
        ...
        buttons += makeRemoveButton('button-row-remove', pk, 'Eliminar parte sustituta');
    ...
    <div class='alert alert-success alert-block'>
        Seleccione y añada una nueva parte sustituta usando la siguiente entrada
    </div>
    `;
    ...
    function addRemoveCallback(modal, element) {
        $(modal).find(element).click(function() {
            ...
            ¿Está seguro que desea eliminar este enlace de la parte sustituta?
            </div>
            `;

            constructForm(`/api/bom/substitute/${pk}/`, {
                method: 'DELETE',
                title: 'Eliminar parte sustituta',
                preFormContent: pre,
                confirm: true,
                onSuccess: function() {
    ...
    constructForm('/api/bom/substitute/', {
        ...
        preFormContent: html,
        closeText: 'Cerrar',
        submitText: 'Añadir sustituto',
        title: 'Editar sustitutos de artículos BOM',
...
function deleteBomItems(items, options={}) {
    ...
    Todos los artículos BOM seleccionados serán eliminados
    </div>
    ...
    constructForm('/api/bom/', {
        method: 'DELETE',
        multi_delete: true,
        title: '¿Eliminar artículos BOM seleccionados?',
        form_data: {
            items: ids,
        },
        preFormContent: html,
...
function loadBomTable(table, options={}) {
    ...
    cols.push(
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.substitutes && row.substitutes.length > 0) {
                    html += makeIconBadge('fa-exchange-alt', 'Sustitutos Disponibles');
                }
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (row.consumable) {
                text += ` <small>(Consumible)</small>`;
            }

            if (row.optional) {
                text += ' <small>(Opcional)</small>';
            }
        ...
        cols.push({
            ...
            formatter: function(value, row) {
                ...
                if (row.part == options.parent_id) {
                    ...
                    var bValidate = makeIconButton('fa-check-circle icon-green', 'bom-validate-button', row.pk, 'Validar Artículo para el BOM');

                    var bValid = makeIconButton('fa-check-double icon-green', 'bom-valid-button', row.pk, 'Esta línea ha sido validada', {disabled: true});

                    var bSubs = makeIconButton('fa-exchange-alt icon-blue', 'bom-substitutes-button', row.pk, 'Editar partes sustitutas');

                    var bEdit = makeEditButton('bom-edit-button', row.pk, 'Editar Artículo de BOM');

                    var bDelt = makeDeleteButton('bom-delete-button', row.pk, 'Eliminar Artículo de BOM');
            ...
            footerFormatter: function(data) {
                return `
                <button class='btn btn-success float-right' type='button' title='Añadir artículo al BOM' id='bom-item-new-footer'>
                    ${makeIcon('fa-plus-circle')} Añadir artículo al BOM
                </button>
                `;
            }
        });
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'No se encontraron artículos BOM';
        },
        queryParams: filters,
        original: params,
        columns: cols,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.parent && row.parent != 'top-level-item') {
                        html += ` <em>(Heredado de BOM superior)</em>`;
                    }
