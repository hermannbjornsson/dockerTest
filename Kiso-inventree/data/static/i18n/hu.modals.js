...
function createNewModal(options={}) {
    ...
    if (!options.hideCloseButton) {
        buttons += `<button type='button' class='btn btn-secondary' id='modal-form-close' data-bs-dismiss='modal'>Mégsem</button>`;
    }
    ...
    modalSetTitle(modal_name, options.title || 'Form megnevezése');
    modalSetSubmitText(modal_name, options.submitText || 'Küldés');
    modalSetCloseText(modal_name, options.closeText || 'Mégsem');
...
function loadingMessageContent() {
    ...
    return `<span class='glyphicon glyphicon-refresh glyphicon-refresh-animate'></span> Várakozás a kiszolgálóra...`;
}
        ...
        <div class='panel'>
            <div class='panel panel-heading'>
                <div class='panel-title'>
                    <a data-bs-toggle='collapse' href="#collapse-error-info">Hibainformációk megjelenítése</a>
                </div>
            </div>
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.title = title;
    options.submitText = options.accept_text || 'Elfogadás';
    options.closeText = options.cancel_text || 'Mégsem';
...
function openModal(options) {
    ...
    if (options.title) {
        ...
        modalSetTitle(modal, 'Adatok betöltése...');
    }
    ...
    modalSubmit(modal, function() {
        $(modal).find('.js-modal-form').ajaxSubmit({
                ...
                if ('form_valid' in response) {
                    ...
                    if (response.form_valid) {
                        ...
                        if (response.html_form) {
                            ...
                            $(modal).modal('hide');
                            showAlertDialog('Rossz válasz a kiszolgálótól', 'Űrlap adat hiányzik a kiszolgálótól kapott válaszban');
                        }
                    }
                } else {
                    $(modal).modal('hide');
            ...
            error: function(xhr) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Form adat küldési hiba', renderErrorMessage(xhr));
            },
            complete: function() {
                // TODO
            }
    ...
    var ajax_data = {
        ...
        success: function(response) {
            ...
            if (response.html_form) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Érvénytelen válasz a szervertől', 'JSON válasz hiányzó form adatok');
            }
        },
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    'Nincs válasz',
                    'Nincs válasz az InvenTree kiszolgálótól',
                );
            } else if (xhr.status == 400) {
                showAlertDialog(
                    'Error 400: Rossz kérelem',
                    'A kiszolgáló 400-as hibakódot adott vissza',
                );
            } else if (xhr.status == 401) {
                showAlertDialog(
                    'Error 401: Nincs hitelesítve',
                    'Hitelesítési adatok nem lettek megadva',
                );
            } else if (xhr.status == 403) {
                showAlertDialog(
                    'Error 403: Hozzáférés megtagadva',
                    'Nincs meg a szükséges jogosultságod, hogy elérd ezt a funkciót',
                );
            } else if (xhr.status == 404) {
                showAlertDialog(
                    'Error 404: Erőforrás nem található',
                    'A kért erőforrás nem található a kiszolgálón',
                );
            } else if (xhr.status == 408) {
                showAlertDialog(
                    'Error 408: Időtúllépés',
                    'Időtúllépés a kiszolgálótól való adatlekérés közben',
                );
            } else {
                showAlertDialog('Form adat lekérése sikertelen', renderErrorMessage(xhr));
            }
