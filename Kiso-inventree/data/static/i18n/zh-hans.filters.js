...
function generateAvailableFilterList(tableKey) {
    ...
    html += `<option value=''>选择筛选项</option>`;
...
function makeCustomActionGroup(action_group, table) {
    ...
    let title = action_group.title || '操作';
...
function setupFilterList(tableKey, table, target, options={}) {
    ...
    if (report_button || labels_button) {
        ...
        if (labels_button) {
            print_buttons += `<li><a class='dropdown-item' href='#' id='print-labels-${tableKey}'><span class='fas fa-tag'></span> 打印标签</a></li>`;
        }
