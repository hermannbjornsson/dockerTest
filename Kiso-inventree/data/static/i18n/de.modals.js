...
function createNewModal(options={}) {
    ...
    if (!options.hideCloseButton) {
        buttons += `<button type='button' class='btn btn-secondary' id='modal-form-close' data-bs-dismiss='modal'>Abbrechen</button>`;
    }

    // Add in a "submit" button
    if (!options.hideSubmitButton) {
        buttons += `<button type='button' class='btn btn-${submitClass}' id='modal-form-submit'>Abschicken</button>`;
    }
    ...
    modalSetTitle(modal_name, options.title || 'Formulartitel');
    modalSetSubmitText(modal_name, options.submitText || 'Abschicken');
    modalSetCloseText(modal_name, options.closeText || 'Abbrechen');
...
function loadingMessageContent() {
    ...
    return `<span class='glyphicon glyphicon-refresh glyphicon-refresh-animate'></span> Warte auf Server...`;
}
        ...
        <div class='panel'>
            <div class='panel panel-heading'>
                <div class='panel-title'>
                    <a data-bs-toggle='collapse' href="#collapse-error-info">Fehler-Informationen anzeigen</a>
                </div>
            </div>
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.title = title;
    options.submitText = options.accept_text || 'Akzeptieren';
    options.closeText = options.cancel_text || 'Abbrechen';
...
function openModal(options) {
    ...
    if (options.title) {
        ...
        modalSetTitle(modal, 'Lade Daten...');
    }
    ...
    var submit_text = options.submit_text || 'Abschicken';
    var close_text = options.close_text || 'Schliessen';
    ...
    modalSubmit(modal, function() {
        $(modal).find('.js-modal-form').ajaxSubmit({
                ...
                if ('form_valid' in response) {
                    ...
                    if (response.form_valid) {
                        ...
                        if (response.html_form) {
                            ...
                            $(modal).modal('hide');
                            showAlertDialog('ungültige Antwort vom Server', 'Formulardaten fehlen bei Serverantwort');
                        }
                    }
                } else {
                    $(modal).modal('hide');
            ...
            error: function(xhr) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Formulardaten fehlerhaft', renderErrorMessage(xhr));
            },
            complete: function() {
                // TODO
            }
    ...
    var ajax_data = {
        ...
        success: function(response) {
            ...
            if (response.html_form) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Ungültige Antwort von Server', 'JSON Antwort enthält keine Formulardaten');
            }
        },
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    'Keine Antwort',
                    'keine Antwort vom InvenTree Server',
                );
            } else if (xhr.status == 400) {
                showAlertDialog(
                    'Fehler 400: Ungültige Anfrage',
                    'Fehler 400 von Server erhalten',
                );
            } else if (xhr.status == 401) {
                showAlertDialog(
                    'Fehler 401: Nicht Angemeldet',
                    'Authentication Kredentials nicht angegeben',
                );
            } else if (xhr.status == 403) {
                showAlertDialog(
                    'Fehler 403: keine Berechtigung',
                    'Fehlende Berechtigung für diese Aktion',
                );
            } else if (xhr.status == 404) {
                showAlertDialog(
                    'Fehler 404: Ressource nicht gefunden',
                    'Die angefragte Ressource kann auf diesem Server nicht gefunden werden',
                );
            } else if (xhr.status == 408) {
                showAlertDialog(
                    'Fehler 408: Zeitüberschreitung',
                    'Verbindungszeitüberschreitung bei der Datenanforderung',
                );
            } else {
                showAlertDialog('Fehler bei Formulardaten-Anfrage', renderErrorMessage(xhr));
            }
