...
function createSalesOrderLineItem(options={}) {
    ...
    constructForm('/api/order/so-line/', {
        fields: fields,
        method: 'POST',
        title: 'Dodaj element zamówienia',
        onSuccess: function(response) {
            handleFormSuccess(response, options);
        },
    });
            ...
            if (!allocations || allocations.length == 0) {
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        stock = `Numer Seryjny: ${allocation.serial}`;
                    } else {
                        stock = `Ilość: ${allocation.quantity}`;
                    }
...
function loadSalesOrderTable(table, options) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nie znaleziono zamówień sprzedaży';
        },
...
function loadSalesOrderShipmentTable(table, options={}) {
    ...
    function makeShipmentActions(row) {
        ...
        html += makeEditButton('button-shipment-edit', pk, 'Edytuj wysyłkę');

        if (!options.shipped) {
            html += makeIconButton('fa-truck icon-green', 'button-shipment-ship', pk, 'Kompletna wysyłka');
        }

        var enable_delete = row.allocations && row.allocations.length == 0;

        html += makeDeleteButton('button-shipment-delete', pk, 'Usuń wysyłkę', {disabled: !enable_delete});
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nie odnaleziono pasujących przesyłek';
        },
        columns: [
            {
                visible: false,
            ...
            {
                field: 'reference',
                title: 'Numer referencyjny przesyłki',
                switchable: false,
            },
            {
                field: 'allocations',
...
function allocateStockToSalesOrder(order_id, line_items, options={}) {
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Komponent</th>
                <th style='min-width: 250px;'>Element magazynowy</th>
                <th>Ilość</th>
                <th></th>
        </thead>
        <tbody>
    ...
    constructForm(`/api/order/so/${order_id}/allocate/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Potwierdź przydział zapasów',
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `Ilość: ${row.quantity}`;

                    if (item && item.serial != null && row.quantity == 1) {
                        text = `Numer Seryjny: ${item.serial}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        ...
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || 'Lokalizacja',
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        html += `<span class='badge bg-success badge-right'>Wysłane</span>`;
                    } else {
...
function loadSalesOrderLineItemTable(table, options={}) {
    ...
    columns.push({
        ...
        formatter: function(value, row, index, field) {
            ...
            if (row.part && row.part_detail) {
                ...
                if (options.allow_edit && (row.shipped < row.quantity)) {
                    ...
                    if (part.purchaseable) {
                        buttons += makeIconButton('fa-shopping-cart', 'button-buy', row.part, 'Cena zakupu');
                    }
            ...
            buttons += makeIconButton('fa-dollar-sign icon-green', 'button-price', pk, 'Oblicz cenę');
            ...
            launchModalForm(
                '/order/purchase-order/pricing/',
                {
                    ...
                    buttons: [
                        {
                            name: 'update_price',
                            title: 'Zaktualizuj cenę jednostkową'
                        },
                    ],
                    success: reloadTable,
                }
