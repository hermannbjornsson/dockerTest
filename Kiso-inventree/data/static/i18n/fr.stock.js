    ...
    if (options.part) {
        ...
        inventreeGet(`/api/part/${options.part}/serial-numbers/`, {}, {
            success: function(data) {
                if (data.next) {
                    options.fields.serial_numbers.placeholder = `Prochain numéro de série disponible: ${data.next}`;
                } else if (data.latest) {
                    options.fields.serial_numbers.placeholder = `Dernier numéro de série: ${data.latest}`;
                }
            },
            async: false,
        });
...
function stockItemFields(options={}) {
    var fields = {
                    ...
                    if (data.trackable) {
                        ...
                        inventreeGet(`/api/part/${data.pk}/serial-numbers/`, {}, {
                            success: function(data) {
                                var placeholder = '';
                                if (data.next) {
                                    placeholder = `Prochain numéro de série disponible: ${data.next}`;
                                } else if (data.latest) {
                                    placeholder = `Dernier numéro de série: ${data.latest}`;
                                }
        ...
        serial_numbers: {
            ...
            help_text: 'Entrez les numéros de série pour le nouveau stock (ou laisser vide)',
            required: false,
        },
        serial: {
            icon: 'fa-hashtag',
...
function createNewStockItem(options={}) {
    ...
    options.title = 'Nouvel article de stock';
    options.method = 'POST';
    ...
    options.successMessage = 'Article en stock créé';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                var details = `
                <br>Quantité: ${response.quantity}
                <br>Numéros de série: ${response.serial_numbers}
                `;
...
function findStockItemBySerialNumber(part_id) {
    ...
    constructFormBody({}, {
        title: 'Trouver un numéro de série',
        fields: {
            serial: {
                label: 'Numéro de série',
                help_text: 'Entrer le numéro de série',
                    ...
                    success: function(response) {
                        if (response.length == 0) {
                            ...
                            handleFormErrors(
                                {
                                    'serial': [
                                        'Aucun numéro de série correspondant',
                                    ]
                                }, fields, opts
                            );
                            ...
                            handleFormErrors(
                                {
                                    'serial': [
                                        'Plus d\'un résultat correspondant trouvé',
                                    ]
                                }, fields, opts
                            );
                        } else {
...
function assignStockToCustomer(items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.serial && item.quantity == 1) {
            quantity = `Numéro de série: ${item.serial}`;
        } else {
            quantity = `Quantité: ${item.quantity}`;
        }
    ...
    constructForm('/api/stock/assign/', {
        ...
        confirm: true,
        confirmMessage: 'Confirmer l\'assignation de stock',
        title: 'Assigner le stock au client',
...
function mergeStockItems(items, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    <h5>Attention : l\'opération de fusion est irréversible</h5>
    <strong>Certaines informations seront perdues lors de la fusion des articles en stock:</strong>
    <ul>
        <li>L\'historique des transactions de stock sera supprimé pour les éléments fusionnés</li>
        <li>Les informations sur la pièce du fournisseur seront supprimées pour les éléments fusionnés</li>
    </ul>
    </div>
    `;
    ...
    constructForm('/api/stock/merge/', {
        ...
        confirm: true,
        confirmMessage: 'Confirmer la fusion de l\'article en stock',
        title: 'Fusionner les articles en stock',
        afterRender: function(fields, opts) {
            // Add button callbacks to remove rows
            $(opts.modal).find('.button-stock-item-remove').click(function() {
                var pk = $(this).attr('pk');
...
function adjustStock(action, items, options={}) {
    ...
    switch (action) {
    case 'move':
        formTitle = 'Transférer le stock';
        actionTitle = 'Transférer';
        specifyLocation = true;
        allowSerializedStock = true;
        url = '/api/stock/transfer/';
        break;
    case 'count':
        formTitle = 'Compter le stock';
        actionTitle = 'Compter';
        url = '/api/stock/count/';
        break;
    case 'take':
        formTitle = 'Supprimer du stock';
        actionTitle = 'Supprimer';
        url = '/api/stock/remove/';
        break;
    case 'add':
        formTitle = 'Ajouter du stock';
        actionTitle = 'Ajouter';
        url = '/api/stock/add/';
        break;
    case 'delete':
        formTitle = 'Supprimer le stock';
        allowSerializedStock = true;
        break;
    default:
        break;
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantity += ` - <small>Lot: ${item.batch}</small>`;
        }
        ...
        if (actionTitle != null) {
            actionInput = constructField(
                `items_quantity_${pk}`,
                {
                    ...
                    max_value: maxValue,
                    value: value,
                    title: readonly ? 'La quantité ne peut pas être ajustée pour un stock sérialisé' : 'Spécifiez la quantité du stock',
                    required: true,
                },
                {
                    hideLabels: true,
...
function passFailBadge(result) {
    ...
    if (result) {
        return `<span class='badge badge-right rounded-pill bg-success'>RÉUSSI</span>`;
    } else {
        return `<span class='badge badge-right rounded-pill bg-danger'>ÉCHEC</span>`;
    }
}

function noResultBadge() {
    return `<span class='badge badge-right rounded-pill bg-info'>AUCUN RÉSULTAT</span>`;
}
...
function loadStockTestResultsTable(table, options) {
    ...
    function makeButtons(row, grouped) {
        ...
        html += makeIconButton('fa-plus icon-green', 'button-test-add', row.test_name, 'Ajouter un résultat de test');

        if (!grouped && row.result != null) {
            var pk = row.pk;
            html += makeEditButton('button-test-edit', pk, 'Modifier le résultat du test');
            html += makeDeleteButton('button-test-delete', pk, 'Supprimer le résultat du test');
        }
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Aucun résultat de test trouvé';
        },
        queryParams: filters,
        original: params,
        onPostBody: function() {
    ...
    $(table).on('click', '.button-test-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
        <strong>Supprimer le résultat du test:</strong> ${row.test_name || row.test || row.key}
        </div>`;
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        text = 'Consommé par ordre de construction';
        ...
        text = '<i>Aucun emplacement de stock défini</i>';
        url = '';
    }
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Articles de stock fusionnés', {
                            style: 'success',
                        });
                    }
                });
...
function loadStockTable(table, options) {
    ...
    col = {
        ...
        formatter: function(value, row) {
            ...
            if (row.is_building) {
                html += makeIconBadge('fa-tools', 'L\'article de stock est en production');
            }

            if (row.sales_order) {
                // Stock item has been assigned to a sales order
                html += makeIconBadge('fa-truck', 'L\'article en stock a été assigné à une commande de vente');
            } else if (row.customer) {
                // StockItem has been assigned to a customer
                html += makeIconBadge('fa-user', 'L\'article en stock a été assigné à un client');
            } else if (row.allocated) {
                if (row.serial != null && row.quantity == 1) {
                    html += makeIconBadge('fa-bookmark icon-yellow', 'L\'article de stock sérialisé a été alloué');
                } else if (row.allocated >= row.quantity) {
                    html += makeIconBadge('fa-bookmark icon-yellow', 'L\'article de stock a été complètement alloué');
                } else {
                    html += makeIconBadge('fa-bookmark', 'L\'article de stock a été partiellement alloué');
                }
            } else if (row.belongs_to) {
                html += makeIconBadge('fa-box', 'L\'article en stock a été installé dans un autre article');
            } else if (row.consumed_by) {
                html += makeIconBadge('fa-tools', 'Stock item has been consumed by a build order');
            }

            if (row.expired) {
                html += makeIconBadge('fa-calendar-times icon-red', 'L\'article en stock a expiré');
            } else if (row.stale) {
                html += makeIconBadge('fa-stopwatch', 'L\'article en stock va bientôt expirer');
            }

            // Special stock status codes
            if (row.status == stockCodes.REJECTED.key) {
                html += makeIconBadge('fa-times-circle icon-red', 'L\'article de stock a été rejeté');
            } else if (row.status == stockCodes.LOST.key) {
                html += makeIconBadge('fa-question-circle', 'L\'article de stock est perdu');
            } else if (row.status == stockCodes.DESTROYED.key) {
                html += makeIconBadge('fa-skull-crossbones', 'L\'article de stock est détruit');
            }

            if (row.quantity <= 0) {
                html += `<span class='badge badge-right rounded-pill bg-danger'>Epuisé</span>`;
            }
    ...
    col = {
        ...
        formatter: function(value, row) {
            ...
            if (row.supplier_part_detail) {
                ...
                text = `<i>Pièce de fournisseur non précisée</i>`;
            }
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Aucun article de stock ne correspond à la requête';
        },
        ...
        buttons: options.allowTreeView ? [
            {
                icon: 'fas fa-bars',
                attributes: {
                    title: 'Afficher sous forme de liste',
            ...
            },
            {
                icon: 'fas fa-sitemap',
                attributes: {
                    title: 'Afficher sous forme d\'arborescence',
        ...
        formatter: function(details, row) {
            ...
            if (details.location) {
                ...
                if (details.location_detail) {
                    ...
                    html += `<i>L\'emplacement n\'existe plus</i>`;
                }
            ...
            if (details.buildorder) {
                html += `<tr><th>Ordre de Fabrication</th>`;
                html += `<td>`;
            ...
            if (details.purchaseorder) {
                html += `<tr><th>Commande d’achat</th>`;
                html += '<td>';
                ...
                if (details.purchaseorder_detail) {
                    ...
                    html += `<i>La commande d\'achat n\'existe plus</i>`;
                }
            ...
            if (details.customer) {
                ...
                if (details.customer_detail) {
                    ...
                    html += `<i>Le client n\'existe plus</i>`;
                }
            ...
            if (details.stockitem) {
                html += '<tr><th>Article en stock</td>';
                ...
                if (details.stockitem_detail) {
                    ...
                    html += `<i>L\'article de stock n\'existe plus</i>`;
                }
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            if (value) {
                ...
                return `<i>Pas d\'informations sur l\'utilisateur</i>`;
            }
        }
    });
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        html += `Numéro de série: ${row.serial}`;
                    } else {
                        html += `${row.quantity}`;
                    }
