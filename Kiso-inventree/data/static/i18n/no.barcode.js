...
function makeBarcodeInput(placeholderText='', hintText='') {
    ...
    placeholderText = placeholderText || 'Skann strekkodedata her ved å bruke strekkodeleser';

    hintText = hintText || 'Angi strekkodedata';
    ...
    <div class='form-group'>
        <label class='control-label' for='barcode'>Strekkode</label>
        <div class='controls'>
            <div class='input-group'>
                ...
                <button title='Skann strekkode ved hjelp av tilkoblet webkamera' id='barcode_scan_btn' type='button' class='btn btn-secondary' onclick='onBarcodeScanClicked()' style='display: none;'>
                    ${makeIcon('fa-camera')}
                </button>
            </div>
...
function makeNotesField(options={}) {
    ...
    var tooltip = options.tooltip || 'Angi valgfrie notater for lageroverføring';
    var placeholder = options.placeholder || 'Skriv inn notater';

    return `
    <div class='form-group'>
        <label class='control-label' for='notes'>Notater</label>
        <div class='controls'>
            <div class='input-group'>
                <span class='input-group-text'>
            ...
            error: function(xhr) {
                ...
                case 400:
                    ...
                    if (options.onError400) {
                        ...
                        showBarcodeMessage(modal, data.error || 'Serverfeil');
                    }
                    break;
                default:
...
function showInvalidResponseError(modal, response, status) {
    showBarcodeMessage(
        modal,
        `Ugyldig svar fra serveren<br>Status: '${status}'`
    );
}
...
function barcodeDialog(title, options={}) {
    ...
    var details = options.details || 'Skann strekkodedata';
...
function barcodeScanDialog(options={}) {
    ...
    let title = options.title || 'Skann strekkode';
...
function unlinkBarcode(data, options={}) {
    ...
    var html = `<b>Fjern strekkodekobling</b><br>`;

    html += 'Dette vil fjerne lenken til den tilknyttede strekkoden';

    showQuestionDialog(
        'Fjern strekkodekobling',
        html,
        {
            accept_text: 'Koble fra',
            accept: function() {
                inventreePut(
                    '/api/barcode/unlink/',
                    data,
...
function barcodeCheckInStockItems(location_id, options={}) {
    ...
    function reloadTable() {
        ...
        items.forEach(function(item) {
            ...
            <tr pk='${item.pk}'>
                ...
                <td>${location_info}</td>
                <td>${item.quantity}</td>
                <td>${makeRemoveButton('button-item-remove', item.pk, 'Fjern lagervare')}</td>
            </tr>`;
        });
    ...
    barcodeDialog(
        'Skann lagervarer til plassering',
        {
            details: 'Skann lagervarens strekkode for å sjekke inn på denne plasseringen',
            headerContent: table,
            preShow: function() {
