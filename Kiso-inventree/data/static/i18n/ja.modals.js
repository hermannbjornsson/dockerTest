...
function createNewModal(options={}) {
    ...
    if (!options.hideSubmitButton) {
        buttons += `<button type='button' class='btn btn-${submitClass}' id='modal-form-submit'>確定</button>`;
    }
    ...
    modalSetSubmitText(modal_name, options.submitText || '確定');
...
function openModal(options) {
    ...
    var submit_text = options.submit_text || '確定';
    var close_text = options.close_text || '閉じる';
