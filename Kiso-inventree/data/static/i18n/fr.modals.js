...
function createNewModal(options={}) {
    ...
    if (!options.hideCloseButton) {
        buttons += `<button type='button' class='btn btn-secondary' id='modal-form-close' data-bs-dismiss='modal'>Annuler</button>`;
    }
    ...
    modalSetCloseText(modal_name, options.closeText || 'Annuler');
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.closeText = options.cancel_text || 'Annuler';
    ...
    var ajax_data = {
        ...
        success: function(response) {
            ...
            if (response.html_form) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Réponse du serveur invalide', 'JSON response missing form data');
            }
        },
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    'Aucune réponse',
                    'Aucune réponse du serveur InvenTree',
                );
                ...
                showAlertDialog(
                    ...
                    'Vous n\'avez pas les autorisations requises pour accéder à cette fonction',
                );
                ...
                showAlertDialog(
                    'Erreur 408: Délai dépassé',
                    'Délai de connexion dépassé lors de la demande de données depuis le serveur',
                );
            } else {
