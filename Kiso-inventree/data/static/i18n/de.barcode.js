...
function makeBarcodeInput(placeholderText='', hintText='') {
    ...
    placeholderText = placeholderText || 'Barcode Daten hier mit Barcode Scanner scannen';

    hintText = hintText || 'Barcode-Daten eingeben';
    ...
    <div class='form-group'>
        ...
        <div class='controls'>
            <div class='input-group'>
                ...
                <button title='Barcode mittels angeschlossener Webcam scannen' id='barcode_scan_btn' type='button' class='btn btn-secondary' onclick='onBarcodeScanClicked()' style='display: none;'>
                    ${makeIcon('fa-camera')}
                </button>
            </div>
...
function makeNotesField(options={}) {
    ...
    var tooltip = options.tooltip || 'Optionale Notizen zu Bestandsübertragung eingeben';
    var placeholder = options.placeholder || 'Notizen eingeben';

    return `
    <div class='form-group'>
        <label class='control-label' for='notes'>Notizen</label>
        <div class='controls'>
            <div class='input-group'>
                <span class='input-group-text'>
            ...
            error: function(xhr) {
                ...
                case 400:
                    ...
                    if (options.onError400) {
                        ...
                        showBarcodeMessage(modal, data.error || 'Server-Fehler');
                    }
                    break;
                default:
            ...
            success: function(response, status) {
                ...
                if (status == 'success') {
                    ...
                    if ('success' in response) {
                        ...
                        showBarcodeMessage(
                            modal,
                            'Unbekannte Antwort von Server erhalten',
                            'warning'
                        );
                    }
                } else {
...
function showInvalidResponseError(modal, response, status) {
    showBarcodeMessage(
        modal,
        `Ungültige Antwort von Server<br>Status: '${status}'`
    );
}
...
function barcodeDialog(title, options={}) {
    ...
    var details = options.details || 'Barcode Daten scannen';
...
function barcodeScanDialog(options={}) {
    ...
    let title = options.title || 'Barcode scannen';
...
function unlinkBarcode(data, options={}) {
    ...
    var html = `<b>Barcode abhängen</b><br>`;

    html += 'Dadurch wird der Link zu dem zugehörigen Barcode entfernt';

    showQuestionDialog(
        'Barcode abhängen',
        html,
        {
            accept_text: 'Entfernen',
            accept: function() {
                inventreePut(
                    '/api/barcode/unlink/',
                    data,
...
function barcodeCheckInStockItems(location_id, options={}) {
    ...
    function reloadTable() {
        ...
        items.forEach(function(item) {
            ...
            <tr pk='${item.pk}'>
                ...
                <td>${location_info}</td>
                <td>${item.quantity}</td>
                <td>${makeRemoveButton('button-item-remove', item.pk, 'Lagerartikel entfernen')}</td>
            </tr>`;
        });
    ...
    barcodeDialog(
        'Lagerartikel in Lagerort buchen',
        {
            details: 'Barcode des Lagerartikels scannen um ihn an diesen Ort einzuchecken',
            headerContent: table,
            preShow: function() {
                modalSetSubmitText(modal, 'Einbuchen');
                modalEnable(modal, false);
                reloadTable();
            },
            onShow: function() {
            ...
            onSubmit: function() {
                ...
                if (entries.length == 0) {
                    showBarcodeMessage(modal, 'Kein Barcode vorhanden', 'warning');
                    return;
                }
            ...
            onScan: function(response) {
                if ('stockitem' in response) {
                    ...
                    inventreeGet(
                        ...
                        {},
                        {
                            success: function(stockitem) {
                                ...
                                if (duplicate) {
                                    showBarcodeMessage(modal, 'Lagerartikel bereits gescannt', 'warning');
                                } else {

                                    if (stockitem.location == location_id) {
                                        showBarcodeMessage(modal, 'Lagerartikel besteht bereits in diesem Lagerort');
                                        return;
                                    }
                                    ...
                                    showBarcodeMessage(modal, 'Lagerartikel hinzugefügt', 'success');
                    ...
                    showBarcodeMessage(modal, 'Barcode entspricht keinem Lagerartikel', 'warning');
                }
            },
        }
    );
...
function barcodeCheckInStockLocations(location_id, options={}) {
    ...
    barcodeDialog(
        'Diesen Lagerort per Scan an einen anderen Lagerort verschieben',
        {
            details: 'Barcode des Lagerorts scannen um ihn an diesen Ort einzuchecken',
            ...
            onScan: function(response) {
                if ('stocklocation' in response) {
                    ...
                    showBarcodeMessage(modal, 'Barcode entspricht keinem Lagerort', 'warning');
                }
            }
        }
    );
                ...
                if ('stocklocation' in response) {
                    ...
                    inventreeGet(`/api/stock/location/${pk}/`, {}, {
                        ...
                        error: function() {
                            ...
                            showBarcodeMessage(
                                modal,
                                'Barcode entspricht keinem Lagerort',
                                'warning',
                            );
                        }
                    });
