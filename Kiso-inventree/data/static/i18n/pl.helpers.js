...
function yesNoLabel(value, options={}) {
    ...
    if (toBool(value)) {
        text = options.pass || 'TAK';
        color = 'bg-success';
    } else {
        text = options.fail || 'Nie';
        color = 'bg-warning';
    }
...
function renderClipboard(s, prepend=false) {
    ...
    let clipString = `<span class="d-none d-xl-inline"><button class="btn clip-btn" type="button" data-bs-toggle='tooltip' title='skopiuj do schowka'><em class="fas fa-copy"></em></button></span>`;
