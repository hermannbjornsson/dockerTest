...
function yesNoLabel(value, options={}) {
    ...
    if (toBool(value)) {
        text = options.pass || 'IGEN';
        color = 'bg-success';
    } else {
        text = options.fail || 'NEM';
        color = 'bg-warning';
    }
...
function editButton(url, text='Szerkesztés') {
...
function renderClipboard(s, prepend=false) {
    ...
    let clipString = `<span class="d-none d-xl-inline"><button class="btn clip-btn" type="button" data-bs-toggle='tooltip' title='vágólapra másolás'><em class="fas fa-copy"></em></button></span>`;
