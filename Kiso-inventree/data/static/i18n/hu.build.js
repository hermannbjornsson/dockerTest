...
function editBuildOrder(pk) {
    ...
    constructForm(`/api/build/${pk}/`, {
        fields: fields,
        reload: true,
        title: 'Gyártási utasítás szerkesztése',
    });
}
...
function newBuildOrder(options={}) {
    ...
    constructForm(`/api/build/`, {
        ...
        follow: true,
        method: 'POST',
        title: 'Gyártási utasítás létrehozása',
...
function cancelBuildOrder(build_id, options={}) {
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Gyártási utasítás törlése',
            confirm: true,
            fields: {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Biztosan meg szeretnéd szakítani ezt a gyártást?
                </div>`;

                if (opts.context.has_allocated_stock) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Ehhez a gyártáshoz készlet lett hozzárendelve
                    </div>`;
                }
                ...
                if (opts.context.incomplete_outputs) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Ennek a gyártásnak befejezetlen kimenetei vannak
                    </div>`;
                }
    ...
    constructForm(`/api/build/${build_id}/finish/`, {
        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                Gyártási utasítás készen áll a befejezésre'
                </div>`;
            } else {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>A gyártási utasítás befejezetlen kimeneteket tartalmaz</strong><br>
                    A rendelés nem jelölhető késznek mivel függő kimenetek vannak
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    <strong>Gyártási utasítás befejezetlen</strong>
                    </div>
                    `;
                }

                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>A szükséges készlet nem lett teljesen lefoglalva</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>Szükséges gyártási mennyiség nem lett elérve</div>`;
                }
            }
        ...
        reload: true,
        confirm: true,
        title: 'Gyártási utasítás befejezése',
        method: 'POST',
    });
}
        ...
        {},
        {
            success: function(build) {
                ...
                inventreeGet(`/api/part/${build.part}/serial-numbers/`, {}, {
                    success: function(data) {
                        if (data.next) {
                            fields.serial_numbers.placeholder = `Következő szabad sorozatszám: ${data.next}`;
                        } else if (data.latest) {
                            fields.serial_numbers.placeholder = `Legutolsó sorozatszám: ${data.latest}`;
                        }
                    },
                    async: false,
                });
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Az alkatrészjegyzék követésre kötelezett alkatrészeket tartalmaz.<br>
                        A gyártási kimeneteket egyesével kell előállítani.
                    </div>
                    `;
                }
                ...
                if (trackable) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        A követésre kötelezett alkatrészekhez sorozatszámot lehet rendelni<br>
                        Adj meg sorozatszámokat a több egyedi gyártási kimenet létrehozásához
                    </div>
                    `;
                }
                ...
                constructForm(`/api/build/${build_id}/create-output/`, {
                    method: 'POST',
                    title: 'Gyártási kimenet létrehozása',
                    confirm: true,
                    fields: fields,
                    preFormContent: html,
                    onSuccess: function(response) {
...
function makeBuildOutputButtons(output_id, build_info, options={}) {
    ...
    if (options.has_tracked_lines) {
        ...
        html += makeIconButton(
            ...
            output_id,
            'Készlet tételek foglalása ehhez a gyártási kimenethez',
        );
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Sorozatszám: ${output.serial}`;
    } else {
        output_html += `Mennyiség: ${output.quantity}`;
    ...
    buttons += makeRemoveButton('button-row-remove', pk, 'Sor törlése');
...
function completeBuildOutputs(build_id, outputs, options={}) {
    ...
    if (outputs.length == 0) {
        showAlertDialog(
            'Gyártási kimenetek kiválasztása',
            'Legalább egy gyártási kimenetet ki kell választani',
        );
        return;
    }
    ...
    constructForm(`/api/build/${build_id}/complete/`, {
        ...
        confirm: true,
        title: 'Gyártási kimenetek befejezése',
...
function scrapBuildOutputs(build_id, outputs, options={}) {
    ...
    constructForm(`/api/build/${build_id}/scrap-outputs/`, {
        ...
        confirm: true,
        title: 'Gyártási kimenetek selejtezése',
        afterRender: function(fields, opts) {
            // Setup callbacks to remove outputs
            $(opts.modal).find('.button-row-remove').click(function() {
                let pk = $(this).attr('pk');
...
function deleteBuildOutputs(build_id, outputs, options={}) {
    ...
    constructForm(`/api/build/${build_id}/delete-outputs/`, {
        ...
        fields: {},
        confirm: true,
        title: 'Gyártási kimenetek törlése',
        afterRender: function(fields, opts) {
            // Setup callbacks to remove outputs
            $(opts.modal).find('.button-row-remove').click(function() {
                var pk = $(this).attr('pk');
...
function loadBuildOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nincs gyártási utasításhoz történő foglalás';
        },
        columns: [
            {
                field: 'pk',
...
function loadBuildOutputTable(build_info, options={}) {
    ...
    setupFilterList('builditems', $(table), options.filterTarget || '#filter-list-incompletebuilditems', {
        ...
        singular_name: 'gyártás kimenet',
        plural_name: 'gyártás kimenetek',
        custom_actions: [{
            label: 'buildoutput',
            icon: 'fa-tools',
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nem található aktív gyártási kimenet';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        text = `Sorozatszám: ${row.serial}`;
                    } else {
                        text = `Mennyiség: ${row.quantity}`;
...
function allocateStockToBuild(build_id, line_items, options={}) {
    ...
    if (line_items.length == 0) {
        ...
        showAlertDialog(
            ...
            'Legalább egy alkatrész választása szükséges a foglaláshoz',
        );
    ...
    function renderBuildLineRow(build_line, quantity) {
        ...
        var quantity_input = constructField(
            `items_quantity_${pk}`,
            {
                ...
                min_value: 0,
                value: quantity || 0,
                title: 'Készlet foglalási mennyiség megadása',
                required: true,
            },
            {
                hideLabels: true,
    ...
    if (table_entries.length == 0) {
        ...
        showAlertDialog(
            ...
            'Minden kiválasztott alkatrész teljesen lefoglalva',
        );
    ...
    html += constructField(
        'take_from',
        {
            type: 'related field',
            label: 'Forrás hely',
            help_text: 'Válassz forrás helyet (vagy hagyd üresen ha bárhonnan)',
            required: false,
        },
        {},
    );
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Alkatrész</th>
                <th>Lefoglalva</th>
                <th style='min-width: 250px;'>Készlet tétel</th>
                <th>Mennyiség</th>
                <th></th>
            </tr>
        </thead>
    ...
    constructForm(`/api/build/${build_id}/allocate/`, {
        ...
        fields: {},
        preFormContent: html,
        title: 'Készlet foglalása a gyártási utasításhoz',
        afterRender: function(fields, options) {
            ...
            var take_from_field = {
                ...
                noResults: function(query) {
                    return 'Nincs egyező készlethely';
                },
            };
...
function autoAllocateStockToBuild(build_id, bom_items=[], options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
    <strong>Automatikus készlet foglalás</strong><br>
    A készlet automatikusan lefoglalásra kerül ehhez a gyártási utasításhoz, a következő feltételek szerint:
    <ul>
        <li>Ha egy készlet hely meg van adva, akkor készlet csak arról a helyről lesz foglalva</li>
        <li>Ha a készlet helyettesíthetőnek minősül, akkor az első rendelkezésre álló helyről lesz lefoglalva</li>
        <li>Ha a helyettesítő készlet engedélyezve van, akkor ott az lesz használva ha az elsődleges alkatrésznek nincs készlete</li>
    </ul>
    </div>
    `;
    ...
    constructForm(`/api/build/${build_id}/auto-allocate/`, {
        method: 'POST',
        fields: fields,
        title: 'Készlet tételek foglalása',
        confirm: true,
        preFormContent: html,
        onSuccess: function(response) {
            if (options.onSuccess) {
...
function loadBuildTable(table, options) {
    ...
    $(table).inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Nincs a lekérdezéssel egyező gyártási utasítás';
        },
        url: '/api/build/',
        queryParams: filters,
        groupBy: false,
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Gyártás késésben van');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.location_detail) {
                        ...
                        return '<i>Nincs beállítva hely</i>';
                    }
                }
            },
            {
                ...
                formatter: function(value, row) {
                    let buttons = '';
                    buttons += makeEditButton('button-allocation-edit', row.pk, 'Készlet foglalások szerkesztése');
                    buttons += makeDeleteButton('button-allocation-delete', row.pk, 'Készlet foglalások törlése');
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.allow_variants) {
                        html += makeIconBadge('fa-sitemap', 'Készletváltozatok engedélyezve');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.consumable) {
                        icons += `<span class='fas fa-info-circle icon-blue float-right' title='Fogyóeszköz tétel'></span>`;
                    } else {
                        if (available < (row.quantity - row.allocated)) {
                            icons += makeIconBadge('fa-times-circle icon-red', 'Nincs elegendő');
                        } else {
                            icons += makeIconBadge('fa-check-circle icon-green', 'Van elegendő');
                        }

                        if (available <= 0) {
                            icons += `<span class='badge rounded-pill bg-danger'>Nincs szabad</span>`;
                        } else {
                            let extra = '';
                            if ((row.available_substitute_stock > 0) && (row.available_variant_stock > 0)) {
                                extra = 'Változatokkal és helyettesítőkkel együtt';
                            } else if (row.available_variant_stock > 0) {
                                extra = 'Változatokkal együtt';
                            } else if (row.available_substitute_stock > 0) {
                                extra = 'Helyettesítőkkel együtt';
                            }
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `Rendelve: ${formatDecimal(row.on_order)}`);
                    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.allocated < row.quantity) {
                        ...
                        if (row.part_detail.assembly) {
                            buttons += makeIconButton('fa-tools icon-blue', 'button-build', pk, 'Gyártási készlet');
                        }

                        // Add a button to "purchase" stock for this line
                        if (row.part_detail.purchaseable) {
                            buttons += makeIconButton('fa-shopping-cart icon-blue', 'button-buy', pk, 'Készlet rendelés');
                        }

                        // Add a button to "allocate" stock for this line
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-allocate', pk, 'Lefoglalt készlet');
                    }
