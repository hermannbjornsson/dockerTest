            ...
            if (!allocations || allocations.length == 0) {
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        stock = `Серийный номер: ${allocation.serial}`;
                    } else {
                        stock = `Количество: ${allocation.quantity}`;
                    }
    ...
    constructForm(
        `/api/order/so/${order_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                var html = `
                <div class='alert alert-block alert-warning'>
                Отмена этого заказа означает, что заказ нельзя будет редактировать.
                </div>`;
...
function loadSalesOrderTable(table, options) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Заказы на продажу не найдены';
        },
...
function allocateStockToSalesOrder(order_id, line_items, options={}) {
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Детали</th>
                <th style='min-width: 250px;'>Предметы на складе</th>
                <th>Количество</th>
                <th></th>
        </thead>
        <tbody>
    ...
    constructForm(`/api/order/so/${order_id}/allocate/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Подтвердите выделение запасов',
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    var text = `Предметы на складе ${row.item}`;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `Количество: ${row.quantity}`;

                    if (item && item.serial != null && row.quantity == 1) {
                        text = `Серийный номер: ${item.serial}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        ...
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || 'Расположение',
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        html += `<span class='badge bg-success badge-right'>Доставлено</span>`;
                    } else {
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        text = `Серийный номер: ${row.serial}`;
                    } else {
                        text = `Количество: ${row.quantity}`;
                    }
