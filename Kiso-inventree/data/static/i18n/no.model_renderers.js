...
function renderStockItem(data, parameters={}) {
    ...
    if (data.quantity == 0) {
        ...
        if (data.serial && data.quantity == 1) {
            stock_detail = `Serienummer: ${data.serial}`;
        } else {
            if (render_available_quantity) {
                var available = data.quantity - data.allocated;
                stock_detail = `Tilgjengelig: ${available}`;
            } else {
                stock_detail = `Antall: ${data.quantity}`;
            }
        }

        if (data.batch) {
            stock_detail += ` - <small>Parti: ${data.batch}</small>`;
        }
    }
    ...
    return renderModel(
        {
            ...
            textSecondary: `Forsendelse ${data.reference}`,
        },
        parameters
    );
}
